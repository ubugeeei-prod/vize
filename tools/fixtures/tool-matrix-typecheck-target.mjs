...
export function validateTypecheckPerformanceTarget(
  ...
  // Optional: when the shared `vueGlobs` reach past what `tsconfig` owns, the
  // typechecker corpus is narrowed here instead, so one config is never asked
  // to answer for files it never included (#4454).
