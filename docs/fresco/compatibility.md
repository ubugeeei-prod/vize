...
| `useFocus`                         | `focus()`/`blur()` + focus events | `useCurrentFocusedElement`   | `useFocus`                         | Partial     | Ordered registry with `isActive`/`autoFocus`, parent IDs, descendant-focus state, and global Tab/Shift+Tab traversal; no traps or delegation yet (M1).     |
| `useFocusManager`                  | renderer focus APIs               | `useFocusManager`            | `useFocusManager`                  | Implemented | `focus`, `focusNext`/`focusPrevious`, `hasFocusedDescendant`, enable/disable, `activeId`, `focusableIds`.                                                  |
...
| Focus model                             | `focus`/`blur`, focus events, `hasFocusedDescendant`                        | Ordered registry, enable/disable, autoFocus, parent IDs, descendant state, and global Tab order; no traps or delegation yet (M1).                                                                                                                                                                  | Partial     |
