...
- [#493](https://github.com/ubugeeei/vize/issues/493): fresh-install smoke coverage for runtime package tarballs across supported OS and Node targets. The scheduled/manual hosted matrix covers darwin-x64 (`macos-15-intel`), darwin-arm64 (`blacksmith-12vcpu-macos-15`), linux-x64-gnu (`blacksmith-32vcpu-ubuntu-2404`), linux-arm64-gnu (`blacksmith-32vcpu-ubuntu-2404-arm`), win32-x64-msvc (`blacksmith-32vcpu-windows-2025`), and win32-arm64-msvc (`windows-11-arm`) on Node 22 and Node 24 without running on every PR push. Release tags remain blocked by tarball install smoke before npm publish. The remaining declared targets (linux-x64-musl and linux-arm64-musl) are documented as hosted-runner gaps in [stability.md](https://github.com/ubugeeei/vize/blob/main/docs/content/stability.md#runtime-support) and continue to receive build artifacts plus manual install smoke.
