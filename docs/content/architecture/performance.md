...
The historical numbers below were captured on a local workstation. For reproducible CI-hosted
numbers suitable for release notes and docs updates, use the
[Blacksmith benchmark snapshot](./performance-blacksmith) generated by the Tool Benchmark workflow.
