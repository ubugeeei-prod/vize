...
| `whitespace`           | `compiler.whitespace`、`vize({ whitespace })`、または `template.compilerOptions.whitespace` | Vue の `"condense"` (既定) と `"preserve"` を選択します。`preserve` では混在テキスト内の空白を保ちますが、兄弟ノード間の空白だけのノードは Vue と同様に 1 個のスペースになります。 |
