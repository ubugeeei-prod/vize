---
title: "Virtualizer"
description: "Headless Virtualizer; covers windowed rendering, scroll anchoring, scroll restoration."
---
...
Headless Virtualizer; covers windowed rendering, scroll anchoring, scroll restoration.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/virtualizer` |
| Maturity | stable |
| Own the source | `vize lib pull virtualizer` |
| Requires | [measure](./measure.md) |
| Aliases | virtual list, virtual scroll, windowing, infinite scroll |
| Covers | windowed rendering, scroll anchoring, scroll restoration |
...
```ts
function createGridVirtualizer(options: GridVirtualizerOptions): GridVirtualizerController
```
...
```ts
function useGridVirtualizer(options: GridVirtualizerOptions): GridVirtualizerController
```
...
```ts
function createInfiniteLoader(options: InfiniteLoaderOptions): InfiniteLoaderController
```
...
```ts
function useInfiniteLoader(options: InfiniteLoaderOptions): InfiniteLoaderController
```
...
```ts
function createMeasureCache(config: MeasureCacheConfig): MeasureCache
```

### `invalidOptionDiagnostic`

```ts
const invalidOptionDiagnostic
```

### `disposedDiagnostic`

```ts
const disposedDiagnostic
```
...
```ts
function resolveVirtualizerOptions(options: VirtualizerOptions): VirtualizerOptionReaders
```
...
```ts
function createVirtualizer(options: VirtualizerOptions): VirtualizerController
```
...
```ts
function useVirtualizer(options: VirtualizerOptions): VirtualizerController
```
...
```ts
function createViewportBinding(config: ViewportBindingConfig): ViewportBinding
```
...
```ts
function computeVirtualWindow(input: VirtualWindowInput): VirtualWindow
```
