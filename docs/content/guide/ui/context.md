...
description: "Headless Typed Component Context; covers Reka UI createContext, Radix context scope."
---

<!-- Generated by npm/ui/scripts/generate-reference-docs.ts from the catalogs and source TSDoc. Do not edit by hand. -->

# Typed Component Context

Headless Typed Component Context; covers Reka UI createContext, Radix context scope.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/context` |
| Maturity | stable |
| Own the source | `vize lib pull context` |
| Requires | — |
| Aliases | provide inject context, compound component context |
| Covers | Reka UI createContext, Radix context scope |

## API

### `createContext`

Creates an immutable typed context for a compound component family. A private sentinel distinguishes a missing provider from a provider whose value is explicitly `undefined`.

```ts
function createContext<Value>(name: string): ComponentContext<Value>
```
