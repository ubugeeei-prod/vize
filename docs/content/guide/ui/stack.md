---
title: "Stack"
description: "Headless Stack; covers CSS flexbox, CSS logical properties, Reka UI Primitive."
---
...
Headless Stack; covers CSS flexbox, CSS logical properties, Reka UI Primitive.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/stack` |
| Maturity | stable |
| Own the source | `vize lib pull stack` |
| Requires | — |
| Aliases | layout stack, vertical stack, logical stack, single-axis layout |
| Covers | CSS flexbox, CSS logical properties, Reka UI Primitive |

## Usage

```ts
import { Stack } from "@vizejs/ui/stack";
```

Or copy the source into your project with `vize lib pull stack` (see [Source Distribution](../lib-pull.md)).

## API

### `Stack`

Source: `src/families/layout/stack/stack.vue`
...
| `axis` | `StackAxis` | `"block"` | Logical flex main axis. `block` stacks vertically in the current writing mode; `inline` follows the document direction, including RTL. |
| `reversed` | `boolean` | `false` | Reverse the selected logical main axis without changing DOM order. |
| `gap` | `StackGap` | `"1rem"` | Native CSS `gap` value between direct children. |
| `align` | `StackAlign` | `"stretch"` | Native CSS `align-items` value for the cross axis. |
| `justify` | `StackJustify` | `"start"` | Native CSS `justify-content` value for the main axis. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `StackSlotState` | Renders stack children with the resolved logical layout state. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `axis` | `StackAxis` | Logical axis used for the flex main axis. |
| `reversed` | `boolean` | Whether the selected logical axis is reversed. |
| `direction` | `StackFlexDirection` | CSS `flex-direction` applied to the rendered host. |
| `gap` | `StackGap` | Native CSS `gap` value between direct children. |
| `align` | `StackAlign` | Native CSS `align-items` value for the cross axis. |
| `justify` | `StackJustify` | Native CSS `justify-content` value for the main axis. |
| `state` | `StackState` | Stable state token for styling and tests. |
| `element` | `StackElement \| null` | Rendered host element or component instance. |
