---
title: "Spinner"
description: "Accessible, unstyled status/progress indicator for pending work."
---
...
|  |  |
| --- | --- |
| Package | `@vizejs/ui/spinner` |
| Maturity | stable |
| Own the source | `vize lib pull spinner` |
| Requires | [id](./id.md) |
| Aliases | spinner, loading indicator, progress spinner, activity indicator |
| Covers | WAI-ARIA status role, WAI-ARIA progressbar role, MUI CircularProgress |

## Usage

```ts
import { Spinner } from "@vizejs/ui/spinner";
```

Or copy the source into your project with `vize lib pull spinner` (see [Source Distribution](../lib-pull.md)).

## API

### `Spinner`

Source: `src/families/feedback/spinner/spinner.vue`
...
| `id` | `string \| null` | `undefined` | Consumer-owned spinner id. `null` and `undefined` select a deterministic fallback. |
| `loading` | `boolean` | `true` | Whether the spinner represents pending work. |
| `visible` | `boolean` | `true` | Whether the spinner remains rendered and visible in layout. |
| `role` | `SpinnerRole` | `"status"` | Accessibility semantics for the host. |
| `value` | `number \| null` | `null` | Optional determinate progress value for `role="progressbar"`. |
| `min` | `number` | `0` | Lower progress bound for `role="progressbar"`. |
| `max` | `number` | `100` | Upper progress bound for `role="progressbar"`. |
| `atomic` | `boolean` | `true` | Whether status announcements should be atomic. |
| `ariaHidden` | `boolean` | `undefined` | Hide the spinner from assistive technology and suppress status/progress semantics. |
| `ariaLabel` | `string` | `undefined` | Accessible name when no visible label or `aria-labelledby` supplies one. |
| `ariaLabelledby` | `string` | `undefined` | Space-separated ids that label the spinner. |
| `ariaDescribedby` | `string` | `undefined` | Space-separated ids that describe the spinner. |
| `ariaValueText` | `string` | `undefined` | Human-readable progress value text for assistive technology. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `SpinnerSlotState` | Optional spinner contents. Receives current state for composition. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `loading` | `boolean` | Whether the spinner represents pending work. |
| `visible` | `boolean` | Whether the spinner remains visible in layout. |
| `state` | `SpinnerState` | Stable state token for styling and tests. |
| `ariaState` | `SpinnerAriaState` | Resolved accessibility semantics for the rendered host. |
| `progressState` | `SpinnerProgressState` | Progress value policy used by the rendered host. |
| `value` | `number \| null` | Current normalized value, or `null` when indeterminate. |
| `min` | `number` | Normalized lower progress bound. |
| `max` | `number` | Normalized upper progress bound. |
| `percent` | `number \| null` | Current completion percentage from 0 to 100, or `null` when indeterminate. |
| `complete` | `boolean` | Whether determinate progress has reached the normalized maximum. |
| `element` | `SpinnerElement \| null` | Rendered host element or component instance. |
