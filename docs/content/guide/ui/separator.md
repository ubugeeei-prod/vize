---
title: "Separator"
description: "Headless Separator; covers HTML hr, WAI-ARIA Separator, Radix UI Separator."
---
...
Headless Separator; covers HTML hr, WAI-ARIA Separator, Radix UI Separator.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/separator` |
| Maturity | stable |
| Own the source | `vize lib pull separator` |
| Requires | — |
| Aliases | divider, rule, thematic break, splitter boundary |
| Covers | HTML hr, WAI-ARIA Separator, Radix UI Separator, Reka UI Separator |

## Usage

```ts
import { Separator } from "@vizejs/ui/separator";
```

Or copy the source into your project with `vize lib pull separator` (see [Source Distribution](../lib-pull.md)).

## API

### `Separator`

Source: `src/families/layout/separator/separator.vue`

#### Props

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `as` | `PrimitiveAs` | `"hr"` | Native element, custom element, or component to render. |
| `orientation` | `SeparatorOrientation` | `"horizontal"` | Logical separator axis announced to assistive technology. |
| `decorative` | `boolean` | `false` | Hide the separator from assistive technology when it is purely visual. |
| `ariaLabel` | `string` | `undefined` | Accessible name for a semantic separator when no visible label supplies one. |
| `ariaLabelledby` | `string` | `undefined` | Space-separated ids that label a semantic separator. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `element` | `SeparatorElement \| null` | Rendered host element or component instance. |
| `orientation` | `SeparatorOrientation` | Logical axis announced by ARIA and `data-orientation`. |
| `decorative` | `boolean` | Whether the separator is hidden from the accessibility tree. |
