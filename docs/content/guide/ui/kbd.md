---
title: "Kbd"
description: "Headless keyboard-input primitive for consumer-owned shortcut styling."
---
...
|  |  |
| --- | --- |
| Package | `@vizejs/ui/kbd` |
| Maturity | stable |
| Own the source | `vize lib pull kbd` |
| Requires | — |
| Aliases | keyboard input, shortcut, keycap, keyboard sequence |
| Covers | HTML kbd element, CSS inline layout, Reka UI Primitive |

## Usage

```ts
import { Kbd } from "@vizejs/ui/kbd";
```

Or copy the source into your project with `vize lib pull kbd` (see [Source Distribution](../lib-pull.md)).

## API

### `Kbd`

Source: `src/families/typography/kbd/kbd.vue`

#### Props

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `as` | `PrimitiveAs` | `"kbd"` | Native element, custom element, or component to render. |
| `size` | `KbdSize` | `"md"` | Consumer visual-size token mirrored to `data-size`; no CSS is emitted. |
| `variant` | `KbdVariant` | `"key"` | Keyboard presentation token mirrored to `data-variant`; no CSS is emitted. |
| `tone` | `KbdTone` | `"neutral"` | Consumer tone token mirrored to `data-tone`; no CSS is emitted. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `KbdSlotState` | Renders keyboard input content with current presentation hook state. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `size` | `KbdSize` | Visual size token mirrored to `data-size`. |
| `variant` | `KbdVariant` | Presentation variant mirrored to `data-variant`. |
| `tone` | `KbdTone` | Consumer tone token mirrored to `data-tone`. |
| `element` | `KbdElement \| null` | Rendered host element or component instance. |
