---
title: "Callout"
description: "Headless structured feedback primitive for notes, status messages, and inline alerts."
---
...
Headless structured feedback primitive for notes, status messages, and inline alerts.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/callout` |
| Maturity | stable |
| Own the source | `vize lib pull callout` |
| Requires | [id](./id.md) |
| Aliases | callout, banner, inline message, notice, message |
| Covers | WAI-ARIA note role, WAI-ARIA status role, WAI-ARIA alert role, Radix Themes Callout, shadcn/ui Alert, Vuetify Alert |

## Usage

```ts
import { Callout } from "@vizejs/ui/callout";
```

Or copy the source into your project with `vize lib pull callout` (see [Source Distribution](../lib-pull.md)).

## API

### `Callout`

Source: `src/families/feedback/callout/callout.vue`
...
| `id` | `string` | `undefined` | Consumer-owned root id for anchors or application state. |
| `role` | `CalloutRole` | `"note"` | Accessibility role. `note` is static, `status` is polite, and `alert` is assertive. |
| `open` | `boolean` | `true` | Whether the Callout is visible. |
| `atomic` | `boolean` | `true` | Whether assistive technology should present the whole live region on updates. |
| `tone` | `CalloutTone` | `"neutral"` | Styling tone mirrored to `data-tone`; no CSS is emitted. |
| `density` | `CalloutDensity` | `"comfortable"` | Spacing density mirrored to `data-density`; no CSS is emitted. |
| `iconAriaHidden` | `boolean` | `true` | Whether the optional icon wrapper is hidden from assistive technology. |
| `ariaHidden` | `boolean` | `undefined` | Hide the entire Callout from assistive technology. |
| `ariaLabel` | `string` | `undefined` | Accessible name when no visible title or `aria-labelledby` supplies one. |
| `ariaLabelledby` | `string` | `generated from the title slot when present` | Space-separated ids that label the Callout. |
| `ariaDescribedby` | `string` | `generated from the description slot when present` | Space-separated ids that describe the Callout. |
| `titleId` | `string` | `generated when the title slot is present` | Consumer-owned id for the title slot wrapper. |
| `descriptionId` | `string` | `generated when the description slot is present` | Consumer-owned id for the description slot wrapper. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `CalloutSlotState` | Renders the main Callout body. |
| `icon` | `CalloutSlotState` | Renders an optional consumer-owned icon. |
| `title` | `CalloutSlotState` | Renders an optional accessible title. |
| `description` | `CalloutSlotState` | Renders an optional accessible description. |
| `actions` | `CalloutSlotState` | Renders optional interactive or navigational actions. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `open` | `boolean` | Whether the Callout is visible. |
| `state` | `CalloutState` | Visibility state mirrored to `data-state`. |
| `role` | `CalloutRole` | Requested accessibility role. |
| `ariaState` | `CalloutAriaState` | Resolved accessibility state after `ariaHidden` is applied. |
| `live` | `CalloutLive \| undefined` | Derived live-region politeness, when `role` is `alert` or `status`. |
| `atomic` | `boolean` | Whether assistive technology should present the whole live region on updates. |
| `tone` | `CalloutTone` | Consumer styling tone mirrored to `data-tone`. |
| `density` | `CalloutDensity` | Consumer spacing density mirrored to `data-density`. |
| `titleId` | `string \| undefined` | Resolved id for the title wrapper. |
| `descriptionId` | `string \| undefined` | Resolved id for the description wrapper. |
| `ariaLabelledby` | `string \| undefined` | Resolved `aria-labelledby` value for the root. |
| `ariaDescribedby` | `string \| undefined` | Resolved `aria-describedby` value for the root. |
| `hasIcon` | `boolean` | Whether the icon slot is rendered. |
| `hasTitle` | `boolean` | Whether the title slot is rendered. |
| `hasDescription` | `boolean` | Whether the description slot is rendered. |
| `hasActions` | `boolean` | Whether the actions slot is rendered. |
| `element` | `CalloutElement \| null` | Rendered host element or component instance. |
