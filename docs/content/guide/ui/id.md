...
description: "Headless Deterministic ID Provider; covers React Aria SSRProvider, Vue useId."
---
...
Headless Deterministic ID Provider; covers React Aria SSRProvider, Vue useId.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/id` |
| Maturity | stable |
| Own the source | `vize lib pull id` |
| Requires | [context](./context.md) |
| Aliases | id provider, deterministic ids, hydration ids |
| Covers | React Aria SSRProvider, Vue useId |

## Usage

```ts
import { IdProvider } from "@vizejs/ui/id";
```

Or copy the source into your project with `vize lib pull id` (see [Source Distribution](../lib-pull.md)).

## API

### `IdProvider`

Source: `src/families/foundations/id/deterministic-id-provider.vue`

#### Props

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `prefix` | `string` | `The parent prefix when nested; otherwise "vize"` | Namespace prefix for this subtree. |
| `seed` | `string \| number` | `Vue's SSR- and hydration-stable useId() value` | Stable request, island, or subtree seed. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `{ readonly namespace: string; readonly prefix: string }` | Renders descendants with the resolved deterministic identifier namespace. |
