---
title: "ProgressBar"
description: "Headless ProgressBar; covers WAI-ARIA progressbar role, React Aria ProgressBar, HTML progress."
---
...
Headless ProgressBar; covers WAI-ARIA progressbar role, React Aria ProgressBar, HTML progress.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/progress-bar` |
| Maturity | stable |
| Own the source | `vize lib pull progress-bar` |
| Requires | [id](./id.md) |
| Aliases | progress, loading progress, task progress, upload progress |
| Covers | WAI-ARIA progressbar role, React Aria ProgressBar, HTML progress |

## Usage

```ts
import { ProgressBar } from "@vizejs/ui/progress-bar";
```

Or copy the source into your project with `vize lib pull progress-bar` (see [Source Distribution](../lib-pull.md)).

## API

### `ProgressBar`

Source: `src/families/feedback/progress-bar/progress-bar.vue`

#### Props

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `as` | `PrimitiveAs` | `"div"` | Native element, custom element, or component rendered as the root. |
| `id` | `string \| null` | `undefined` | Consumer-owned progressbar id. |
| `value` | `number \| null` | `null` | Current determinate value. `null` and `undefined` render indeterminate. |
| `min` | `number \| null` | `0` | Lower bound for the progress range. |
| `max` | `number \| null` | `100` | Upper bound for the progress range. |
| `dir` | `ProgressBarDirection` | `"ltr"` | Reading direction for inline-start fill and indeterminate motion. |
| `label` | `string` | `undefined` | Optional visible label rendered in the label part. |
| `valueLabel` | `string` | `undefined` | Optional visible value text reused as `aria-valuetext`. |
...
| `ariaValueText` | `string` | `undefined` | Human-readable assistive value text, overriding `valueLabel`. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `ProgressBarSlotState` | Render consumer-owned content inside the root with normalized progress state. |
| `label` | `ProgressBarSlotState` | Render a visible label inside the deterministic label node. |
| `value` | `ProgressBarSlotState` | Render visible value text inside the deterministic value node. |
| `indicator` | `ProgressBarSlotState` | Render optional content inside the indicator part. |
...
| `ratio` | `number \| null` | Unitless completion ratio from 0 to 1, or `null` when indeterminate. |
| `dir` | `ProgressBarDirection` | Reading direction reflected to `dir` and `data-dir`. |
...
| `invalid` | `boolean` | Whether raw inputs had to be repaired before reaching ARIA or CSS. |
| `state` | `ProgressBarState` | Stable state token for styling and tests. |
| `id` | `string` | Deterministic id used by the root progressbar. |
| `labelId` | `string` | Deterministic id used by the internal label node when rendered. |
| `valueId` | `string` | Deterministic id used by the internal value node when rendered. |
| `style` | `ProgressBarStyle` | Native CSS custom property hooks applied to the root. |
| `root` | `PrimitiveElement \| null` | Rendered root element or component instance. |
| `track` | `HTMLSpanElement \| null` | Rendered track element. |
| `indicator` | `HTMLSpanElement \| null` | Rendered indicator element. |
| `focus` | `(options?: FocusOptions) => void` | Moves DOM focus to the root when the rendered host supports it. |
