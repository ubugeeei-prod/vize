---
title: "Avatar"
description: "Headless avatar primitive for consumer-owned identity, fallback, and presence styling."
---
...
Headless avatar primitive for consumer-owned identity, fallback, and presence styling.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/avatar` |
| Maturity | stable |
| Own the source | `vize lib pull avatar` |
| Requires | — |
| Aliases | profile image, user avatar, presence avatar, fallback avatar |
| Covers | HTML img element, native image loading, native image decoding |

## Usage

```ts
import { Avatar } from "@vizejs/ui/avatar";
```

Or copy the source into your project with `vize lib pull avatar` (see [Source Distribution](../lib-pull.md)).

## API

### `Avatar`

Source: `src/families/layout/avatar/avatar.vue`

#### Props

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `as` | `PrimitiveAs` | `"span"` | Native element, custom element, or component to render as the root. |
| `src` | `string` | `undefined` | Native image source. Missing or failed sources render the fallback part. |
| `alt` | `string` | `""` | Native image alternative text. Avatar does not infer this from `name`. |
| `name` | `string` | `undefined` | Consumer-owned display name exposed to slots and data presence hooks. |
| `fallback` | `string` | `undefined` | Consumer-owned fallback text rendered when no fallback slot is provided. |
| `status` | `AvatarStatus` | `"none"` | Consumer presence token mirrored to `data-status`; no CSS is emitted. |
| `loading` | `AvatarImageLoading` | `undefined` | Native image loading policy. |
| `decoding` | `AvatarImageDecoding` | `undefined` | Native image decoding policy. |
| `fetchPriority` | `AvatarImageFetchPriority` | `undefined` | Native image fetch-priority hint. |
| `crossOrigin` | `AvatarImageCrossOrigin` | `undefined` | Native image CORS policy. |
| `referrerPolicy` | `AvatarImageReferrerPolicy` | `undefined` | Native image referrer policy. |

#### Events

| Event | Payload | Description |
| --- | --- | --- |
| `load` | `[nativeEvent: Event]` | Fired after the image part dispatches a native load event. |
| `error` | `[nativeEvent: Event]` | Fired after the image part dispatches a native error event and Avatar renders fallback. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `AvatarSlotState` | Renders fallback content with the current avatar hooks. |
| `fallback` | `AvatarSlotState` | Renders named fallback content with the current avatar hooks. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `state` | `AvatarState` | Current render branch. |
| `status` | `AvatarStatus` | Consumer presence token mirrored to `data-status`. |
| `src` | `string \| undefined` | Image source passed to the native image part. |
| `alt` | `string` | Native image alternative text. |
| `name` | `string \| undefined` | Consumer-owned display name, never transformed by Avatar. |
| `fallback` | `string \| undefined` | Consumer-owned fallback text, never generated by Avatar. |
| `image` | `AvatarPresence` | Whether the image source hook is present. |
| `nameState` | `AvatarPresence` | Whether the name hook is present. |
| `fallbackState` | `AvatarPresence` | Whether fallback content is present through prop or slot. |
| `element` | `AvatarElement \| null` | Rendered root element or component instance. |
| `imageElement` | `AvatarImageElement \| null` | Rendered native image part. |
| `fallbackElement` | `AvatarFallbackElement \| null` | Rendered native fallback part. |
