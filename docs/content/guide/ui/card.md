---
title: "Card"
description: "Headless surface primitive for consumer-owned card, panel, and section composition."
---
...
Headless surface primitive for consumer-owned card, panel, and section composition.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/card` |
| Maturity | stable |
| Own the source | `vize lib pull card` |
| Requires | — |
| Aliases | surface card, panel, content surface, section shell |
| Covers | HTML section, HTML article, shadcn/ui Card, Reka UI Primitive |

## Usage

```ts
import { Card } from "@vizejs/ui/card";
```

Or copy the source into your project with `vize lib pull card` (see [Source Distribution](../lib-pull.md)).

## API

### `Card`

Source: `src/families/layout/card/card.vue`
...
| `variant` | `CardVariant` | `"card"` | Surface usage variant mirrored to `data-variant` for consumer-owned styling. |
| `density` | `CardDensity` | `"comfortable"` | Density token mirrored to `data-density`; no spacing CSS is emitted. |
| `tone` | `CardTone` | `"neutral"` | Styling tone mirrored to `data-tone`; no color CSS is emitted. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `CardSlotState` | Renders card content with the current variant, density, and tone. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `variant` | `CardVariant` | Card usage variant mirrored to `data-variant`. |
| `density` | `CardDensity` | Card density mirrored to `data-density`. |
| `tone` | `CardTone` | Consumer styling tone mirrored to `data-tone`. |
| `element` | `CardElement \| null` | Rendered host element or component instance. |
