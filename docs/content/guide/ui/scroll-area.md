---
title: "Scroll Area"
description: "Headless Scroll Area; covers CSS overflow, CSS overscroll-behavior, CSS scrollbar-gutter."
---
...
Headless Scroll Area; covers CSS overflow, CSS overscroll-behavior, CSS scrollbar-gutter.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/scroll-area` |
| Maturity | stable |
| Own the source | `vize lib pull scroll-area` |
| Requires | — |
| Aliases | scroll area, scroll viewport, native scroll container, overflow region |
| Covers | CSS overflow, CSS overscroll-behavior, CSS scrollbar-gutter, CSS scrollbar-width, WAI-ARIA named region, Radix UI ScrollArea, Reka UI ScrollArea |

## Usage

```ts
import { ScrollArea } from "@vizejs/ui/scroll-area";
```

Or copy the source into your project with `vize lib pull scroll-area` (see [Source Distribution](../lib-pull.md)).

## API

### `ScrollArea`

Source: `src/families/layout/scroll-area/scroll-area.vue`

#### Props

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `as` | `ScrollAreaAs` | `"div"` | Native element, custom element, or component to render as the root. |
| `orientation` | `ScrollAreaOrientation` | `"vertical"` | Logical scroll axis controlled by the native viewport overflow. |
| `dir` | `ScrollAreaDirection` | `"ltr"` | Reading direction reflected with `dir` and `data-dir`. |
| `focusable` | `boolean` | `false` | Make the viewport keyboard-focusable for standalone scrollable regions. |
| `blockSize` | `ScrollAreaLength` | `"auto"` | Root block size. Numbers resolve to px lengths. |
| `inlineSize` | `ScrollAreaLength` | `"auto"` | Root inline size. Numbers resolve to px lengths. |
| `maxBlockSize` | `ScrollAreaLength` | `"none"` | Root max block size. Numbers resolve to px lengths. |
| `maxInlineSize` | `ScrollAreaLength` | `"none"` | Root max inline size. Numbers resolve to px lengths. |
| `overscrollBehavior` | `ScrollAreaOverscrollBehavior` | `"auto"` | Native overscroll policy for the viewport. |
| `scrollBehavior` | `ScrollAreaScrollBehavior` | `"auto"` | Native scroll behavior for programmatic scrolling. |
| `scrollbarGutter` | `ScrollAreaScrollbarGutter` | `"auto"` | Native scrollbar gutter policy. |
| `scrollbarWidth` | `ScrollAreaScrollbarWidth` | `"auto"` | Native scrollbar width hook. |
| `ariaLabel` | `string` | `undefined` | Accessible name for the scroll viewport; also promotes it to `role="region"`. |
| `ariaLabelledby` | `string` | `undefined` | Space-separated ids that label the scroll viewport. |
| `ariaDescribedby` | `string` | `undefined` | Space-separated ids that describe the scroll viewport. |

#### Events

| Event | Payload | Description |
| --- | --- | --- |
| `scroll` | `[nativeEvent: Event]` | Fired when the native viewport dispatches a scroll event. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `ScrollAreaSlotState` | Renders scrollable content with current viewport, direction, and CSS-hook state. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `ariaLabel` | `string \| undefined` | Normalized `aria-label` value, or `undefined` when absent. |
| `ariaLabelledby` | `string \| undefined` | Normalized `aria-labelledby` value, or `undefined` when absent. |
| `ariaDescribedby` | `string \| undefined` | Normalized `aria-describedby` value, or `undefined` when absent. |
| `as` | `ScrollAreaAs` | Rendered root host. |
| `orientation` | `ScrollAreaOrientation` | Logical scroll axis. |
| `dir` | `ScrollAreaDirection` | Reading direction reflected on the viewport. |
| `focusable` | `boolean` | Whether the viewport is keyboard-focusable. |
| `blockSize` | `ScrollAreaResolvedLength` | Resolved root block size. |
| `inlineSize` | `ScrollAreaResolvedLength` | Resolved root inline size. |
| `maxBlockSize` | `ScrollAreaResolvedLength` | Resolved root max block size. |
| `maxInlineSize` | `ScrollAreaResolvedLength` | Resolved root max inline size. |
| `overflowX` | `ScrollAreaOverflow` | Native viewport horizontal overflow value. |
| `overflowY` | `ScrollAreaOverflow` | Native viewport vertical overflow value. |
| `overscrollBehavior` | `ScrollAreaOverscrollBehavior` | Native viewport overscroll policy. |
| `scrollBehavior` | `ScrollAreaScrollBehavior` | Native viewport scroll behavior. |
| `scrollbarGutter` | `ScrollAreaScrollbarGutter` | Native viewport scrollbar gutter policy. |
| `scrollbarWidth` | `ScrollAreaScrollbarWidth` | Native viewport scrollbar width hook. |
| `labelled` | `boolean` | Whether the viewport has an accessible name. |
| `described` | `boolean` | Whether the viewport has an accessible description reference. |
| `state` | `ScrollAreaState` | Stable state token for styling and tests. |
| `style` | `ScrollAreaStyle` | Native CSS custom property hooks applied to the root. |
| `root` | `ScrollAreaRootElement \| null` | Rendered root element or component instance. |
| `viewport` | `HTMLDivElement \| null` | Rendered native scroll viewport. |
| `focus` | `(options?: FocusOptions) => void` | Move DOM focus to the native viewport. |
| `scrollTo` | `(options?: ScrollToOptions) => void` | Scroll the native viewport using the platform `scrollTo` API. |
| `scrollBy` | `(options?: ScrollToOptions) => void` | Scroll the native viewport using the platform `scrollBy` API. |
