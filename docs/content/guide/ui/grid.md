---
title: "Grid"
description: "Headless Grid; covers CSS grid, CSS gap, Reka UI Primitive."
---
...
Headless Grid; covers CSS grid, CSS gap, Reka UI Primitive.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/grid` |
| Maturity | stable |
| Own the source | `vize lib pull grid` |
| Requires | — |
| Aliases | layout grid, responsive grid, two-dimensional layout, card grid |
| Covers | CSS grid, CSS gap, Reka UI Primitive |

## Usage

```ts
import { Grid } from "@vizejs/ui/grid";
```

Or copy the source into your project with `vize lib pull grid` (see [Source Distribution](../lib-pull.md)).

## API

### `Grid`

Source: `src/families/layout/grid/grid.vue`
...
| `columns` | `GridColumns` | `1` | Native CSS `grid-template-columns` value. Numbers resolve to equal fr tracks. |
| `gap` | `GridGap` | `0` | Native CSS `gap` value between direct children. Numbers resolve to px lengths. |
| `rowGap` | `GridGap` | `undefined` | Native CSS `row-gap` override. Numbers resolve to px lengths. |
| `columnGap` | `GridGap` | `undefined` | Native CSS `column-gap` override. Numbers resolve to px lengths. |
| `align` | `GridAlign` | `"stretch"` | Native CSS `align-items` value for grid items. |
| `justify` | `GridJustify` | `"stretch"` | Native CSS `justify-items` value for grid items. |
| `autoFlow` | `GridAutoFlow` | `"row"` | Native CSS `grid-auto-flow` auto-placement mode. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `GridSlotState` | Renders grid children with the resolved native grid state. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `columns` | `GridResolvedColumns` | Resolved CSS grid template columns value. |
| `gap` | `GridResolvedGap` | Resolved CSS gap value between direct children. |
| `rowGap` | `GridResolvedGap` | Resolved CSS row gap value between direct children. |
| `columnGap` | `GridResolvedGap` | Resolved CSS column gap value between direct children. |
| `align` | `GridAlign` | Native CSS `align-items` value for grid items. |
| `justify` | `GridJustify` | Native CSS `justify-items` value for grid items. |
| `autoFlow` | `GridAutoFlow` | Native CSS `grid-auto-flow` value for auto placement. |
| `style` | `GridStyle` | Native CSS grid style object applied to the host. |
| `element` | `GridElement \| null` | Rendered host element or component instance. |
