...
description: "Headless Command Router; covers VS Code command registry, Ariakit Combobox commands, kbar actions."
---
...
Headless Command Router; covers VS Code command registry, Ariakit Combobox commands, kbar actions.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/command` |
| Maturity | stable |
| Own the source | `vize lib pull command` |
| Requires | — |
| Aliases | command registry, command routing, action dispatcher |
| Covers | VS Code command registry, Ariakit Combobox commands, kbar actions |

## API

### `createCommandRouter`

Create a typed command router with enablement and help metadata. The router owns no DOM and is safe to create during server rendering. Registering an identifier twice throws a conflict diagnostic so palettes, menus, and shortcut layers cannot silently shadow one another. Call `CommandRouter.dispose` when using this factory outside a Vue effect scope.

```ts
function createCommandRouter<Id extends string = string>( options: CommandRouterOptions<Id> = {}, ): CommandRouter<Id>
```
...
```ts
function useCommandRouter<Id extends string = string>( options: CommandRouterOptions<Id> = {}, ): CommandRouter<Id>
```
