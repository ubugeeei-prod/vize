---
title: "Banner"
description: "Accessible, unstyled page and section banner primitive."
---
...
|  |  |
| --- | --- |
| Package | `@vizejs/ui/banner` |
| Maturity | stable |
| Own the source | `vize lib pull banner` |
| Requires | [id](./id.md) |
| Aliases | announcement banner, dismissible notice, page notice, status banner |
| Covers | WAI-ARIA region role, WAI-ARIA status role, WAI-ARIA alert role, shadcn/ui Alert, Vuetify Alert, React Aria live regions |

## Usage

```ts
import { Banner } from "@vizejs/ui/banner";
```

Or copy the source into your project with `vize lib pull banner` (see [Source Distribution](../lib-pull.md)).

## API

### `Banner`

Source: `src/families/feedback/banner/banner.vue`
...
| `id` | `string` | `undefined` | Optional root id used as the base for deterministic title and description ids. |
| `title` | `string` | `undefined` | Visible title rendered into the deterministic title node. |
| `description` | `string` | `undefined` | Visible description rendered into the deterministic description node. |
| `role` | `BannerRole \| Exclude<BannerRole, "region">` | `"region"` | Persistent banner role. `region` requires a name; live roles may announce changes. |
| `tone` | `BannerTone` | `"neutral"` | Consumer styling tone mirrored to `data-tone`; no CSS is emitted. |
| `open` | `boolean` | `true` | Controlled visibility. `false` hides the banner and suppresses ARIA semantics. |
| `dismissible` | `boolean` | `false` | Render a native dismiss control that requests `open=false`; local state is never mutated. |
| `dismissLabel` | `string` | `"Dismiss banner"` | Accessible label for the rendered dismiss control. |
| `atomic` | `boolean` | `true` | Whether live-role announcements should be atomic. |
| `ariaLabel` | `string` | `undefined` | Accessible name when no visible title or `aria-labelledby` supplies one. |
| `ariaLabelledby` | `string` | `undefined` | Space-separated ids that label the banner. |
| `ariaDescribedby` | `string` | `undefined` | Space-separated ids that describe the banner. |

#### Events

| Event | Payload | Description |
| --- | --- | --- |
| `update:open` | `[open: boolean]` | Request that the controlled `open` prop moves to a new value. |
| `dismiss` | `[nativeEvent: MouseEvent]` | Fired after the dismiss control is activated. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `BannerSlotState` | Render the primary banner body. |
| `title` | `BannerSlotState` | Render a consumer-owned title inside the deterministic title node. |
| `description` | `BannerSlotState` | Render a consumer-owned description inside the deterministic description node. |
| `actions` | `BannerSlotState` | Render trailing actions. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `state` | `BannerState` | Open state mirrored to `data-state`. |
| `role` | `BannerRole` | Persistent banner role selected by props. |
| `tone` | `BannerTone` | Consumer styling tone mirrored to `data-tone`. |
| `live` | `BannerLive` | Resolved live-region politeness. |
| `named` | `boolean` | Whether the rendered host has a valid accessible name. |
| `ariaState` | `BannerAriaState` | Resolved accessibility quality mirrored to `data-aria-state`. |
| `titleId` | `string` | Deterministic id for the rendered title part. |
| `descriptionId` | `string` | Deterministic id for the rendered description part. |
| `ariaLabelledby` | `string \| undefined` | Resolved `aria-labelledby` value, if any. |
| `ariaDescribedby` | `string \| undefined` | Resolved `aria-describedby` value, if any. |
| `dismissible` | `boolean` | Whether the controlled dismiss affordance is rendered. |
| `element` | `BannerElement \| null` | Rendered host element or component instance. |
| `focus` | `(options?: FocusOptions) => void` | Programmatically focus the rendered banner host, when present and focusable. |
| `dismiss` | `(nativeEvent?: MouseEvent) => void` | Request controlled dismissal without changing local state. |
