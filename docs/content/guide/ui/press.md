---
title: "Press"
description: "Headless Press; covers React Aria usePress, Pointer Events activation."
---
...
Headless Press; covers React Aria usePress, Pointer Events activation.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/press` |
| Maturity | stable |
| Own the source | `vize lib pull press` |
| Requires | — |
| Aliases | press, activation, pointer |
| Covers | React Aria usePress, Pointer Events activation |

## API

### `createPress`

Create an SSR-safe press normalizer for one host element. Spread the returned `pressProps` onto the host and call `dispose` when using this factory outside a Vue effect scope. No DOM global is read at setup.

```ts
function createPress(options: PressOptions = {}): PressController
```
...
```ts
function usePress(options: PressOptions = {}): PressController
```

### `PressActivationMemory`

Own short-lived tokens that associate release, cancellation, and click.

```ts
class PressActivationMemory
```
...
```ts
function readBooleanOption( value: PressOptions[keyof Pick< PressOptions, "allowTextSelectionOnPress" | "isDisabled" | "preventFocusOnPress" | "shouldCancelOnPointerExit" >], name: string, ): boolean
```
...
```ts
function readKeyboardBehavior( value: PressOptions["keyboardBehavior"], ): PressKeyboardBehavior
```
...
```ts
function pointerTypeOf(event: PointerEvent): PressPointerType
```
...
```ts
function createPressEvent( type: PressEventType, target: Element, pointerType: PressPointerType, originalEvent: Event | null, isCanceled = false, touchIdentifier: number | null = null, ): PressEvent
```
...
```ts
function keyboardActivation( target: Element, key: string, behavior: PressKeyboardBehavior, ): "custom" | "native" | null
```
...
```ts
function isEventInside( event: Event, target: Element, touchIdentifier: number | null = null, ): boolean
```
...
```ts
function validatePressOptions(options: PressOptions): void
```
...
```ts
function createPressHandlers(lifecycle: PressLifecycle): PressHandlers
```
