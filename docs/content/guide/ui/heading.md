---
title: "Heading"
description: "Headless semantic heading primitive for consumer-owned typography systems."
---
...
|  |  |
| --- | --- |
| Package | `@vizejs/ui/heading` |
| Maturity | stable |
| Own the source | `vize lib pull heading` |
| Requires | — |
| Aliases | semantic heading, typography heading, title, section heading |
| Covers | HTML heading content, ARIA document structure, Reka UI Primitive |

## Usage

```ts
import { Heading } from "@vizejs/ui/heading";
```

Or copy the source into your project with `vize lib pull heading` (see [Source Distribution](../lib-pull.md)).

## API

### `Heading`

Source: `src/families/typography/heading/heading.vue`

#### Props

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `as` | `PrimitiveAs` | `undefined` | Native element, custom element, or component to render. When omitted, Heading renders the native `h${level}` element. |
| `level` | `HeadingLevel` | `2` | Semantic heading level used for the default native host and `data-level`. |
| `size` | `HeadingSize` | `"md"` | Consumer visual-size token mirrored to `data-size`; no CSS is emitted. |
| `weight` | `HeadingWeight` | `"semibold"` | Consumer font-weight token mirrored to `data-weight`; no CSS is emitted. |
| `tone` | `HeadingTone` | `"neutral"` | Consumer tone token mirrored to `data-tone`; no CSS is emitted. |
| `truncate` | `boolean` | `false` | Consumer truncation hook mirrored to `data-truncate`; no CSS is emitted. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `HeadingSlotState` | Renders heading content with current semantic and typography hook state. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `level` | `HeadingLevel` | Semantic heading level mirrored to `data-level`. |
| `size` | `HeadingSize` | Consumer visual-size token mirrored to `data-size`. |
| `weight` | `HeadingWeight` | Consumer font-weight token mirrored to `data-weight`. |
| `tone` | `HeadingTone` | Consumer tone token mirrored to `data-tone`. |
| `truncate` | `boolean` | Whether the consumer requested a truncation styling hook. |
| `element` | `HeadingElement \| null` | Rendered host element or component instance. |
