---
title: "Move"
description: "Headless Move; covers React Aria useMove, Pointer Events capture."
---
...
Headless Move; covers React Aria useMove, Pointer Events capture.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/move` |
| Maturity | stable |
| Own the source | `vize lib pull move` |
| Requires | — |
| Aliases | move, drag, keyboard |
| Covers | React Aria useMove, Pointer Events capture |
...
```ts
function createMove(options: MoveOptions = {}): MoveController
```
...
```ts
function useMove(options: MoveOptions = {}): MoveController
```

### `readBoolean`

```ts
function readBoolean(value: MoveOptions["isDisabled"]): boolean
```

### `readKeyboardStep`

```ts
function readKeyboardStep(value: MoveOptions["keyboardStep"]): number
```

### `positionOf`

```ts
function positionOf(event: Event, touchId: number | null = null): Position | null
```

### `pointerTypeOf`

```ts
function pointerTypeOf(event: PointerEvent): Exclude<MovePointerType, "keyboard">
```

### `createMoveEvent`

```ts
function createMoveEvent( type: MoveEventType, target: Element, pointerType: MovePointerType, originalEvent: Event | null, deltaX = 0, deltaY = 0, isCanceled = false, touchId: number | null = null, ): MoveEvent
```
...
```ts
function validateOptions(options: MoveOptions): void
```
...
```ts
function installMoveListeners( document: Document, source: PointerSource, context: MoveListenerContext, ): () => void
```
