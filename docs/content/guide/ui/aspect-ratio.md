---
title: "Aspect Ratio"
description: "Headless Aspect Ratio; covers CSS aspect-ratio, Radix UI AspectRatio, Reka UI AspectRatio."
---
...
Headless Aspect Ratio; covers CSS aspect-ratio, Radix UI AspectRatio, Reka UI AspectRatio.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/aspect-ratio` |
| Maturity | stable |
| Own the source | `vize lib pull aspect-ratio` |
| Requires | — |
| Aliases | aspect ratio, intrinsic ratio, responsive media box |
| Covers | CSS aspect-ratio, Radix UI AspectRatio, Reka UI AspectRatio |

## Usage

```ts
import { AspectRatio } from "@vizejs/ui/aspect-ratio";
```

Or copy the source into your project with `vize lib pull aspect-ratio` (see [Source Distribution](../lib-pull.md)).

## API

### `AspectRatio`

Source: `src/families/layout/aspect-ratio/aspect-ratio.vue`
...
| `ratio` | `number` | `1` | Positive finite width divided by height. Invalid values fall back to `1`. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `AspectRatioSlotState` | Renders the box contents with the normalized ratio state. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `element` | `AspectRatioElement \| null` | Rendered host element or component instance. |
| `ratio` | `number` | Ratio used for the rendered box after validation. |
| `invalid` | `boolean` | Whether the provided ratio fell back to the default square ratio. |
