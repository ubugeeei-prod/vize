...
description: "Headless Controlled and Uncontrolled State; covers Radix useControllableState, React Aria controlled state."
---
...
Headless Controlled and Uncontrolled State; covers Radix useControllableState, React Aria controlled state.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/controllable-state` |
| Maturity | stable |
| Own the source | `vize lib pull controllable-state` |
| Requires | — |
| Aliases | controllable state, controlled state, uncontrolled state |
| Covers | Radix useControllableState, React Aria controlled state |

## API

### `useControllableState`

Create one state contract for controlled props and internal state. The last controlled value is retained if the source becomes uncontrolled, preventing an abrupt jump back to the initial default.

```ts
function useControllableState<Value>( options: ControllableStateOptions<Value>, ): ControllableState<Value>
```
