---
title: "Text"
description: "Headless inline typography primitive for consumer-owned text styling."
---
...
|  |  |
| --- | --- |
| Package | `@vizejs/ui/text` |
| Maturity | stable |
| Own the source | `vize lib pull text` |
| Requires | — |
| Aliases | body text, typography text, copy, inline text, truncated text |
| Covers | HTML phrasing content, CSS text-overflow, Reka UI Primitive |

## Usage

```ts
import { Text } from "@vizejs/ui/text";
```

Or copy the source into your project with `vize lib pull text` (see [Source Distribution](../lib-pull.md)).

## API

### `Text`

Source: `src/families/typography/text/text.vue`
...
| `size` | `TextSize` | `"md"` | Consumer text-size token mirrored to `data-size`; no CSS is emitted. |
| `weight` | `TextWeight` | `"regular"` | Consumer font-weight token mirrored to `data-weight`; no CSS is emitted. |
| `tone` | `TextTone` | `"neutral"` | Consumer tone token mirrored to `data-tone`; no CSS is emitted. |
...
| --- | --- | --- |
| `default` | `TextSlotState` | Renders text content with current typography hook state. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `size` | `TextSize` | Consumer size token mirrored to `data-size`. |
| `weight` | `TextWeight` | Consumer font-weight token mirrored to `data-weight`. |
| `tone` | `TextTone` | Consumer tone token mirrored to `data-tone`. |
| `truncate` | `boolean` | Whether the consumer requested a truncation styling hook. |
| `element` | `TextElement \| null` | Rendered host element or component instance. |
