---
title: "Skeleton"
description: "Headless Skeleton; covers WAI-ARIA status role, shadcn/ui Skeleton, React Aria ProgressBar."
---
...
Headless Skeleton; covers WAI-ARIA status role, shadcn/ui Skeleton, React Aria ProgressBar.

|  |  |
| --- | --- |
| Package | `@vizejs/ui/skeleton` |
| Maturity | stable |
| Own the source | `vize lib pull skeleton` |
| Requires | — |
| Aliases | placeholder, loading skeleton, deferred content, suspense fallback |
| Covers | WAI-ARIA status role, shadcn/ui Skeleton, React Aria ProgressBar |

## Usage

```ts
import { Skeleton } from "@vizejs/ui/skeleton";
```

Or copy the source into your project with `vize lib pull skeleton` (see [Source Distribution](../lib-pull.md)).

## API

### `Skeleton`

Source: `src/families/feedback/skeleton/skeleton.vue`
...
| `loading` | `boolean` | `true` | Whether the placeholder represents pending content. |
| `visible` | `boolean` | `true` | Whether the placeholder remains rendered and visible in layout. |
| `ariaLabel` | `string` | `undefined` | Accessible status text when the skeleton should be announced. |
| `ariaHidden` | `boolean` | `undefined` | Override the default accessibility policy. By default, labelled skeletons are exposed as status regions and unlabelled skeletons are decorative. |
| `blockSize` | `string` | `"1em"` | Value published to `--vize-ui-skeleton-block-size`. |
| `inlineSize` | `string` | `"100%"` | Value published to `--vize-ui-skeleton-inline-size`. |

#### Slots

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `SkeletonSlotState` | Optional placeholder content. Receives current loading, visibility, and ARIA state. |

#### Exposed

| Member | Type | Description |
| --- | --- | --- |
| `loading` | `boolean` | Whether the placeholder represents work still loading. |
| `visible` | `boolean` | Whether the placeholder remains visible in layout. |
| `state` | `SkeletonState` | Stable state token for styling and tests. |
| `ariaState` | `SkeletonAriaState` | Whether accessibility semantics are decorative or status-like. |
| `element` | `SkeletonElement \| null` | Rendered host element or component instance. |
