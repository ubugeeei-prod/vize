...
| `whitespace`           | `compiler.whitespace`, `vize({ whitespace })`, or `template.compilerOptions.whitespace` | Use Vue's `"condense"` (default) or `"preserve"` template whitespace mode. `preserve` keeps spacing inside mixed text; Vue still normalizes whitespace-only nodes between siblings to one space. |
