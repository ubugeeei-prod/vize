...
| Desempenho | Analisador, compilador, linter, formatador, verificador de tipos, cache, percorrimento de grafos de projeto, saída gerada ou I/O CLI.                            | `.github/workflows/benchmark.yml`, `tools/benchmarks/scripts/compare-pr.mjs`, `tools/benchmarks/scripts/enforce-pr-budget.mjs`, o status `pr-benchmark-budget`, tarefas locais de `bench:*` e `vize lint --profile`, `vize check --profile``vize fmt --profile` ou quando a regressão precisa de atribuição.                           |
...
- `tools/benchmarks/scripts/test-inventory.mjs` reporta o inventário atual de ativos de teste no PR CI.
