...
| Fuzzing    | Análise sintáctica orientada a bytes, recuperação de sintaxe, análise sintática CSS, análise de expressões JS/TS, lexing de templates ou recuperação de codegen. | `.github/workflows/fuzz.yml`, `tests/fuzz/Cargo.toml`, `tools/fuzz/seed_corpus.mjs`, `cargo +nightly fuzz run <target>`, artefatos `fuzz-reproducers-*` carregados e uma regressão determinística minimizada após o crash, timeout ou OOM foi compreendida.                                      |
