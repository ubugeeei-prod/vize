...
> [!NOTE]
> **Early re-cut 2026-09-21, while phase 3 is still live.** The plan README's rule is that a phase is re-cut at its predecessor's exit and that a provisional task cannot be picked up until it carries the full contract. Phase 4 is re-cut **before** the phase-3 exit because almost all of it consumes only L2 (phase 2, exited 2026-09-12), and the maintainer's completion target (**2026-09-23**, ahead of the Vue Fes Japan 2026 presentation on **2026-10-24**) leaves no room to idle the consumer half of the program behind L3 work it does not read. The [plan README](./README.md#phase-files) records the early re-cut rule this phase follows: every task states its **start gate** — either _startable now_ (P3-independent) or _gated on_ a named phase-3 task — and a gated task is not picked up until that task lands. The exit gate below is unchanged and still requires the phase-3 exit (P4-17 depends on P3-16). The target date orders work; it does not relax a gate, a ratchet or a waiver ledger (roadmap: phases are ordered, not scheduled).
...
6. **Diagnostics have no unified channel yet.** `vize_davinci::Diagnostic` (P2-1) carries a `Witness` slot with `LegacyExempt`; nothing outside the L1→L2 lowering produces it. FP-1 (script-relative offsets rendered against whole-file lines, [ledger](./ledger-fp.md)) is deferred to exactly this work. P4-6 splits into types (P4-6a), witness verifier (P4-6b) and Patina adoption with FP-1 fixed (P4-6c).
7. **Davinci stage crates are publishable now.** The P2-era firewall (published crates could not depend on `vize_davinci`/`vize_l2`) is lifted: [`davinci-stage-release-firewall.test.ts`](../../../tests/tooling/davinci-stage-release-firewall.test.ts) asserts all six stage crates are publishable. Patina, Canon, Glyph and Musea may therefore take ordinary runtime dependencies on `vize_davinci`, `vize_l1` and `vize_l2`, subject to the [stage dependency policy](./stage-dependencies.md).
8. **Pug has no parser anywhere.** No crate parses pug; Maestro has a 339-line line-heuristic cursor helper (`vize_maestro/src/ide/pug.rs`) and corpus coverage scans pug line-heuristically. Charter #12's first-class L1 pug dialect is therefore its own task (P4-12c), independent of the Glyph rewrite (P4-12b).
9. **L1 is template-only.** `vize_l1` (1,381 lines) is the Vue-template surface tree; SFC block splitting lives in `vize_croquis/src/sfc/` (3,687 lines) and there is no OXC-backed lossless script wrapper. P4-12b and P4-13 consume the existing splitter plus L1 template trees and name the script wrapper as a non-goal.

**Follow-up audit 2026-09-26 (`339f29e3`):** P4-12c has since landed its Pug parser and lowering slices. The script/JSX lossless L1 wrapper remains absent; [P4-12d](./phase-4-tasks-last.md#p4-12d--lossless-script-and-jsx-l1) now owns this open migration. Functional JSX L2 support already exists through OXC → legacy `RootNode` → L2 projection; that does not establish script/JSX L1 fidelity.

## Carried from phase 3 (so far)

| Phase-3 state (2026-09-21)                                                                                        | Phase-4 tasks affected | How                                                                                                                                             |
| ----------------------------------------------------------------------------------------------------------------- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **P3-2 landed** — L3 reactivity lattice fact group in `vize_impeto`, `proven/refuted/unknown` orthogonal to value | P4-3d, P4-6a           | The Croquis `reactivity` wave merges into this group instead of creating a second lattice; P4-6a reuses its verdict axis for witnesses.         |
| **P3-6 open** — Vapor on L3 is the intended `EffectGraph` consumer                                                | P4-3e, P4-4b           | Both gated on P3-6. Doctor and croquis_cf complexity stay the only consumers until then.                                                        |
| **P3-9 open** — only module-start source-map anchors exist; no span-carrying L4 document                          | P4-5b                  | Gated on P3-9's structured emission slice. P4-5a (mapping model) proceeds now and defines the projection-side row type P3-9's document carries. |
...
| G    | P4-9a, P4-9b              | `crates/vize_l1_to_l2/src/pass/cfg*`, `crates/vize_croquis_cf/src/rules/complexity*`, `crates/vize_curator/src/complexity*`, `crates/vize_patina/src/rules/facts/max_template_complexity*`                                                                                      |
...
| K    | P4-12c                    | `crates/vize_l1/src/pug*`, `crates/vize_l1_to_l2/src/lower/pug*`                                                                                                                                                                                                                |
...
| P    | P4-12d                    | `crates/vize_l1/src/script*`, `crates/vize_l1/tests/script*`, `crates/vize_l1_to_l2/src/lower/script*`, `crates/vize_atelier_jsx/src/`                                                                                                                                          |
...
- [x] [P4-3d](./phase-4-tasks.md#p4-3d--reactivity-merges-into-the-l3-lattice) Reactivity merges into the L3 lattice — lane B · startable now
...
- [x] [P4-5b](./phase-4-tasks.md#p4-5b--l2-projection-as-an-l4-target) L2 projection as an L4 target — lane C · gated on P3-9
...
- [x] [P4-7a](./phase-4-tasks-later.md#p4-7a--l2-backed-markup-facade) L2-backed markup facade — lane E · startable now
...
- [ ] [P4-12b](./phase-4-tasks-last.md#p4-12b--glyph-on-l1) Glyph on L1 — lane J · startable now
- [ ] [P4-12c](./phase-4-tasks-last.md#p4-12c--pug-as-an-l1-dialect) Pug as an L1 dialect — lane K · startable now
- [ ] [P4-12d](./phase-4-tasks-last.md#p4-12d--lossless-script-and-jsx-l1) Lossless script and JSX L1 — lane P · startable now
- [x] [P4-13](./phase-4-tasks-last.md#p4-13--musea-onto-l0-and-l1) Musea onto L0 and L1 — lane L · startable now
...
- [ ] Croquis `virtual_ts` (the third generator) deleted with the other two; pug compiles, lints and formats through L1 (charter #12)
