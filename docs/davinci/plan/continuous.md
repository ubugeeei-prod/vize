...
- [x] C-2 L1/L2 folio pages in the inspector (starts: P2-18)
      ...
      payload stays L1-only on purpose (it rides in share URLs; the playground
...
- [x] C-3 Pass timeline + fusion-group view from timing JSON (starts: P2-13)
      ...
      consecutive L2 pages through the inspector's line diff, and shows each
      ...
      with the walk's time. Today's L2 plans fuse nothing (barriers and one
...
- [x] C-5 L3 pages, provenance navigation, remarks rendering (starts: P3-13)
      ...
      focused L2 op shows why it exists - its lowering rule plus the facts
      ...
      arguments and source site, a focused L2 op shows the remarks about
...
- [ ] C-20 Fuzz-target expansion per new stage (L1 parser, folio parser, L2/L3 verifier inputs)
...
- [ ] C-23 Independent Lean folio checker, expanded stage-by-stage (starts: P2 folios stable)
  - _S3 stage 2026-09-22:_ `Impeto.Checker` re-checks the TS-27 graph contract
    ...
    nesting, which the Rust validator never saw; the fixture is fixed. L1/L2
    stages remain.
- [ ] C-24 Theorem backlog: lattice laws → effect-grouping edge preservation → IVM linearity (starts: P3-4)
  - _2026-09-21:_ all three landed with P3-15; later L3 semantics extend them in the same lane.
