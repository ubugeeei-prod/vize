...
**Deliverable:** every cached artifact declares its ambient inputs — tsconfig content, toolchain version, Corsa version, feature flags, platform — in a key manifest, documented per artifact in `docs/davinci/plan/key-manifests.md`; an undeclared input is a cache-corruption bug by definition.
