# L1 Folio Format (`s1-page@1`)

The L1 page is the lossless surface tree (`vize_l1::SurfaceTree`) as a folio
...
and no escaping rule can lose one. L2 page spans stay file-absolute; the
...
The close variants and hole tokens are `vize_l1`'s hole policy one to one:
