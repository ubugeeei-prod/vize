<!-- GENERATED FILE — do not edit by hand.
     Regenerate: rust-script tools/commands/davinci/croquis-consumers.rs --write
     Verify:     rust-script tools/commands/davinci/croquis-consumers.rs --check -->

# Croquis consumption: `vize_l2`

One shard of the [Croquis consumption matrix](../croquis-consumption.md): every fact the matrix records about `crates/vize_l2/src`. The method and the product set live on that page.

## Resolved product sites

_None._

## Non-product `vize_croquis` imports

_None._

## Naive grep disagreements (resolved/grep)

| product            | resolved | grep |
| ------------------ | -------: | ---: |
| `Scope`            |        0 |    1 |
| `ScopeBinding`     |        0 |    9 |
| `Span`             |        0 |  121 |
| `Croquis.bindings` |        0 |   21 |
