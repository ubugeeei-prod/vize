...
not change the S2 to S3 lowering behavior, and leaves the Lean reference to
P3-4. The backend differential wiring stays with that task.

## Decisions

| subject             | decision                                                                                                             |
| ------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Reference home      | `docs/davinci/plan/impeto-ops.md` is the human op reference.                                                         |
| Normative companion | `tests/formal/impeto/` remains the executable semantics companion for VDOM/Vapor traces.                           |
...
P3-4 has since closed with mounted backend behavior traces compared against
the Lean reference. P3-3 has since closed with TS-17 S3 Folio snapshots and TS-20 S2 to
