...
| Real-project corpus         | `tests/_fixtures/_git` + `tools/commands/fixtures/tool-matrix-report.rs` | 134 pinned projects, ~35k `.vue` files, with compiler/linter/formatter/type-checker oracles already trusted (536 runs, 0 failures). This is the migration parity oracle.                                                               |
