...
def damages (p : Program) : Except String (List (String × Program × List String)) := do
  ...
  pure [
    ("duplicate op id", { p with ops := p.ops ++ [last] }, ["L3V001"]),
    ...
    ("op in a missing region", { p with ops := p.ops.map fun o =>
      if o.id == last.id then { o with region := 999 } else o }, ["L3V003", "L3V007"]),
    ("op names a missing effect", { p with ops := p.ops.map fun o =>
      if o.id == last.id then { o with effect := some 999 } else o }, ["L3V007"]),
    ("dangling edge target", { p with edges := p.edges ++ [dangling] }, ["L3V004"]),
    ("parent cycle", { p with regions := region fun r => { r with parent := some grand.id } },
      ["L3V005", "L3V006"]),
    ("backward scheduled edges", { p with phase := .scheduled, ops := p.ops.reverse }, ["L3V008"])
  ]
...
def check : IO UInt32 := do
  match (<- run) with
  | .ok count =>
      IO.println s!"folio checker: {count} committed L3 graphs are well formed"
      pure 0
