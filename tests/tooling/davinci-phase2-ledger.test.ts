...
test("P2-11 records current installments without presenting stale remainders", () => {
  ...
  for (const [label, source] of Object.entries(currentEvidence)) {
    assert.match(source, /#5205/, `${label} must cite the current installment-35 PR`);
    assert.match(
      source,
      /35 (?:landed\s+)?installments|installment 35|\| 35\s+\|/i,
      `${label} must cite current installment 35 evidence`,
    );
    assert.doesNotMatch(source, /pending installment 35|\| 35\s+\| pending/i);
  }
  ...
  assert.match(text.p2_11, /Current named remainder \(after #5205\)/);
