...
import { DEFAULT_MUSEA_FILE_COUNT, buildMuseaSurface } from "../../tools/benchmarks/scripts/compare-tools-musea.mjs";
...
test("the MoonBit bench dispatcher and the bench package agree on the lane script", () => {
  ...
  const manifest = JSON.parse(fs.readFileSync(path.join(root, "tools/benchmarks/scripts/package.json"), "utf8"));
