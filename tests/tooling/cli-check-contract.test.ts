...
test("vize check exits 0 when explicit inputs match no supported files", () => {
  withWorkspace((dir) => {
    ...
    assert.equal(
      result.stderr,
      'No .vue, .ts, .tsx, .mts, .cts, .js, .mjs, .cjs, .jsx, .d.ts, .d.mts, or .d.cts files found matching inputs: ["does-not-exist.vue"]\n',
    );
  });
});
...
test("vize check exits 0 in an empty project with no inputs", () => {
  withWorkspace((dir) => {
    ...
    assert.equal(
      result.stderr,
      "No .vue, .ts, .tsx, .mts, .cts, .js, .mjs, .cjs, .jsx, .d.ts, .d.mts, or .d.cts files found matching inputs: []\n",
    );
  });
});
