...
test("the guard reports leaked and zombie checkers and nothing else", () => {
  ...
  assert.deepEqual([...GUARDED_COMMANDS], ["node", "vize", "tsgo", "corsa"]);
});
...
test("the constrained PID budget is the measured baseline plus documented headroom", () => {
  assert.deepEqual(resolveBudget(1000, 320, 63812), {
    baselineTasks: 1000,
    clamped: false,
    taskBudget: 320,
    ulimitProcesses: 1320,
  });
  assert.deepEqual(resolveBudget(1000, 320, "unlimited"), {
    baselineTasks: 1000,
    clamped: false,
    taskBudget: 320,
    ulimitProcesses: 1320,
  });
  assert.deepEqual(resolveBudget(4000, 320, 4100), {
    baselineTasks: 4000,
    clamped: true,
    taskBudget: 320,
...
test("the budgeted argv lowers the limit and then execs the checker in place", () => {
