...
for (const [extension, declaration, throughBarrel] of [
  ["d.ts", "export declare function expectString(value: string): void;", false],
  ["ts", "export function expectString(value: string) { return value; }", false],
  ["ts", "export function expectString(value: string) { return value; }", true],
] as const) {
  test(
    `CLI and LSP preserve external ${extension} contracts${throughBarrel ? " through a barrel" : ""}`,
    { timeout: 120_000 },
    async () => {
      ...
      const dependency = path.join(directory, `shared.${extension}`);
      fs.writeFileSync(dependency, declaration);
      if (throughBarrel)
        fs.writeFileSync(
          path.join(directory, "barrel.ts"),
          'export { expectString } from "./shared";',
        );
      const source = `<script setup lang="ts">\nimport { expectString } from "../${throughBarrel ? "barrel" : "shared"}";\nexpectString(1);\n</script>`;
