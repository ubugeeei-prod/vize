...
const typecheckerTestRows = [
  [
    "crates/vize_canon/src/batch/type_checker/tests/generic_props/inline_callback_union_props.rs",
    "test",
    1,
  ],
];
...
void test("Canon typechecker tests import S0 through the preferred name", () => {
  const rows = typechecker().fileRows;

  for (const [relPath, mode, sites] of typecheckerTestRows) {
  ...
  const migratedPaths = new Set(typecheckerTestRows.map(([relPath]) => relPath));
