...
test("recorded output matches the installed @nuxt/eslint packages", async () => {
  ...
  assert.equal(recomputed.schemaVersion, 7);
  ...
  assert.deepEqual(recomputed.nuxtConfigKeysOrderCases, recorded.nuxtConfigKeysOrderCases);
...
test("inventory markdown documents exactly the corpus cases", () => {
  ...
  const ids = [
    ...
    ...corpus.nuxtConfigKeysOrderCases.map((entry: { id: string }) => entry.id),
