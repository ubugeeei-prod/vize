...
test("test:scripts requires its typecheck dependencies", () => {
  ...
  assert.match(
    command,
    /&& VIZE_TEST_REQUIRE_TSGO=1 node --test --test-concurrency=1 tests\/tooling\/\*\.test\.ts tests\/tooling\/\*\.test\.mjs tests\/tooling\/davinci\/\*\.test\.ts tests\/tooling\/davinci\/\*\.test\.mjs tests\/tooling\/release\/\*\.test\.ts$/,
  );
  assert.match(
    workflow,
    /\n  test-scripts:\n[\s\S]*?git submodule update --init --force --depth 1 -- tests\/_fixtures\/_git\/ant-design-vue tests\/_fixtures\/_git\/create-vue tests\/_fixtures\/_git\/vue-select && vp install --frozen-lockfile --prefer-offline[\s\S]*?\n  editor-extensions:\n/,
  );
});
...
test("plain script and reference Rust CLI gates use the fail-closed Corsa helper", () => {
  const files = [
    ...
    ["check_reference_types_cli.rs", 5],
