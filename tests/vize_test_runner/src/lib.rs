...
//! tests/expected/ (generated by tools/moon/cmd/generate_expected using @vue/compiler-sfc).
...
/// Load a test fixture, preferring a typed Pkl file over legacy TOML.
///
/// A sibling `.pkl` (or a `.pkl` path) is evaluated with the project-local `pkl`
/// binary; otherwise the `.toml` is parsed. This lets fixtures migrate to typed
/// Pkl one directory at a time without touching the call sites.
pub fn load_fixture(path: &Path) -> Result<TestFixture, Box<dyn std::error::Error>> {
    let pkl = path.with_extension("pkl");
    if pkl.exists() {
        return load_pkl_fixture(&pkl);
    }
...
/// Evaluate a Pkl fixture into a [`TestFixture`] using a project-local `pkl`
/// binary when available (cargo test does not put `node_modules/.bin` on PATH).
fn load_pkl_fixture(path: &Path) -> Result<TestFixture, Box<dyn std::error::Error>> {
    ...
    let result = manager.evaluate_module_typed::<TestFixture>(&evaluator, ModuleSource::file(path));
...
fn pkl_evaluator_options(path: &Path) -> EvaluatorOptions {
    match path.parent() {
        Some(dir) => EvaluatorOptions::preconfigured().root_dir(dir.to_string_lossy().as_ref()),
        None => EvaluatorOptions::preconfigured(),
    }
}
...
pub fn run_fixture_tests(fixture_path: &Path, expected_path: &Path) -> Vec<TestResult> {
    ...
    // Prefer a typed Pkl fixture when present so callers can keep passing the
    // legacy `.toml` path while fixtures migrate to Pkl one directory at a time.
    let pkl_path = fixture_path.with_extension("pkl");
    let fixture_path = if pkl_path.exists() {
        pkl_path.as_path()
    } else {
        fixture_path
    };
