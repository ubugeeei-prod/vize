...
export default {
  ...
  async setup(__props) {
    ...
    const accounts = ([__temp, __restore] = _withAsyncContext(() => getAccounts()), __temp = await __temp, __restore(), __temp);
    function refreshAllAccounts() {
      // TODO
    }
    function showMenu(host, id, ev) {
      let menu;
      menu = [{
        text: i18n.ts.switch,
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(_component_SearchMarker, {
        ...
        default: _withCtx(() => [_createElementVNode("div", { class: "_gaps" }, [_createElementVNode("div", { class: "_buttons" }, [_createVNode(MkButton, {
          primary: "",
          onClick: addAccount
        }, {
          default: _withCtx(() => [
          ...
          _Fragment,
          null,
          _renderList(_unref(accounts), (x) => {
            return _openBlock(), _createElementBlock(
              _Fragment,
              { key: x.host + x.id },
              [x.user ? (_openBlock(), _createBlock(MkUserCardMini, {
                key: 0,
                user: x.user,
                class: _normalizeClass(_ctx.$style.user),
                onClick: _withModifiers(($event) => showMenu(x.host, x.id, $event), ["prevent"])
              }, null, 10, ["user", "onClick"])) : _createCommentVNode("v-if", true)],
              64
              /* STABLE_FRAGMENT */
            );
          }),
