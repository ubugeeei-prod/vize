...
export default {
  __name: "invite",
  setup(__props) {
    const currentInviteLimit = ref(null);
    const inviteLimit = $i != null && $i.policies.inviteLimit || $i == null && instance.policies.inviteLimit;
    const inviteLimitCycle = $i != null && $i.policies.inviteLimitCycle || $i == null && instance.policies.inviteLimitCycle;
    const paginator = markRaw(new Paginator("invite/list", { limit: 10 }));
    const resetCycle = computed(() => {
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(
        _component_PageWithHeader,
        null,
        {
          default: _withCtx(() => [!_unref(instance).disableRegistration || !(_unref($i) && (_unref($i).isAdmin || _unref($i).policies.canInvite)) ? (_openBlock(), _createElementBlock("div", {
            key: 0,
            class: "_spacer",
            style: "--MI_SPACER-w: 1200px;"
          }, [_createVNode(_component_MkResult, { type: "empty" })])) : (_openBlock(), _createElementBlock("div", {
            key: 1,
            class: "_spacer",
            ...
            resetCycle.value && _unref(inviteLimit) ? (_openBlock(), _createElementBlock(
              "div",
              { key: 0 },
              _toDisplayString(_unref(i18n).tsx.inviteLimitResetCycle({
                time: resetCycle.value,
                limit: _unref(inviteLimit)
              })),
              1
              /* TEXT */
            ...
            _createVNode(MkButton, {
              ...
              default: _withCtx(() => [
                ...
                _createTextVNode(" "),
                _createTextVNode(
                  _toDisplayString(_unref(i18n).ts.createInviteCode),
                  1
                  /* TEXT */
                )
              ]),
            ...
            currentInviteLimit.value !== null ? (_openBlock(), _createElementBlock(
              "div",
              { key: 0 },
              _toDisplayString(_unref(i18n).tsx.createLimitRemaining({ limit: currentInviteLimit.value })),
              1
              /* TEXT */
            )) : _createCommentVNode("v-if", true),
