  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const end = computed(() => {
      switch (widgetProps.period) {
        case "3day": return new Date(begin.value.getTime() + 1e3 * 60 * 60 * 24 * 3);
        case "week": return new Date(begin.value.getTime() + 1e3 * 60 * 60 * 24 * 7);
        case "month": return new Date(begin.value.getTime() + 1e3 * 60 * 60 * 24 * 30);
        default: return begin.value;
      }
    });
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createBlock(MkContainer, {
        ...
        header: _withCtx(() => [_createTextVNode(
          _toDisplayString(_unref(i18n).ts._widgets.birthdayFollowings),
          1
          /* TEXT */
        )]),
        ...
        default: _withCtx(() => [_createVNode(MkPagination, { paginator: _unref(birthdayUsersPaginator) }, {
          default: _withCtx(({ items }) => [_createElementVNode("div", null, [(_openBlock(true), _createElementBlock(
            _Fragment,
            null,
            _renderList(items, (user, i) => {
              return _openBlock(), _createElementBlock(
                _Fragment,
                { key: user.id },
                [i > 0 && _unref(isSeparatorNeeded)(_unref(birthdayUsersPaginator).items.value[i - 1].birthday, user.birthday) ? (_openBlock(), _createElementBlock("div", { key: 0 }, [_createElementVNode(
                  "div",
                  { class: _normalizeClass(_ctx.$style.date) },
                  [
                    _createElementVNode("span", null, [_hoisted_3, _createTextVNode(
                      " " + _toDisplayString(_unref(getSeparatorInfo)(_unref(birthdayUsersPaginator).items.value[i - 1].birthday, user.birthday)?.prevText),
                      1
                      /* TEXT */
                    )]),
                    _hoisted_4,
                    _createElementVNode("span", null, [_createTextVNode(
                      _toDisplayString(_unref(getSeparatorInfo)(_unref(birthdayUsersPaginator).items.value[i - 1].birthday, user.birthday)?.nextText) + " ",
                      1
                      /* TEXT */
                    ), _hoisted_5])
                  ],
                  ...
                  item: user
                }, null, 10, ["item"])])) : (_openBlock(), _createBlock(XUser, {
                  key: 1,
                  class: _normalizeClass(_ctx.$style.user),
                  item: user
                }, null, 10, ["item"]))],
