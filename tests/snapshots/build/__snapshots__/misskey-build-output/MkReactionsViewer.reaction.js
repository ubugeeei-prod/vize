  ...
  setup(__props, { emit: __emit }) {
    ...
    async function toggleReaction() {
      ...
      if (oldReaction) {
        ...
        misskeyApi("notes/reactions/delete", { noteId: props.noteId }).then(() => {
    ...
    async function menu(ev) {
      ...
      if (canGetInfo.value) {
        menuItems.push({
          ...
          action: async () => {
            const { dispose } = os.popup(MkCustomEmojiDetailedDialog, { emoji: await misskeyApiGet("emoji", { name: props.reaction.replace(/:/g, "").replace(/@\./, "") }) }, { closed: () => dispose() });
          }
        });
      }
    ...
    return (_ctx, _cache) => {
      ...
      return _withDirectives((_openBlock(), _createElementBlock(
        "button",
        {
          ...
          class: _normalizeClass(["_button", [_ctx.$style.root, {
            [_ctx.$style.reacted]: __props.myReaction == __props.reaction,
            [_ctx.$style.canToggle]: canToggle.value,
            [_ctx.$style.small]: _unref(prefer).s.reactionsDisplaySize === "small",
            [_ctx.$style.large]: _unref(prefer).s.reactionsDisplaySize === "large"
          }]]),
          onClick: _cache[0] || (_cache[0] = ($event) => toggleReaction()),
        ...
        }, null, 10, ["reaction", "emojiUrl"]), _createElementVNode(
          "span",
      ...
      )), [[_directive_ripple, canToggle.value]]);
    };
  }
};
