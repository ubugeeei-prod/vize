  ...
  setup(__props, { emit: __emit }) {
    ...
    onMounted(async () => {
      if (props.modelValue.fileId == null) {
        ...
        misskeyApi("drive/files/show", { fileId: props.modelValue.fileId }).then((fileResponse) => {
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createBlock(XContainer, {
        ...
        header: _withCtx(() => [
          ...
          _createTextVNode(" "),
          _createTextVNode(
            _toDisplayString(_unref(i18n).ts._pages.blocks.image),
            1
            /* TEXT */
          )
        ]),
        func: _withCtx(() => [_createElementVNode("button", { onClick: _cache[1] || (_cache[1] = ($event) => choose()) }, [_hoisted_2])]),
        default: _withCtx(() => [_createElementVNode("section", null, [file.value ? (_openBlock(), _createBlock(MkDriveFileThumbnail, {
          key: 0,
          style: "height: 150px;",
          file: file.value,
          fit: "contain",
          onClick: _cache[2] || (_cache[2] = ($event) => choose())
        }, null, 8, ["file"])) : _createCommentVNode("v-if", true)])]),
        _: 1
      }, 8, ["draggable"]);
    };
  }
