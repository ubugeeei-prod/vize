...
export default {
  ...
  setup(__props) {
    ...
    async function edit(avatarDecoration) {
      const { dispose } = await os.popupAsyncWithDialog(import("./avatar-decoration-edit-dialog.vue").then((x) => x.default), { avatarDecoration }, {
        done: (result) => {
          if (result.updated) {
          ...
          [(_openBlock(true), _createElementBlock(
            ...
            _renderList(avatarDecorations.value, (avatarDecoration) => {
              return _withDirectives((_openBlock(), _createElementBlock("div", {
                ...
                onClick: ($event) => edit(avatarDecoration)
              }, [_createElementVNode(
                "div",
                { class: _normalizeClass(_ctx.$style.decorationName) },
                [_createVNode(_component_MkCondensedLine, { minScale: .5 }, {
                  default: _withCtx(() => [_createTextVNode(
                    _toDisplayString(avatarDecoration.name),
                    1
                    /* TEXT */
                  )]),
                  _: 2
                ...
                forceShowDecoration: ""
              }, null, 8, ["user", "decorations"])], 10, ["onClick"])), [[_directive_panel]]);
            }),
            128
            /* KEYED_FRAGMENT */
          ))],
