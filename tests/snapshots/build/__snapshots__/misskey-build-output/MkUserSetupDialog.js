...
const _hoisted_5 = /* @__PURE__ */ _createElementVNode("i", {
  ...
  style: "display: block; margin: auto; font-size: 3em; color: var(--MI_THEME-accent);"
});
...
export default {
  ...
  setup(__props, { emit: __emit }) {
    ...
    function launchTutorial() {
      setupComplete();
      nextTick(async () => {
        const { dispose } = await os.popupAsyncWithDialog(import("@/components/MkTutorialDialog.vue").then((x) => x.default), { initialPage: 1 }, { closed: () => dispose() });
      });
    }
    async function later(later) {
      if (later) {
        const { canceled } = await os.confirm({
          type: "warning",
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createBlock(MkModalWindow, {
        ...
        height: 550,
        "data-cy-user-setup": "",
        onClose: _cache[0] || (_cache[0] = ($event) => close(true)),
        onClosed: _cache[1] || (_cache[1] = ($event) => emit("closed"))
      }, _createSlots({ _: 2 }, [page.value === 1 ? {
        name: "header",
        fn: _withCtx(() => [
          _hoisted_1,
          _createTextVNode(" "),
