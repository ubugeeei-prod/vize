...
export default {
  ...
  setup(__props, { emit: __emit }) {
    const emit = __emit;
    const props = __props;
    const emojis = ref(deepClone(props.palette.emojis));
    watch(emojis, () => {
      emit("updateEmojis", emojis.value);
    }, { deep: true });
    function remove(reaction, ev) {
      os.popupMenu([{
        text: i18n.ts.remove,
        action: () => {
    ...
    function pick(ev) {
      os.pickEmoji(getHTMLElement(ev), { showPinned: false }).then((it) => {
        const emoji = it;
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(MkFolder, { defaultOpen: true }, {
        ...
        label: _withCtx(() => [_createTextVNode(
          _toDisplayString(__props.palette.name === "" ? "(" + _unref(i18n).ts.noName + ")" : __props.palette.name),
          1
          /* TEXT */
        )]),
        footer: _withCtx(() => [_createElementVNode("div", { class: "_buttons" }, [
          ...
          _createVNode(MkButton, {
            ...
            default: _withCtx(() => [
              ...
              _createTextVNode(" "),
              _createTextVNode(
                _toDisplayString(_unref(i18n).ts.paste),
                1
                /* TEXT */
              )
            ]),
        ...
        default: _withCtx(() => [_createElementVNode("div", null, [_withDirectives(_createElementVNode("div", { style: "border-radius: 6px;" }, [_createVNode(MkDraggable, {
          ...
          default: _withCtx(({ item }) => [_createElementVNode("button", {
            class: _normalizeClass(["_button", _ctx.$style.emojisItem]),
            onClick: ($event) => remove(item.emoji, $event)
          }, [item.emoji[0] === ":" ? (_openBlock(), _createBlock(MkCustomEmoji, {
            key: 0,
            ...
            emoji: item.emoji,
            normal: true
          }, null, 8, ["emoji", "normal"]))], 10, ["onClick"])]),
