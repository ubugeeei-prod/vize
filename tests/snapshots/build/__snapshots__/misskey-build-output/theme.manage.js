  ...
  setup(__props) {
    ...
    const { model: selectedThemeId, def: selectedThemeIdDef } = useMkSelect({
      items: computed(() => [{
        ...
        items: installedThemes.value.map((x) => ({
          label: x.name,
          value: x.id
        }))
      }, {
        ...
        items: builtinThemes.value.map((x) => ({
          label: x.name,
          value: x.id
        }))
      }]),
    ...
    function uninstall() {
      removeTheme(selectedTheme.value);
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createElementBlock("div", { class: "_gaps_m" }, [_createVNode(MkSelect, {
        ...
        "onUpdate:modelValue": _cache[0] || (_cache[0] = ($event) => selectedThemeId.value = $event)
      }, {
        ...
        _: 1
      }, 8, ["items", "modelValue"]), selectedTheme.value != null ? (_openBlock(), _createElementBlock(
        _Fragment,
        { key: 0 },
        [
          ...
          _createVNode(MkTextarea, {
            ...
            caption: _withCtx(() => [_createElementVNode(
              ...
              {
                class: "_textButton",
                onClick: _cache[1] || (_cache[1] = ($event) => copyThemeCode())
              },
