  ...
  setup(__props) {
    ...
    function menu(ev) {
      ...
      menuItems.push({
        ...
        action: async () => {
          const { dispose } = os.popup(MkCustomEmojiDetailedDialog, { emoji: await misskeyApiGet("emoji", { name: props.emoji.name }) }, { closed: () => dispose() });
        }
      });
      if ($i?.isModerator ?? $i?.isAdmin) {
        menuItems.push({
          ...
          icon: "ti ti-pencil",
          action: async () => {
            const detailedEmoji = await misskeyApiGet("emoji", { name: props.emoji.name });
            const { dispose } = await os.popupAsyncWithDialog(import("@/pages/emoji-edit-dialog.vue").then((x) => x.default), { emoji: detailedEmoji }, { closed: () => dispose() });
          }
        });
      }
      ...
      return _openBlock(), _createElementBlock(
        ...
        [_createElementVNode("img", {
          ...
          [_createElementVNode(
            ...
            _toDisplayString(__props.emoji.aliases.join(" ")),
            3
            /* TEXT, CLASS */
          )],
          2
