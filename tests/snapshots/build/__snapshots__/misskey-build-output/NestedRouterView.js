...
export default {
  ...
  setup(__props) {
    ...
    function resolveNested(current, d = 0) {
      if (d === currentDepth) {
        return current;
      } else {
        if (current.child) {
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(_Suspense, { timeout: 0 }, {
        ...
        default: _withCtx(() => [_createVNode(
          _resolveDynamicComponent(currentPageComponent.value),
          _mergeProps(Object.fromEntries(currentPageProps.value), { key: key.value }),
          null,
          16
          /* FULL_PROPS */
        )]),
