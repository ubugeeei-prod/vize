  ...
  setup(__props) {
    const { model: type, def: typeDef } = useMkSelect({
      items: [{
        label: i18n.ts.all,
        value: null
      }, ...Misskey.moderationLogTypes.map((t) => ({
        label: i18n.ts._moderationLogTypes[t] ?? t,
        value: t
      }))],
      initialValue: null
    });
        ...
        default: _withCtx(() => [_createElementVNode("div", {
          ...
          _createVNode(MkPaginationControl, {
            ...
            default: _withCtx(() => [_createVNode(MkSelect, {
              ...
              "onUpdate:modelValue": _cache[1] || (_cache[1] = ($event) => moderatorId.value = $event)
            }, {
              label: _withCtx(() => [_createTextVNode(
                _toDisplayString(_unref(i18n).ts.moderator) + "(ID)",
                1
                /* TEXT */
              )]),
              _: 1
          ...
          _createVNode(_resolveDynamicComponent(_unref(prefer).s.enablePullToRefresh ? MkPullToRefresh : "div"), { refresher: () => _unref(paginator).reload() }, {
            default: _withCtx(() => [_unref(paginator).fetching.value ? (_openBlock(), _createBlock(_component_MkLoading, { key: 0 })) : _unref(paginator).error.value ? (_openBlock(), _createBlock(_component_MkError, {
              key: 1,
              onRetry: _cache[2] || (_cache[2] = ($event) => _unref(paginator).init())
            })) : (_openBlock(), _createBlock(MkTl, {
              key: 2,
              events: timeline.value,
              groupBy: "d"
            }, {
              left: _withCtx(({ event }) => [_createElementVNode("div", null, [_createVNode(_component_MkAvatar, {
                user: event.user,
                style: "width: 26px; height: 26px;"
              }, null, 8, ["user"])])]),
              right: _withCtx(({ event, timestamp, delta }) => [_createElementVNode("div", { style: "margin: 4px 0;" }, [_createVNode(XModLog, {
                key: event.id,
                log: event
              }, null, 8, ["log"])])]),
              _: 1
