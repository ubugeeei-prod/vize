...
export default {
  ...
  setup(__props, { emit: __emit }) {
    ...
    */
    const columns = ref(columnSettings.map(createColumn));
    /**
    ...
    */
    const previousCellAddress = ref(CELL_ADDRESS_NONE);
    /**
    * 編集中のセルのアドレスを保持するための変数。
    */
    const editingCellAddress = ref(CELL_ADDRESS_NONE);
    /**
    * 列の範囲選択をする際の開始地点となるインデックスを保持するための変数。
    * この開始地点からマウスが動いた地点までの範囲を選択する。
    */
    const firstSelectionColumnIdx = ref(CELL_ADDRESS_NONE.col);
    /**
    * 行の範囲選択をする際の開始地点となるインデックスを保持するための変数。
    * この開始地点からマウスが動いた地点までの範囲を選択する。
    */
    const firstSelectionRowIdx = ref(CELL_ADDRESS_NONE.row);
    /**
    ...
    */
          ...
          makeHotkey({
            ...
            "shift+right": () => {
              updateSelectionRange({
                ...
                rightBottom: {
                  col: bounds.rightBottom.col > selectedCellAddress.col || bounds.leftTop.col === selectedCellAddress.col ? bounds.rightBottom.col + 1 : selectedCellAddress.col,
                  row: bounds.rightBottom.row
                }
              });
            },
            "shift+left": () => {
              updateSelectionRange({
                leftTop: {
                  col: bounds.leftTop.col < selectedCellAddress.col || bounds.rightBottom.col === selectedCellAddress.col ? bounds.leftTop.col - 1 : selectedCellAddress.col,
                  row: bounds.leftTop.row
                },
                rightBottom: {
                  col: bounds.rightBottom.col > selectedCellAddress.col ? bounds.rightBottom.col - 1 : selectedCellAddress.col,
                  row: bounds.rightBottom.row
                }
              });
            },
            "shift+up": () => {
              updateSelectionRange({
                leftTop: {
                  col: bounds.leftTop.col,
                  row: bounds.leftTop.row < selectedCellAddress.row || bounds.rightBottom.row === selectedCellAddress.row ? bounds.leftTop.row - 1 : selectedCellAddress.row
                },
                rightBottom: {
                  col: bounds.rightBottom.col,
                  row: bounds.rightBottom.row > selectedCellAddress.row ? bounds.rightBottom.row - 1 : selectedCellAddress.row
                }
              });
            },
            "shift+down": () => {
              updateSelectionRange({
                ...
                rightBottom: {
                  col: bounds.rightBottom.col,
                  row: bounds.rightBottom.row > selectedCellAddress.row || bounds.leftTop.row === selectedCellAddress.row ? bounds.rightBottom.row + 1 : selectedCellAddress.row
                }
              });
            },
            "down": () => {
    ...
    function onLeftMouseDown(ev) {
      const cellAddress = getCellAddress(ev.target);
      if (_DEV_) {
      ...
      switch (state.value) {
        ...
        case "normal": {
          if (availableCellAddress(cellAddress)) {
            ...
            getCellElement(ev.target)?.focus();
    ...
    function onMouseMove(ev) {
      ev.preventDefault();
      const targetCellAddress = getCellAddress(ev.target);
    ...
    function onCellEditBegin(sender) {
    ...
    function onChangeCellValue(sender, newValue) {
      applyRowRules([sender]);
      emitCellValue(sender, newValue);
    }
    function onChangeCellContentSize(sender, contentSize) {
      const _cells = cells.value;
    ...
    function onHeaderCellChangeWidth(sender, width) {
      switch (state.value) {
        case "colResizing": {
    ...
    function onHeaderCellChangeContentSize(sender, newSize) {
      switch (state.value) {
        case "normal": {
    ...
    function onHeaderCellWidthLargest(sender) {
      switch (state.value) {
        case "normal": {
          calcLargestCellWidth(sender);
          break;
    ...
    */
    function calcLargestCellWidth(column) {
      const _cells = cells.value;
      const largestColumnWidth = columns.value[column.index].contentSize.width;
      const largestCellWidth = _cells.length > 0 ? _cells.map((row) => row.cells[column.index]).reduce((acc, value) => Math.max(acc, value.contentSize.width), 0) : 0;
      if (_DEV_) {
    ...
    */
    function emitCellValue(sender, newValue) {
    ...
    */
    function selectionRange(...targets) {
      const _cells = cells.value;
    ...
    */
    function unSelectionOutOfRange(leftTop, rightBottom) {
      const safeBounds = getSafeAddressBounds({
        leftTop,
        rightBottom
      });
      const _cells = rangedCells.value;
    ...
    */
    function expandCellRange(leftTop, rightBottom) {
      const safeBounds = getSafeAddressBounds({
        leftTop,
        rightBottom
      });
    ...
    */
    function expandRowRange(top, bottom) {
    ...
    */
    function applyRowRules(targetCells) {
      ...
      for (const group of rowGroups.filter((it) => it.row.using)) {
        ...
        const newStyles = rowSetting.styleRules.filter((it) => it.condition({
          row,
          targetCols,
          cells: rowCells
    ...
    function availableCellAddress(cellAddress) {
      const safeBounds = availableBounds.value;
      return cellAddress.row >= safeBounds.leftTop.row && cellAddress.col >= safeBounds.leftTop.col && cellAddress.row <= safeBounds.rightBottom.row && cellAddress.col <= safeBounds.rightBottom.col;
    }
    function isColumnHeaderCellAddress(cellAddress) {
      return cellAddress.row === -1 && cellAddress.col >= 0;
    }
    function isRowNumberCellAddress(cellAddress) {
    ...
    function refreshData() {
      ...
      const _rows = _data.length > rowSetting.minimumDefinitionCount ? _data.map((_, index) => createRow(index, true, rowSetting)) : Array.from({ length: rowSetting.minimumDefinitionCount }, (_, index) => createRow(index, index < _data.length, rowSetting));
      const _cols = columns.value;
      // 行・列の定義から、元データの配列より値を取得してセルを作成する。
      // 行・列の定義はそれぞれインデックスを持っており、そのインデックスは元データの配列番地に対応している。
      const _cells = _rows.map((row) => {
        const newCells = row.using ? _cols.map((col) => createCell(col, row, _data[row.index][col.setting.bindTo], cellSettings)) : _cols.map((col) => createCell(col, row, undefined, cellSettings));
        return {
          row,
          cells: newCells,
          origin: _data[row.index]
        };
      });
      rows.value = _rows;
      cells.value = _cells;
    ...
    */
    function patchData(newItems) {
      if (_DEV_) {
        console.log("[grid][patch-data][begin]");
      }
      const _cols = columns.value;
      if (rows.value.length < newItems.length) {
        const newRows = Array.of();
        const newCells = Array.of();
      ...
      for (let rowIdx = 0; rowIdx < newItems.length; rowIdx++) {
        ...
        for (let colIdx = 0; colIdx < oldCells.length; colIdx++) {
          ...
          if (oldCell.value !== newValue) {
            oldCell.value = _col.setting.valueTransformer ? _col.setting.valueTransformer(holder.row, _col, newValue) : newValue;
      ...
      if (changedCells.length > 0) {
        ...
        emitGridEvent({
          ...
          all: cells.value.filter((it) => it.row.using).flatMap((it) => it.cells).map((it) => it.violation).filter((it) => !it.valid)
        });
      }
      if (_DEV_) {
      ...
      return _openBlock(), _createElementBlock(
        "div",
        {
          ...
          class: _normalizeClass(["mk_grid_border", [_ctx.$style.grid, {
            [_ctx.$style.noOverflowHandling]: rootSetting.noOverflowStyle,
        ...
        [_createElementVNode("div", { class: "mk_grid_thead" }, [_createVNode(MkHeaderRow, {
          ...
          "gridSetting",
          "bus"
        ])]), _createElementVNode("div", { class: "mk_grid_tbody" }, [(_openBlock(true), _createElementBlock(
          _Fragment,
          null,
          _renderList(rows.value, (row) => {
