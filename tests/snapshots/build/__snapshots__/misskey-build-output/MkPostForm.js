...
export default {
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const renoteTargetNote = shallowRef(props.renote);
    const replyTargetNote = shallowRef(props.reply);
    const targetChannel = shallowRef(props.channel);
    const serverDraftId = ref(null);
    const postFormActions = getPluginHandlers("post_form_action");
    let textAutocomplete = null;
    let cwAutocomplete = null;
    let hashtagAutocomplete = null;
    const uploader = useUploader({ multiple: true });
    onUnmounted(() => {
      uploader.dispose();
    });
    ...
    const submitText = computed(() => {
      return scheduledAt.value != null ? i18n.ts.schedule : renoteTargetNote.value ? i18n.ts.quote : replyTargetNote.value ? i18n.ts.reply : i18n.ts.note;
    });
    ...
    if (replyTargetNote.value && replyTargetNote.value.text != null) {
      ...
      for (const x of extractMentions(ast)) {
        const mention = x.host ? `@${x.username}@${toASCII(x.host)}` : otherHost == null || otherHost === host ? `@${x.username}` : `@${x.username}@${toASCII(otherHost)}`;
        // 自分は除外
    ...
    if (replyTargetNote.value && [
      ...
      "followers",
      "specified"
    ].includes(replyTargetNote.value.visibility)) {
      ...
      if (visibility.value === "specified") {
        if (replyTargetNote.value.visibleUserIds) {
          misskeyApi("users/show", { userIds: replyTargetNote.value.visibleUserIds.filter((uid) => uid !== $i.id && uid !== replyTargetNote.value?.userId) }).then((users) => {
    ...
    function chooseFileFromDrive(ev) {
      if (props.mock) return;
    ...
    function updateFileSensitive(file, isSensitive) {
      if (props.mock) {
      ...
      const select = await os.select({
        ...
        items: [
          ...
          },
          {
            ...
            label: i18n.ts.likeOnlyForRemote
          },
          {
            value: "nonSensitiveOnly",
            label: i18n.ts.nonSensitiveOnly
          },
          {
    ...
    async function post(ev) {
      ...
      if (props.mock) return;
      if (visibility.value === "public" && (useCw.value && cw.value != null && cw.value.trim() !== "" && isAnnoying(cw.value) || (!useCw.value || cw.value == null || cw.value.trim() === "") && text.value != null && text.value.trim() !== "" && isAnnoying(text.value))) {
      ...
      misskeyApi("notes/create", postData, token).then((res) => {
        ...
        nextTick(() => {
          ...
          if (postData.text && postData.text !== "") {
            const hashtags_ = mfm.parse(postData.text).map((x) => x.type === "hashtag" && x.props.hashtag).filter((x) => x);
            const history = JSON.parse(miLocalStorage.getItem("hashtags") ?? "[]");
    ...
    async function insertMfmFunction(ev) {
      if (textareaEl.value == null) return;
      let pos = textareaEl.value.selectionStart ?? 0;
      let posEnd = textareaEl.value.selectionEnd ?? text.value.length;
      mfmFunctionPicker(ev.currentTarget ?? ev.target, (tag) => {
        if (pos === posEnd) {
    ...
    async function openAccountMenu(ev) {
      if (props.mock) return;
      function showDraftsDialog(scheduled) {
        const { dispose } = os.popup(defineAsyncComponent(() => import("@/components/MkNoteDraftsDialog.vue")), { scheduled }, {
          restore: async (draft) => {
    ...
    function showPerUploadItemMenu(item, ev) {
      const menu = uploader.getMenu(item);
      os.popupMenu(menu, ev.currentTarget ?? ev.target);
    }
    function showPerUploadItemMenuViaContextmenu(item, ev) {
      const menu = uploader.getMenu(item);
      os.contextMenu(menu, ev);
    }
    async function schedule() {
      const { canceled, result } = await os.inputDatetime({ title: i18n.ts.schedulePost });
      if (canceled) return;
    ...
    function showTour() {
      if (textareaEl.value == null || footerEl.value == null || accountMenuEl.value == null || visibilityButton.value == null || otherSettingsButton.value == null || submitButtonEl.value == null) {
        return;
      }
      startTour([
        {
        ...
        [
          _createElementVNode(
            ...
            [_createElementVNode(
              ...
              [!__props.fixed ? (_openBlock(), _createElementBlock(
                "button",
                {
                  key: 0,
              ...
              [
                !(targetChannel.value != null && __props.fixed) ? (_openBlock(), _createElementBlock(
                  _Fragment,
                  { key: 0 },
                  [targetChannel.value == null ? _withDirectives((_openBlock(), _createElementBlock(
                    "button",
                    {
                      key: 0,
                      ref_key: "visibilityButton",
                      ref: visibilityButton,
                      class: _normalizeClass([
                        "_button",
                        _ctx.$style.headerRightItem,
                        _ctx.$style.visibility
                      ]),
                      onClick: setVisibility
                    },
                    [
                      visibility.value === "public" ? (_openBlock(), _createElementBlock("span", { key: 0 }, [_hoisted_2])) : _createCommentVNode("v-if", true),
                      visibility.value === "home" ? (_openBlock(), _createElementBlock("span", { key: 0 }, [_hoisted_3])) : _createCommentVNode("v-if", true),
                      visibility.value === "followers" ? (_openBlock(), _createElementBlock("span", { key: 0 }, [_hoisted_4])) : _createCommentVNode("v-if", true),
                      visibility.value === "specified" ? (_openBlock(), _createElementBlock("span", { key: 0 }, [_hoisted_5])) : _createCommentVNode("v-if", true),
                      _createElementVNode(
                        "span",
                        { class: _normalizeClass(_ctx.$style.headerRightButtonText) },
                        _toDisplayString(_unref(i18n).ts._visibility[visibility.value]),
                        3
                        /* TEXT, CLASS */
                      )
                    ],
                    2
                    /* CLASS */
                  )), [[_directive_tooltip, _unref(i18n).ts.visibility]]) : (_openBlock(), _createElementBlock(
                    "button",
                    {
                      key: 1,
                    ...
                    [_createElementVNode("span", null, [_hoisted_6]), _createElementVNode(
                      "span",
                      { class: _normalizeClass(_ctx.$style.headerRightButtonText) },
                      _toDisplayString(targetChannel.value.name),
                      3
                      /* TEXT, CLASS */
                    )],
                    2
                ...
                visibility.value !== "specified" ? _withDirectives((_openBlock(), _createElementBlock("button", {
                  key: 0,
                ...
                _withDirectives(_createElementVNode("button", {
                  ...
                  [posted.value ? (_openBlock(), _createElementBlock(
                    _Fragment,
                    { key: 0 },
                    null,
                    64
                    /* STABLE_FRAGMENT */
                  )) : posting.value ? (_openBlock(), _createBlock(_component_MkEllipsis, { key: 1 })) : (_openBlock(), _createElementBlock(
                    _Fragment,
                    { key: 2 },
                    [_createTextVNode(
                      _toDisplayString(submitText.value),
                      1
                      /* TEXT */
                    )],
                    64
          ...
          replyTargetNote.value ? (_openBlock(), _createBlock(MkNoteSimple, {
            key: 0,
            class: _normalizeClass(_ctx.$style.targetNote),
            note: replyTargetNote.value
          }, null, 10, ["note"])) : _createCommentVNode("v-if", true),
          renoteTargetNote.value ? (_openBlock(), _createBlock(MkNoteSimple, {
            key: 0,
            class: _normalizeClass(_ctx.$style.targetNote),
            note: renoteTargetNote.value
          }, null, 10, ["note"])) : _createCommentVNode("v-if", true),
          quoteId.value ? (_openBlock(), _createElementBlock(
            "div",
            {
              key: 0,
            ...
            [
              _hoisted_10,
              _createTextVNode(
                " " + _toDisplayString(_unref(i18n).ts.quoteAttached),
                1
                /* TEXT */
              ),
              _createElementVNode("button", { onClick: _cache[0] || (_cache[0] = ($event) => {
                quoteId.value = null;
          ...
          visibility.value === "specified" ? (_openBlock(), _createElementBlock(
            "div",
            {
              key: 0,
            ...
            [_createElementVNode(
              "span",
              _hoisted_12,
              _toDisplayString(_unref(i18n).ts.recipient),
              1
              /* TEXT */
            ), _createElementVNode(
              "div",
              ...
              [(_openBlock(true), _createElementBlock(
                _Fragment,
                null,
                _renderList(visibleUsers.value, (u) => {
                  return _openBlock(), _createElementBlock(
                    "span",
                    {
                      key: u.id,
                    ...
                    [_createVNode(_component_MkAcct, { user: u }, null, 8, ["user"]), _createElementVNode("button", {
                      class: "_button",
                      style: "padding: 4px 8px;",
                      onClick: ($event) => removeVisibleUser(u.id)
                    }, [_hoisted_13], 8, ["onClick"])],
                    2
                    /* CLASS */
                  );
                }),
          ...
          !_unref(store).r.tips.value.postForm ? (_openBlock(), _createBlock(
            MkInfo,
            {
              key: 0,
              class: _normalizeClass(_ctx.$style.showHowToUse),
              closable: "",
              onClose: _cache[1] || (_cache[1] = ($event) => _unref(closeTip)("postForm"))
            },
            {
              default: _withCtx(() => [_createElementVNode(
                ...
                _toDisplayString(_unref(i18n).ts._postForm.showHowToUse),
                1
                /* TEXT */
              )]),
              _: 1
          ...
          scheduledAt.value != null ? (_openBlock(), _createBlock(
            MkInfo,
            {
              key: 0,
            ...
            },
            {
              default: _withCtx(() => [
                _createVNode(_component_I18n, {
                  ...
                  x: _withCtx(() => [_createVNode(_component_MkTime, {
                ...
                _createElementVNode(
                  ...
                  {
                    class: "_textButton",
                    onClick: _cache[2] || (_cache[2] = ($event) => cancelSchedule())
                  },
          ...
          hasNotSpecifiedMentions.value ? (_openBlock(), _createBlock(
            MkInfo,
            {
              key: 0,
              warn: "",
            ...
            },
            {
              default: _withCtx(() => [
                _createTextVNode(
                  _toDisplayString(_unref(i18n).ts.notSpecifiedMentionWarning),
                  1
                  /* TEXT */
                ),
                _createTextVNode(" - "),
                _createElementVNode(
                  ...
                  {
                    class: "_textButton",
                    onClick: _cache[3] || (_cache[3] = ($event) => addMissingMention())
                  },
          ...
          _withDirectives(_createElementVNode(
            ...
            [_withDirectives(_createElementVNode("input", {
              ref_key: "cwInputEl",
              ref: cwInputEl,
              "onUpdate:modelValue": _cache[4] || (_cache[4] = ($event) => cw.value = $event),
              class: _normalizeClass(_ctx.$style.cw),
              placeholder: _unref(i18n).ts.annotation,
              onKeydown,
              onKeyup,
              onCompositionend: onCompositionEnd
            }, null, 42, ["placeholder"]), [[_vModelText, cw.value]]), maxCwTextLength - cwTextLength.value < 20 ? (_openBlock(), _createElementBlock(
              "div",
              {
                key: 0,
                class: _normalizeClass([
                  "_acrylic",
                  _ctx.$style.cwTextCount,
                  { [_ctx.$style.cwTextOver]: cwTextLength.value > maxCwTextLength }
                ])
              },
              _toDisplayString(maxCwTextLength - cwTextLength.value),
              3
              /* TEXT, CLASS */
            )) : _createCommentVNode("v-if", true)],
            2
          ...
          _createElementVNode(
            ...
            [
              targetChannel.value ? (_openBlock(), _createElementBlock(
                "div",
                {
                  key: 0,
              ...
              maxTextLength.value - textLength.value < 100 ? (_openBlock(), _createElementBlock(
                "div",
                {
                  key: 0,
                  class: _normalizeClass([
                    ...
                    { [_ctx.$style.textOver]: textLength.value > maxTextLength.value }
                  ])
                },
                _toDisplayString(maxTextLength.value - textLength.value),
                3
                /* TEXT, CLASS */
              )) : _createCommentVNode("v-if", true)
            ],
          ...
          _withDirectives(_createElementVNode("input", {
            ref_key: "hashtagsInputEl",
            ref: hashtagsInputEl,
            "onUpdate:modelValue": _cache[6] || (_cache[6] = ($event) => hashtags.value = $event),
            class: _normalizeClass(_ctx.$style.hashtags),
            placeholder: _unref(i18n).ts.hashtags,
            list: "hashtags"
          }, null, 10, ["placeholder"]), [[_vModelText, _unref(hashtags)]]),
          _createVNode(XPostFormAttaches, {
            onDetach: detachFile,
          ...
          _unref(uploader).items.value.length > 0 ? (_openBlock(), _createElementBlock("div", {
            ...
            default: _withCtx(() => [_createTextVNode(
              _toDisplayString(_unref(i18n).ts._postForm.uploaderTip),
              1
              /* TEXT */
            )]),
            _: 1
            ...
            onShowMenu: _cache[8] || (_cache[8] = (item, ev) => showPerUploadItemMenu(item, ev)),
            onShowMenuViaContextmenu: _cache[9] || (_cache[9] = (item, ev) => showPerUploadItemMenuViaContextmenu(item, ev))
          }, null, 8, ["items"])])) : _createCommentVNode("v-if", true),
          poll.value ? (_openBlock(), _createBlock(MkPollEditor, {
            key: 0,
            onDestroyed: _cache[10] || (_cache[10] = ($event) => poll.value = null),
            modelValue: poll.value,
            "onUpdate:modelValue": _cache[11] || (_cache[11] = ($event) => poll.value = $event)
          }, null, 8, ["modelValue"])) : _createCommentVNode("v-if", true),
          showPreview.value ? (_openBlock(), _createBlock(MkNotePreview, {
            key: 0,
          ...
          showingOptions.value ? (_openBlock(), _createElementBlock("div", {
            key: 0,
              ...
              [
                ...
                _withDirectives(_createElementVNode(
                  "button",
                  {
                    ...
                    onClick: _cache[12] || (_cache[12] = ($event) => useCw.value = !useCw.value)
                  },
                  [_hoisted_18],
                  2
                  /* CLASS */
                ...
                _withDirectives(_createElementVNode(
                  "button",
                  {
                    ...
                    onClick: _cache[13] || (_cache[13] = ($event) => withHashtags.value = !_unref(withHashtags))
                  },
                  [_hoisted_19],
                  2
                  /* CLASS */
                ...
                showAddMfmFunction.value ? _withDirectives((_openBlock(), _createElementBlock(
                  ...
                  2
                  /* CLASS */
                )), [[_directive_tooltip, _unref(i18n).ts.addMfmFunction]]) : _createCommentVNode("v-if", true),
                _unref(postFormActions).length > 0 ? _withDirectives((_openBlock(), _createElementBlock(
                  "button",
                  {
                    key: 0,
                    class: _normalizeClass(["_button", _ctx.$style.footerButton]),
                  ...
                  2
                  /* CLASS */
                )), [[_directive_tooltip, _unref(i18n).ts.plugins]]) : _createCommentVNode("v-if", true)
              ],
              2
              /* CLASS */
            ), _createElementVNode(
          ...
          _createElementVNode("datalist", { id: "hashtags" }, [(_openBlock(true), _createElementBlock(
            _Fragment,
            null,
            _renderList(recentHashtags.value, (hashtag) => {
