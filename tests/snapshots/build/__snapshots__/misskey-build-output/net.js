...
export default {
  ...
  setup(__props) {
    ...
    const stats = ref([]);
    const inPolylinePoints = ref("");
    const outPolylinePoints = ref("");
    const inPolygonPoints = ref("");
    const outPolygonPoints = ref("");
    const inHeadX = ref();
    const inHeadY = ref();
    const outHeadX = ref();
    const outHeadY = ref();
    const inRecent = ref(0);
    const outRecent = ref(0);
    onMounted(() => {
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createElementBlock("div", { class: "oxxrhrto" }, [_createElementVNode("svg", { viewBox: `0 0 ${viewBoxX.value} ${viewBoxY.value}` }, [
        ...
        _createElementVNode("text", {
          x: "1",
          y: "5"
        }, [_createTextVNode("NET rx "), _createElementVNode(
          "tspan",
          null,
          _toDisplayString(bytes(inRecent.value)),
          1
          /* TEXT */
        )])
        ...
        _createElementVNode("text", {
          x: "1",
          y: "5"
        }, [_createTextVNode("NET tx "), _createElementVNode(
          "tspan",
          null,
          _toDisplayString(bytes(outRecent.value)),
          1
          /* TEXT */
        )])
      ], 8, ["viewBox"])]);
