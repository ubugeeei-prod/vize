  ...
  setup(__props) {
    ...
    function _fetch_() {
      ...
      if (uri.startsWith("https://")) {
        promise = misskeyApi("ap/show", { uri }).then((res) => {
          if (res.type === "User") {
            mainRouter.replace("/@:acct/:page?", { params: { acct: res.object.host != null ? `${res.object.username}@${res.object.host}` : res.object.username } });
          } else if (res.type === "Note") {
            mainRouter.replace("/notes/:noteId/:initialTab?", { params: { noteId: res.object.id } });
          } else {
            os.alert({
              type: "error",
              text: "Not a user"
        ...
        promise = misskeyApi("users/show", Misskey.acct.parse(uri)).then((user) => {
          mainRouter.replace("/@:acct/:page?", { params: { acct: user.host != null ? `${user.username}@${user.host}` : user.username } });
        });
      }
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(_component_PageWithHeader, {
        ...
        }, [state.value === "done" ? (_openBlock(), _createElementBlock("div", {
          key: 0,
