...
export default {
  ...
  setup(__props) {
    const paginatorForNotes = markRaw(new Paginator("notes/featured", { limit: 10 }));
    const paginatorForPolls = markRaw(new Paginator("notes/polls/recommendation", {
      limit: 10,
      offsetMode: true,
      params: { excludeChannels: true }
    }));
    const tab = ref("notes");
    return (_ctx, _cache) => {
      return _openBlock(), _createElementBlock("div", {
        ...
        tabs: [{
          ...
          key: "polls",
          label: _unref(i18n).ts.poll
        }],
        style: "margin-bottom: var(--MI-margin);",
        modelValue: tab.value,
        "onUpdate:modelValue": _cache[0] || (_cache[0] = ($event) => tab.value = $event)
      }, null, 8, ["tabs", "modelValue"]), tab.value === "notes" ? (_openBlock(), _createBlock(MkNotesTimeline, {
        key: 0,
        paginator: _unref(paginatorForNotes)
      }, null, 8, ["paginator"])) : tab.value === "polls" ? (_openBlock(), _createBlock(MkNotesTimeline, {
        key: 1,
        paginator: _unref(paginatorForPolls)
      }, null, 8, ["paginator"])) : _createCommentVNode("v-if", true)]);
    };
  }
};
