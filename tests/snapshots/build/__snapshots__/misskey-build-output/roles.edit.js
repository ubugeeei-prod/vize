...
export default {
  ...
  async setup(__props) {
    ...
    const data = ref(null);
    if (props.id) {
      role.value = await misskeyApi("admin/roles/show", { roleId: props.id });
    ...
    async function save() {
      ...
      if (role.value) {
        ...
        router.push("/admin/roles/:id", { params: { id: role.value.id } });
      } else {
        const created = await os.apiWithDialog("admin/roles/create", { ...data.value });
        router.push("/admin/roles/:id", { params: { id: created.id } });
      }
    }
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(_component_PageWithHeader, { tabs: headerTabs.value }, {
        ...
        }, [data.value ? (_openBlock(), _createBlock(XEditor, {
          key: 0,
