  ...
  setup(__props) {
    ...
    async function renderChart() {
      ...
      chartInstance = new Chart(chartEl.value, {
        type: "bar",
        data: { datasets: [
          makeDataset("Follow (local)", format(raw.local.followings.inc).slice().reverse(), {
            backgroundColor: colorFollowLocal,
            stack: "follow"
          }),
          makeDataset("Follow (remote)", format(raw.remote.followings.inc).slice().reverse(), {
            backgroundColor: colorFollowRemote,
            stack: "follow"
          }),
          makeDataset("Followed (local)", format(raw.local.followers.inc).slice().reverse(), {
            backgroundColor: colorFollowedLocal,
            stack: "followed"
          }),
          makeDataset("Followed (remote)", format(raw.remote.followers.inc).slice().reverse(), {
            backgroundColor: colorFollowedRemote,
            stack: "followed"
          })
        ] },
        options: {
