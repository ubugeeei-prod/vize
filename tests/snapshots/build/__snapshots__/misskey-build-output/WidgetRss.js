...
export default {
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const intervalClear = ref();
    const tick = () => {
      if (window.document.visibilityState === "hidden" && rawItems.value.length !== 0) return;
      window.fetch(fetchEndpoint.value, {}).then((res) => res.json()).then((feed) => {
    ...
    watch(() => widgetProps.refreshIntervalSec, () => {
      ...
      intervalClear.value = useInterval(tick, Math.max(1e4, widgetProps.refreshIntervalSec * 1e3), {
        immediate: true,
        afterMounted: true
      });
    }, { immediate: true });
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(MkContainer, {
        ...
        header: _withCtx(() => [_createTextVNode("RSS")]),
        ...
        default: _withCtx(() => [_createElementVNode("div", { class: "ekmkgxbj" }, [fetching.value ? (_openBlock(), _createBlock(_component_MkLoading, { key: 0 })) : (!items.value || items.value.length === 0) && _unref(widgetProps).showHeader ? (_openBlock(), _createBlock(_component_MkResult, {
          key: 1,
          type: "empty"
          ...
          [(_openBlock(true), _createElementBlock(
            ...
            _renderList(items.value, (item) => {
              return _openBlock(), _createElementBlock("a", {
                ...
                target: "_blank",
                title: item.title
              }, _toDisplayString(item.title), 11, ["href", "title"]);
            }),
            128
            /* KEYED_FRAGMENT */
          ))],
