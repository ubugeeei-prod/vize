...
export default {
  ...
  setup(__props) {
    ...
    function computeTitle(event) {
      event.target.title = getEmojiName(props.emoji);
    }
    function mute() {
      os.confirm({
        type: "question",
    ...
    function onClick(ev) {
      if (props.menu) {
        ...
        menuItems.push({ type: "divider" }, isMuted.value ? {
    ...
    return (_ctx, _cache) => {
      return shouldMute.value ? (_openBlock(), _createElementBlock("img", {
        ...
        onClick
      }, null, 42, ["alt"])) : !useOsNativeEmojis.value ? (_openBlock(), _createElementBlock("img", {
        key: 1,
        class: _normalizeClass(_ctx.$style.root),
        src: url.value,
        alt: props.emoji,
        ...
        onPointerenter: computeTitle,
        onClick
      }, null, 42, ["src", "alt"])) : (_openBlock(), _createElementBlock("span", {
        key: 2,
        alt: props.emoji,
        onPointerenter: computeTitle,
        onClick
      }, _toDisplayString(colorizedNativeEmoji.value), 41, ["alt"]));
    };
  }
};
