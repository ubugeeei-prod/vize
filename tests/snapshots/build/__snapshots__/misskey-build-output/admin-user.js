...
export default {
  ...
  async setup(__props) {
    ...
    const result = ([__temp, __restore] = _withAsyncContext(() => _fetch_()), __temp = await __temp, __restore(), __temp);
    const tab = ref(props.initialTab);
    const { model: chartSrc, def: chartSrcDef } = useMkSelect({
      items: [{
        label: i18n.ts.notes,
        value: "per-user-notes"
      }],
    ...
    const filesPaginator = markRaw(new Paginator("admin/drive/files", {
      limit: 10,
      computedParams: computed(() => ({ userId: props.userId }))
    }));
    const { model: announcementsStatus, def: announcementsStatusDef } = useMkSelect({
      items: [{
        label: i18n.ts.active,
        value: "active"
      }, {
        label: i18n.ts.archived,
        value: "archived"
      }],
      initialValue: "active"
    });
    ...
    async function resetPassword() {
      ...
      if (confirm.canceled) {
        ...
        const { password } = await misskeyApi("admin/reset-password", { userId: user.value.id });
        os.alert({
          type: "success",
    ...
    async function deleteAccount() {
      ...
      if (confirm.canceled) return;
      const typed = await os.inputText({ text: i18n.tsx.typeToConfirm({ x: user.value?.username }) });
      if (typed.canceled) return;
      if (typed.result === user.value?.username) {
        await os.apiWithDialog("admin/delete-account", { userId: user.value.id });
      } else {
        os.alert({
          type: "error",
    ...
    async function assignRole() {
      ...
      if (canceled2) return;
      const expiresAt = period === "indefinitely" ? null : period === "oneHour" ? Date.now() + 1e3 * 60 * 60 : period === "oneDay" ? Date.now() + 1e3 * 60 * 60 * 24 : period === "oneWeek" ? Date.now() + 1e3 * 60 * 60 * 24 * 7 : period === "oneMonth" ? Date.now() + 1e3 * 60 * 60 * 24 * 30 : null;
    ...
    async function createAnnouncement() {
      const { dispose } = await os.popupAsyncWithDialog(import("@/components/MkUserAnnouncementEditDialog.vue").then((x) => x.default), { user: user.value }, { closed: () => dispose() });
    }
    ...
    watch(user, () => {
      misskeyApi("ap/get", { uri: user.value.uri ?? `${url}/users/${user.value.id}` }).then((res) => {
        ap.value = res;
      });
    });
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(_component_PageWithHeader, {
        ...
        default: _withCtx(() => [_createElementVNode("div", {
        ...
        }, [tab.value === "overview" ? (_openBlock(), _createElementBlock("div", {
          key: 0,
          class: "_gaps_m"
        }, [
          _createElementVNode("div", { class: "aeakzknw" }, [_createVNode(_component_MkAvatar, {
            class: "avatar",
            user: user.value,
            indicator: "",
            link: "",
            preview: ""
          }, null, 8, ["user"]), _createElementVNode("div", { class: "body" }, [
            _createElementVNode("span", { class: "name" }, [_createVNode(_component_MkUserName, {
              class: "name",
              user: user.value
            }, null, 8, ["user"])]),
            _createElementVNode("span", { class: "sub" }, [_createElementVNode(
              "span",
              _hoisted_1,
              "@" + _toDisplayString(_unref(acct)(user.value)),
              1
              /* TEXT */
            )]),
            _createElementVNode("span", { class: "state" }, [
              suspended.value ? (_openBlock(), _createElementBlock("span", {
                key: 0,
                class: "suspended"
              }, "Suspended")) : _createCommentVNode("v-if", true),
              silenced.value ? (_openBlock(), _createElementBlock("span", {
                key: 0,
                class: "silenced"
              }, "Silenced")) : _createCommentVNode("v-if", true),
              moderator.value ? (_openBlock(), _createElementBlock("span", {
                key: 0,
                class: "moderator"
              }, "Moderator")) : _createCommentVNode("v-if", true)
            ])
          ])]),
          isSystem.value ? (_openBlock(), _createBlock(MkInfo, { key: 0 }, {
            default: _withCtx(() => [_createTextVNode(
              _toDisplayString(_unref(i18n).ts.isSystemAccount),
              1
              /* TEXT */
            )]),
            _: 1
          })) : _createCommentVNode("v-if", true),
          user.value.host ? (_openBlock(), _createBlock(FormLink, {
            key: 0,
            to: `/instance-info/${user.value.host}`
          }, {
            default: _withCtx(() => [_createTextVNode(
              _toDisplayString(_unref(i18n).ts.instanceInfo),
              1
              /* TEXT */
            )]),
            _: 1
          }, 8, ["to"])) : _createCommentVNode("v-if", true),
          _createElementVNode("div", { style: "display: flex; flex-direction: column; gap: 1em;" }, [_createVNode(MkKeyValue, {
            copy: user.value.id,
            oneline: ""
          }, {
            key: _withCtx(() => [_createTextVNode("ID")]),
            value: _withCtx(() => [_createElementVNode(
              "span",
              _hoisted_2,
              _toDisplayString(user.value.id),
              1
              /* TEXT */
            )]),
            _: 1
          }, 8, ["copy"]), !isSystem.value ? (_openBlock(), _createElementBlock(
            _Fragment,
            { key: 0 },
            [
              _createVNode(MkKeyValue, { oneline: "" }, {
                ...
                value: _withCtx(() => [_createElementVNode("span", { class: "_monospace" }, [_createVNode(_component_MkTime, {
                  time: user.value.createdAt,
                  mode: "detail"
                }, null, 8, ["time", "mode"])])]),
                _: 1
              }),
              info.value ? (_openBlock(), _createBlock(MkKeyValue, {
                key: 0,
                oneline: ""
              }, {
                key: _withCtx(() => [_createTextVNode(
                  _toDisplayString(_unref(i18n).ts.lastActiveDate),
                  1
                  /* TEXT */
                )]),
                value: _withCtx(() => [_createElementVNode("span", { class: "_monospace" }, [_createVNode(_component_MkTime, {
              ...
              info.value ? (_openBlock(), _createBlock(MkKeyValue, {
                ...
                key: _withCtx(() => [_createTextVNode(
                  _toDisplayString(_unref(i18n).ts.email),
                  1
                  /* TEXT */
                )]),
                value: _withCtx(() => [_createElementVNode(
                  "span",
                  _hoisted_3,
                  _toDisplayString(info.value.email),
                  1
                  /* TEXT */
                )]),
                _: 1
          ...
          !isSystem.value ? (_openBlock(), _createBlock(MkTextarea, {
            key: 0,
            manualSave: "",
            modelValue: moderationNote.value,
            "onUpdate:modelValue": _cache[1] || (_cache[1] = ($event) => moderationNote.value = $event)
          }, {
            label: _withCtx(() => [_createTextVNode(
              _toDisplayString(_unref(i18n).ts.moderationNote),
              1
              /* TEXT */
            )]),
            caption: _withCtx(() => [_createTextVNode(
              _toDisplayString(_unref(i18n).ts.moderationNoteDescription),
              1
              /* TEXT */
            )]),
            _: 1
          }, 8, ["modelValue"])) : _createCommentVNode("v-if", true),
          !isSystem.value ? (_openBlock(), _createBlock(FormSection, { key: 0 }, {
            default: _withCtx(() => [_createElementVNode("div", { class: "_gaps" }, [
              _createVNode(MkSwitch, {
                "onUpdate:modelValue": [toggleSuspend, ($event) => suspended.value = $event],
                modelValue: suspended.value
              }, {
                default: _withCtx(() => [_createTextVNode(
                  _toDisplayString(_unref(i18n).ts.suspend),
                  1
                  /* TEXT */
                )]),
                _: 1
              }, 8, ["modelValue"]),
              _createElementVNode("div", null, [user.value.host == null ? (_openBlock(), _createBlock(MkButton, {
                key: 0,
                inline: "",
                ...
                default: _withCtx(() => [
                  ...
                  _createTextVNode(" "),
                  _createTextVNode(
                    _toDisplayString(_unref(i18n).ts.resetPassword),
                    1
                    /* TEXT */
                  )
                ]),
              ...
              _createVNode(MkFolder, null, {
                ...
                default: _withCtx(() => [_createElementVNode("div", { class: "_gaps" }, [(_openBlock(true), _createElementBlock(
                  _Fragment,
                  null,
                  _renderList(Object.keys(info.value.policies), (policy) => {
              ...
              _createVNode(
                ...
                null,
                {
                  ...
                  label: _withCtx(() => [_createTextVNode("IP")]),
                  default: _withCtx(() => [!_unref(iAmAdmin) ? (_openBlock(), _createBlock(MkInfo, {
                    key: 0,
                    warn: ""
                  }, {
                    default: _withCtx(() => [_createTextVNode(
                      _toDisplayString(_unref(i18n).ts.requireAdminForView),
                      1
                      /* TEXT */
                    )]),
                    _: 1
                  })) : (_openBlock(), _createBlock(MkInfo, { key: 1 }, {
                    default: _withCtx(() => [_createTextVNode("The date is the IP address was first acknowledged.")]),
                    _: 1
                  })), _unref(iAmAdmin) && ips.value ? (_openBlock(), _createElementBlock(
                    _Fragment,
                    { key: 0 },
                    [(_openBlock(true), _createElementBlock(
                      _Fragment,
                      null,
                      _renderList(ips.value, (record) => {
                        return _openBlock(), _createElementBlock(
                          "div",
                          {
                            key: record.ip,
                          ...
                          [_createElementVNode(
                            "span",
                            _hoisted_7,
                            _toDisplayString(record.createdAt),
                            1
                            /* TEXT */
                          ), _createElementVNode(
                            "span",
                            _hoisted_8,
                            _toDisplayString(record.ip),
                            1
                            /* TEXT */
                          )],
                          2
              ...
              _createElementVNode("div", null, [_unref(iAmModerator) ? (_openBlock(), _createBlock(MkButton, {
                key: 0,
                inline: "",
                danger: "",
                style: "margin-right: 8px;",
                ...
                default: _withCtx(() => [
                  ...
                  _createTextVNode(" "),
                  _createTextVNode(
                    _toDisplayString(_unref(i18n).ts.unsetUserAvatar),
                    1
                    /* TEXT */
                  )
                ]),
                _: 1
              })) : _createCommentVNode("v-if", true), _unref(iAmModerator) ? (_openBlock(), _createBlock(MkButton, {
                key: 0,
                inline: "",
                danger: "",
                ...
                default: _withCtx(() => [
                  ...
                  _createTextVNode(" "),
                  _createTextVNode(
                    _toDisplayString(_unref(i18n).ts.unsetUserBanner),
                    1
                    /* TEXT */
                  )
                ]),
              ...
              _unref($i).isAdmin ? (_openBlock(), _createBlock(MkButton, {
                key: 0,
                inline: "",
                danger: "",
                ...
                default: _withCtx(() => [_createTextVNode(
                  _toDisplayString(_unref(i18n).ts.deleteAccount),
                  1
                  /* TEXT */
                )]),
                _: 1
        ...
        ])) : tab.value === "roles" ? (_openBlock(), _createElementBlock("div", {
          key: 1,
          class: "_gaps"
        }, [user.value.host == null ? (_openBlock(), _createBlock(MkButton, {
          key: 0,
          primary: "",
          rounded: "",
          onClick: assignRole
          ...
          default: _withCtx(() => [
            ...
            _createTextVNode(" "),
            _createTextVNode(
              _toDisplayString(_unref(i18n).ts.assign),
              1
              /* TEXT */
            )
          ]),
          _: 1
        })) : _createCommentVNode("v-if", true), (_openBlock(true), _createElementBlock(
          _Fragment,
          null,
          _renderList(info.value.roles, (role) => {
            return _openBlock(), _createElementBlock("div", { key: role.id }, [_createElementVNode(
              "div",
              ...
              [
                _createVNode(MkRolePreview, {
                  ...
                  role,
                  forModeration: true
                }, null, 10, ["role", "forModeration"]),
                _createElementVNode("button", {
                  class: "_button",
                  onClick: ($event) => toggleRoleItem(role)
                }, [_hoisted_12], 8, ["onClick"]),
                role.target === "manual" ? (_openBlock(), _createElementBlock("button", {
                  key: 0,
                  class: _normalizeClass(["_button", _ctx.$style.roleUnassign]),
                  onClick: ($event) => unassignRole(role, $event)
                }, [_hoisted_13], 10, ["onClick"])) : (_openBlock(), _createElementBlock(
                  "button",
                  {
                    key: 1,
                    class: _normalizeClass(["_button", _ctx.$style.roleUnassign]),
              ...
              2
              /* CLASS */
            ), expandedRoleIds.value.includes(role.id) ? (_openBlock(), _createElementBlock(
              "div",
              {
                key: 0,
          ...
          128
          /* KEYED_FRAGMENT */
        ))])) : tab.value === "announcements" ? (_openBlock(), _createElementBlock("div", {
          key: 2,
          class: "_gaps"
        }, [
          _createVNode(MkButton, {
            ...
            default: _withCtx(() => [
              ...
              _createTextVNode(" "),
              _createTextVNode(
                _toDisplayString(_unref(i18n).ts.createNew),
                1
                /* TEXT */
              )
            ]),
          ...
          _createVNode(MkSelect, {
            ...
            "onUpdate:modelValue": _cache[2] || (_cache[2] = ($event) => announcementsStatus.value = $event)
          }, {
            label: _withCtx(() => [_createTextVNode(
              _toDisplayString(_unref(i18n).ts.filter),
              1
              /* TEXT */
            )]),
            _: 1
          ...
          _createVNode(MkPagination, { paginator: _unref(announcementsPaginator) }, {
            default: _withCtx(({ items }) => [_createElementVNode("div", { class: "_gaps_s" }, [(_openBlock(true), _createElementBlock(
              _Fragment,
              null,
              _renderList(items, (announcement) => {
                return _withDirectives((_openBlock(), _createElementBlock("div", {
                  key: announcement.id,
                  class: _normalizeClass(_ctx.$style.announcementItem),
                  onClick: ($event) => editAnnouncement(announcement)
                }, [
                  "icon" in announcement ? (_openBlock(), _createElementBlock("span", {
                    key: 0,
                    style: "margin-right: 0.5em;"
                  }, [announcement.icon === "info" ? (_openBlock(), _createElementBlock("i", {
                    key: 0,
                    class: "ti ti-info-circle"
                  })) : announcement.icon === "warning" ? (_openBlock(), _createElementBlock("i", {
                    key: 1,
                    class: "ti ti-alert-triangle",
                    style: "color: var(--MI_THEME-warn);"
                  })) : announcement.icon === "error" ? (_openBlock(), _createElementBlock("i", {
                    key: 2,
                    class: "ti ti-circle-x",
                    style: "color: var(--MI_THEME-error);"
                  })) : announcement.icon === "success" ? (_openBlock(), _createElementBlock("i", {
                    key: 3,
                    class: "ti ti-check",
                  ...
                  _createElementVNode(
                    "span",
                    null,
                    _toDisplayString(announcement.title),
                    1
                    /* TEXT */
                  ),
                  announcement.reads > 0 ? (_openBlock(), _createElementBlock(
                    "span",
                    {
                      key: 0,
                      style: "margin-left: auto; opacity: 0.7;"
                    },
                    _toDisplayString(_unref(i18n).ts.messageRead),
                    1
                    /* TEXT */
                  )) : _createCommentVNode("v-if", true)
                ], 10, ["onClick"])), [[_directive_panel]]);
              }),
              128
              /* KEYED_FRAGMENT */
            ))])]),
        ...
        ])) : tab.value === "drive" ? (_openBlock(), _createElementBlock("div", {
          key: 3,
          class: "_gaps"
        }, [_createVNode(MkFileListForAdmin, {
          paginator: _unref(filesPaginator),
          viewMode: "grid"
        }, null, 8, ["paginator"])])) : tab.value === "chart" ? (_openBlock(), _createElementBlock("div", {
          key: 4,
          class: "_gaps_m"
        }, [_createElementVNode("div", { class: "cmhjzshm" }, [_createElementVNode("div", { class: "selects" }, [_createVNode(MkSelect, {
          items: _unref(chartSrcDef),
          style: "margin: 0 10px 0 0; flex: 1;",
          modelValue: _unref(chartSrc),
          "onUpdate:modelValue": _cache[3] || (_cache[3] = ($event) => chartSrc.value = $event)
        }, null, 8, ["items", "modelValue"])]), _createElementVNode("div", { class: "charts" }, [
          _createElementVNode(
            "div",
            _hoisted_16,
            _toDisplayString(_unref(i18n).tsx.recentNHours({ n: 90 })),
            1
            /* TEXT */
          ),
          ...
          _createElementVNode(
            "div",
            _hoisted_17,
            _toDisplayString(_unref(i18n).tsx.recentNDays({ n: 90 })),
            1
            /* TEXT */
          ),
          _createVNode(MkChart, {
        ...
        ])])])) : tab.value === "raw" ? (_openBlock(), _createElementBlock("div", {
          key: 5,
          class: "_gaps_m"
        }, [info.value && _unref($i).isAdmin ? (_openBlock(), _createBlock(MkObjectView, {
          key: 0,
          tall: "",
          value: info.value
        }, null, 8, ["value"])) : _createCommentVNode("v-if", true), _createVNode(MkObjectView, {
          tall: "",
          value: user.value
