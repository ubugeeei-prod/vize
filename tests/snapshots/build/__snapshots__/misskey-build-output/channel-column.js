  ...
  setup(__props) {
    ...
    onMounted(() => {
      if (props.column.channelId == null) {
        ...
        misskeyApi("channels/show", { channelId: props.column.channelId }).then((value) => updateColumn(props.column.id, { timelineNameCache: value.name }));
      }
    });
    ...
    async function post() {
      ...
      if (!channel.value || channel.value.id !== props.column.channelId) {
        channel.value = await misskeyApi("channels/show", { channelId: props.column.channelId });
      }
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createBlock(XColumn, {
        ...
        header: _withCtx(() => [_hoisted_1, _createElementVNode(
          "span",
          _hoisted_2,
          _toDisplayString(__props.column.name || __props.column.timelineNameCache || _unref(i18n).ts._deck._columns.channel),
          1
          /* TEXT */
        )]),
        default: _withCtx(() => [__props.column.channelId ? (_openBlock(), _createElementBlock(
          _Fragment,
          { key: 0 },
          [_createElementVNode("div", { style: "padding: 8px; text-align: center;" }, [_createVNode(MkButton, {
            primary: "",
            gradate: "",
            rounded: "",
            inline: "",
