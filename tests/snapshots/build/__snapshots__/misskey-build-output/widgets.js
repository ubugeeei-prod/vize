import { openBlock as _openBlock, createElementBlock as _createElementBlock, createVNode as _createVNode, createElementVNode as _createElementVNode, createTextVNode as _createTextVNode, toDisplayString as _toDisplayString, normalizeClass as _normalizeClass, unref as _unref } from "vue";
...
export default {
  ...
  setup(__props) {
    ...
    function updateWidgets(thisWidgets) {
      ...
      if (props.place === "left") {
        prefer.commit("widgets", [...thisWidgets.map((w) => ({
          ...w,
          place: "left"
        })), ...prefer.s.widgets.filter((w) => w.place !== "left" && !thisWidgets.some((t) => w.id === t.id))]);
        return;
      }
      prefer.commit("widgets", [...prefer.s.widgets.filter((w) => w.place === "left" && !thisWidgets.some((t) => w.id === t.id)), ...thisWidgets.map((w) => ({
        ...w,
        place: "right"
      }))]);
    }
    return (_ctx, _cache) => {
      return _openBlock(), _createElementBlock("div", null, [_createVNode(XWidgets, {
        ...
        onExit: _cache[0] || (_cache[0] = ($event) => _ctx.editMode = false)
      }, null, 8, ["edit", "widgets"]), _ctx.editMode ? (_openBlock(), _createElementBlock("button", {
        key: 0,
        class: "_textButton",
        style: "font-size: 0.9em;",
        onClick: _cache[1] || (_cache[1] = ($event) => _ctx.editMode = false)
      }, [_hoisted_1, _createTextVNode(
        " " + _toDisplayString(_unref(i18n).ts.editWidgetsExit),
        1
        /* TEXT */
        ...
        "button",
        {
          ...
          onClick: _cache[2] || (_cache[2] = ($event) => _ctx.editMode = true)
        },
        [_hoisted_2, _createTextVNode(
          " " + _toDisplayString(_unref(i18n).ts.editWidgets),
          1
          /* TEXT */
        )],
        2
