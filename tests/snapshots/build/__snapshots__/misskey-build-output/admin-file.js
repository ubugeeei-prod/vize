...
export default {
  ...
  setup(__props) {
    const props = __props;
    function _fetch_() {
      return Promise.all([misskeyApi("drive/files/show", { fileId: props.fileId }), misskeyApi("admin/drive/show-file", { fileId: props.fileId })]).then((result) => ({
        file: result[0],
        info: result[1]
      }));
    }
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(_component_MkSuspense, {
        ...
        default: _withCtx(({ result }) => [result.file != null && result.info != null ? (_openBlock(), _createBlock(XRoot, {
          key: 0,
          file: result.file,
          info: result.info
        }, null, 8, ["file", "info"])) : _createCommentVNode("v-if", true)]),
        _: 2
      }, 1032, ["p"]);
    };
  }
