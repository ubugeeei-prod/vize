...
export default {
  ...
  setup(__props) {
    ...
    const url = computed(() => {
      ...
      const proxied = rawUrl.value.startsWith("/emoji/") || props.useOriginalSize && isLocal.value ? rawUrl.value : getProxiedImageUrl(rawUrl.value, props.useOriginalSize ? undefined : "emoji", false, true);
      ...
      if (props.menu) {
        ...
        if (isLocal.value) {
          menuItems.push({ type: "divider" }, {
            ...
            action: async () => {
              const { dispose } = os.popup(MkCustomEmojiDetailedDialog, { emoji: await misskeyApiGet("emoji", { name: customEmojiName.value }) }, { closed: () => dispose() });
            }
          });
        }
        if (isMuted.value) {
    ...
    async function edit(name) {
      const emoji = await misskeyApi("emoji", { name });
      const { dispose } = await os.popupAsyncWithDialog(import("@/pages/emoji-edit-dialog.vue").then((x) => x.default), { emoji }, { closed: () => dispose() });
    }
    function mute() {
    ...
    return (_ctx, _cache) => {
      return shouldMute.value ? (_openBlock(), _createElementBlock("img", {
        key: 0,
        class: _normalizeClass([_ctx.$style.root, {
          [_ctx.$style.normal]: __props.normal,
          [_ctx.$style.noStyle]: __props.noStyle
        }]),
        ...
        onClick
      }, null, 10, ["title"])) : errored.value && __props.fallbackToImage ? (_openBlock(), _createElementBlock("img", {
        key: 1,
        class: _normalizeClass([_ctx.$style.root, {
          [_ctx.$style.normal]: __props.normal,
          [_ctx.$style.noStyle]: __props.noStyle
        ...
        draggable: "false",
        style: "-webkit-user-drag: none;"
      }, null, 10, ["title"])) : errored.value ? (_openBlock(), _createElementBlock(
        "span",
        { key: 2 },
        ":" + _toDisplayString(customEmojiName.value) + ":",
        1
        /* TEXT */
        ...
        decoding: "async",
        draggable: "false",
        onError: _cache[0] || (_cache[0] = ($event) => errored.value = true),
        onLoad: _cache[1] || (_cache[1] = ($event) => errored.value = false),
        onClick
      }, null, 42, [
        "src",
        "alt",
