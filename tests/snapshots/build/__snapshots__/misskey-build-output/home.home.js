  ...
  setup(__props) {
    ...
    async function startUser() {
      ...
      os.selectUser({ localOnly: true }).then((user) => {
        router.push("/chat/user/:userId", { params: { userId: user.id } });
      });
    }
    async function createRoom() {
      ...
      if (canceled) return;
      const room = await misskeyApi("chat/rooms/create", { name: result });
      router.push("/chat/room/:roomId", { params: { roomId: room.id } });
    }
    async function search() {
      const res = await misskeyApi("chat/messages/search", { query: searchQuery.value });
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createElementBlock("div", { class: "_gaps" }, [
        _unref($i).policies.chatAvailability === "available" ? (_openBlock(), _createBlock(
          MkButton,
          {
            key: 0,
            primary: "",
          ...
          },
          {
            default: _withCtx(() => [
              ...
              _createTextVNode(" "),
              _createTextVNode(
                _toDisplayString(_unref(i18n).ts.startChat),
                1
                /* TEXT */
              )
            ]),
        ...
        _createVNode(MkFoldableSection, null, {
          header: _withCtx(() => [_createTextVNode(
            _toDisplayString(_unref(i18n).ts._chat.history),
            1
            /* TEXT */
          )]),
          default: _withCtx(() => [_createVNode(MkChatHistories)]),
          _: 1
        })
      ]);
    };
