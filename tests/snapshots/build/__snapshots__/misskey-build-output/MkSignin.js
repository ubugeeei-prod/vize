import { Transition as _Transition, openBlock as _openBlock, createBlock as _createBlock, createElementBlock as _createElementBlock, createVNode as _createVNode, createCommentVNode as _createCommentVNode, resolveComponent as _resolveComponent, normalizeClass as _normalizeClass, withCtx as _withCtx } from "vue";
...
export default {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const credentialRequest = shallowRef(null);
    ...
    function onPasskeyLogin() {
      if (webAuthnSupported()) {
        ...
        waiting.value = true;
        misskeyApi("signin-with-passkey", {}).then((res) => {
...
publicKey: res.option });
    ...
    function onPasskeyDone(credential) {
      waiting.value = true;
    ...
    //#endregion
    async function onUsernameSubmitted(username) {
      waiting.value = true;
      userInfo.value = await misskeyApi("users/show", { username }).catch(() => null);
      await tryLogin({ username });
    }
    async function onPasswordSubmitted(pw) {
      waiting.value = true;
    ...
    async function onTotpSubmitted(token) {
      waiting.value = true;
      if (userInfo.value == null) {
        await os.alert({
          type: "error",
    ...
    async function tryLogin(req) {
      const _req = {
        username: req.username ?? userInfo.value?.username,
        ...req
      };
      function assertIsSigninFlowRequest(x) {
...
publicKey: res.authRequest });
    ...
    async function onLoginSucceeded(res) {
      if (props.autoSet) {
        await login(res.i);
      }
    }
    function onSigninApiError(err) {
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createElementBlock(
        ...
        [_createVNode(_Transition, {
          ...
          default: _withCtx(() => [page.value === "input" ? (_openBlock(), _createBlock(XInput, {
            key: "input",
            ...
            "openOnRemote",
            "initialUsername"
          ])) : page.value === "password" ? (_openBlock(), _createBlock(XPassword, {
            key: "password",
            ...
            onUseTotp
          }, null, 8, ["credentialRequest", "isPerformingPasswordlessLogin"])) : _createCommentVNode("v-if", true)]),
          _: 2
        }, 1032, [
          "enterActiveClass",
          "leaveActiveClass",
          ...
          "leaveToClass",
          "inert"
        ]), waiting.value ? (_openBlock(), _createElementBlock(
          "div",
          {
            key: 0,
