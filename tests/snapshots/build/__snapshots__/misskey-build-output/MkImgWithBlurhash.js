...
const canvasPromise = new Promise((resolve) => {
  ...
  testWorker.addEventListener("message", (event) => {
    if (event.data.result) {
      const workers = new WorkerMultiDispatch(() => new DrawBlurhash(), Math.min(navigator.hardwareConcurrency - 1, 4));
      resolve(workers);
    } else {
      const canvas = window.document.createElement("canvas");
      canvas.width = 64;
  ...
  setup(__props) {
    ...
    function waitForDecode() {
      if (props.src != null && props.src !== "") {
        nextTick().then(() => img.value?.decode()).then(() => {
          loaded.value = true;
        }, (error) => {
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createElementBlock("div", {
        ...
        class: _normalizeClass([
          "chromatic-ignore",
          _ctx.$style.root,
          { [_ctx.$style.cover]: __props.cover }
        ]),
        title: __props.title ?? ""
        ...
        default: _withCtx(() => [_withDirectives(_createElementVNode("canvas", {
          key: "canvas",
          ref_key: "canvas",
          ref: canvas,
          ...
          "height",
          "title"
        ]), [[_vShow, hide.value]]), _withDirectives(_createElementVNode("img", {
          key: "img",
          ref_key: "img",
          ref: img,
