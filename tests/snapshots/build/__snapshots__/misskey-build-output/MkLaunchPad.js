...
export default {
  ...
  setup(__props, { emit: __emit }) {
    const emit = __emit;
    const props = __props;
    const preferedModalType = deviceKind === "desktop" && props.anchorElement != null ? "popup" : deviceKind === "smartphone" ? "drawer" : "dialog";
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(MkModal, {
        ...
        default: _withCtx(({ type, maxHeight }) => [_createElementVNode(
          "div",
          {
            class: _normalizeClass(["szkkfdyq _popup _shadow", { asDrawer: type === "drawer" }]),
            style: _normalizeStyle({ maxHeight: maxHeight ? maxHeight + "px" : "" })
          },
          [_createElementVNode("div", { class: "main" }, [(_openBlock(true), _createElementBlock(
            _Fragment,
            null,
            _renderList(_unref(items), (item) => {
              return _openBlock(), _createElementBlock(
                _Fragment,
                { key: item.text },
                [item.action != null ? _withDirectives((_openBlock(), _createElementBlock("button", {
                  key: 0,
                  class: "_button item",
                  onClick: ($event) => {
                    item.action($event);
                  ...
                  item.indicate && item.indicateValue ? (_openBlock(), _createElementBlock(
                    "span",
                    {
                      key: 0,
                      class: "_indicateCounter indicatorWithValue"
                    },
                    _toDisplayString(item.indicateValue),
                    1
                    /* TEXT */
                  )) : item.indicate ? (_openBlock(), _createElementBlock("span", {
                    key: 1,
                    class: "indicator _blink"
                  }, [_hoisted_2])) : _createCommentVNode("v-if", true)
                ], 8, ["onClick"])), [[_directive_click_anime]]) : item.to != null ? _withDirectives((_openBlock(), _createBlock(_component_MkA, {
                  key: 1,
                  to: item.to,
                  class: "item",
                  onClickPassive: ($event) => close()
                }, {
                  default: _withCtx(() => [
                    _createElementVNode(
                      "i",
                  ...
                  _: 2
                }, 1032, ["to", "onClickPassive"])), [[_directive_click_anime]]) : _createCommentVNode("v-if", true)],
                64
                /* STABLE_FRAGMENT */
              );
            }),
