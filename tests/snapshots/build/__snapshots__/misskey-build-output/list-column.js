  ...
  setup(__props) {
    ...
    onMounted(() => {
      if (props.column.listId == null) {
        ...
        misskeyApi("users/lists/show", { listId: props.column.listId }).then((value) => updateColumn(props.column.id, { timelineNameCache: value.name }));
      }
    });
    watch(withRenotes, (v) => {
    ...
    async function setList() {
      ...
      if (listIdOrOperation === "_CREATE_") {
        const { canceled, result: name } = await os.inputText({ title: i18n.ts.enterListName });
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createBlock(XColumn, {
        ...
        header: _withCtx(() => [_hoisted_1, _createElementVNode(
          "span",
          _hoisted_2,
          _toDisplayString(__props.column.name || __props.column.timelineNameCache || _unref(i18n).ts._deck._columns.list),
          1
          /* TEXT */
        )]),
        default: _withCtx(() => [__props.column.listId ? (_openBlock(), _createBlock(MkStreamingNotesTimeline, {
          key: 0,
          ref_key: "timeline",
          ref: timeline,
          src: "list",
        ...
        }, null, 8, ["list", "withRenotes"])) : _createCommentVNode("v-if", true)]),
        _: 2
      }, 1032, [
        "menu",
        "column",
