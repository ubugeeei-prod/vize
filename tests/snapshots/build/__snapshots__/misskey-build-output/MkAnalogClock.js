...
const graduationsPadding = .5;
const textsPadding = .6;
const handsPadding = 1;
const handsTailLength = .7;
const hHandLengthRatio = .75;
const mHandLengthRatio = 1;
const sHandLengthRatio = 1;
const numbersOpacityFactor = .35;
export default {
  ...
  setup(__props) {
    ...
    let enabled = true;
    const majorGraduationColor = ref();
    //let minorGraduationColor = $ref<string>();
    const sHandColor = ref();
    const mHandColor = ref();
    const hHandColor = ref();
    const nowColor = ref();
    const h = ref(0);
    const m = ref(0);
    const s = ref(0);
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createElementBlock(
        ...
        [
          props.graduations === "dots" ? (_openBlock(), _createElementBlock(
            _Fragment,
            { key: 0 },
            [(_openBlock(true), _createElementBlock(
              _Fragment,
              null,
              _renderList(graduationsMajor.value, (angle, i) => {
                return _openBlock(), _createElementBlock("circle", {
                  cx: 5 + Math.sin(angle) * (5 - graduationsPadding),
                  cy: 5 - Math.cos(angle) * (5 - graduationsPadding),
                  r: .125,
                  fill: (props.twentyfour ? h.value : h.value % 12) === i ? nowColor.value : majorGraduationColor.value,
                  opacity: !props.fadeGraduations || (props.twentyfour ? h.value : h.value % 12) === i ? 1 : Math.max(0, 1 - angleDiff(hAngle.value, angle) / Math.PI - numbersOpacityFactor)
                }, 8, [
                  "cx",
                  "cy",
                  "r",
            ...
            64
            /* STABLE_FRAGMENT */
          )) : props.graduations === "numbers" ? (_openBlock(), _createElementBlock(
            _Fragment,
            { key: 1 },
            [(_openBlock(true), _createElementBlock(
              _Fragment,
              null,
              _renderList(texts.value, (angle, i) => {
                return _openBlock(), _createElementBlock("text", {
                  x: 5 + Math.sin(angle) * (5 - textsPadding),
                  y: 5 - Math.cos(angle) * (5 - textsPadding),
                  "text-anchor": "middle",
                  "dominant-baseline": "middle",
                  "font-size": (props.twentyfour ? h.value : h.value % 12) === i ? 1 : .7,
                  "font-weight": (props.twentyfour ? h.value : h.value % 12) === i ? "bold" : "normal",
                  fill: (props.twentyfour ? h.value : h.value % 12) === i ? nowColor.value : "currentColor",
                  opacity: !props.fadeGraduations || (props.twentyfour ? h.value : h.value % 12) === i ? 1 : Math.max(0, 1 - angleDiff(hAngle.value, angle) / Math.PI - numbersOpacityFactor)
                }, _toDisplayString(i === 0 ? props.twentyfour ? "24" : "12" : i), 9, [
                  "x",
                  "y",
                  "font-size",
                  "font-weight",
          ...
          _createElementVNode("line", {
            ref_key: "sLine",
            ref: sLine,
            class: _normalizeClass([_ctx.$style.s, {
              [_ctx.$style.animate]: !disableSAnimate.value && __props.sAnimation !== "none",
              [_ctx.$style.elastic]: __props.sAnimation === "elastic",
              [_ctx.$style.easeOut]: __props.sAnimation === "easeOut"
            }]),
