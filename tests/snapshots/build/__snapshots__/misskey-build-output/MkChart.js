...
export default {
  ...
  setup(__props) {
      ...
      chartInstance = new Chart(chartEl.value, {
        ...
        data: {
          ...
          datasets: chartData.series.map((x, i) => ({
            ...
            backgroundColor: props.bar ? x.color ? x.color : getColor(i) : alpha(x.color ? x.color : getColor(i), .1),
            gradient: props.bar ? undefined : { backgroundColor: {
              axis: "y",
              colors: {
                0: alpha(x.color ? x.color : getColor(i), 0),
                [maxes[i]]: alpha(x.color ? x.color : getColor(i), .35)
              }
            } },
            barPercentage: .9,
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              ...
              callbacks: { label: (item) => `${item.dataset.label}: ${chartData?.bytes ? bytes(item.parsed.y * 1e3, 1) : item.parsed.y.toString()}` }
            },
            zoom: props.detailed ? {
              pan: { enabled: true },
              zoom: {
    ...
    const fetchFederationChart = async () => {
    ...
    const fetchApRequestChart = async () => {
    ...
    const fetchNotesChart = async (type) => {
      ...
      return { series: [
        {
          name: "All",
          type: "line",
          data: format(type === "combined" ? sum(raw.local.inc, negate(raw.local.dec), raw.remote.inc, negate(raw.remote.dec)) : sum(raw[type].inc, negate(raw[type].dec))),
          color: "#888888"
        },
        {
          name: "Renotes",
          type: "area",
          data: format(type === "combined" ? sum(raw.local.diffs.renote, raw.remote.diffs.renote) : raw[type].diffs.renote),
          color: colors.green
        },
        {
          name: "Replies",
          type: "area",
          data: format(type === "combined" ? sum(raw.local.diffs.reply, raw.remote.diffs.reply) : raw[type].diffs.reply),
          color: colors.yellow
        },
        {
          name: "Normal",
          type: "area",
          data: format(type === "combined" ? sum(raw.local.diffs.normal, raw.remote.diffs.normal) : raw[type].diffs.normal),
          color: colors.blue
        },
        {
          name: "With file",
          type: "area",
          data: format(type === "combined" ? sum(raw.local.diffs.withFile, raw.remote.diffs.withFile) : raw[type].diffs.withFile),
          color: colors.purple
        }
      ] };
    };
    const fetchNotesTotalChart = async () => {
      const raw = await misskeyApiGet("charts/notes", {
        limit: props.limit,
        span: props.span
      });
    ...
    const fetchUsersChart = async (total) => {
      ...
      return { series: [
        {
          name: "Combined",
          type: "line",
          data: format(total ? sum(raw.local.total, raw.remote.total) : sum(raw.local.inc, negate(raw.local.dec), raw.remote.inc, negate(raw.remote.dec)))
        },
        {
          name: "Local",
          type: "area",
          data: format(total ? raw.local.total : sum(raw.local.inc, negate(raw.local.dec)))
        },
        {
          name: "Remote",
          type: "area",
          data: format(total ? raw.remote.total : sum(raw.remote.inc, negate(raw.remote.dec)))
        }
      ] };
    };
        ...
        series: [
          {
            ...
            type: "line",
            dashed: true,
            data: format(sum(raw.local.incSize, negate(raw.local.decSize), raw.remote.incSize, negate(raw.remote.decSize)))
          },
          {
            name: "Local +",
            type: "area",
    ...
    const fetchDriveFilesChart = async () => {
      const raw = await misskeyApiGet("charts/drive", {
        limit: props.limit,
        span: props.span
      });
      return { series: [
        {
          ...
          type: "line",
          dashed: true,
          data: format(sum(raw.local.incCount, negate(raw.local.decCount), raw.remote.incCount, negate(raw.remote.decCount)))
        },
        {
          name: "Local +",
          type: "area",
    ...
    const fetchInstanceRequestsChart = async () => {
    ...
    const fetchInstanceUsersChart = async (total) => {
      const host = props.args?.host;
      if (host == null) return { series: [] };
      const raw = await misskeyApiGet("charts/instance", {
        host,
      ...
      return { series: [{
        ...
        type: "area",
        color: "#008FFB",
        data: format(total ? raw.users.total : sum(raw.users.inc, negate(raw.users.dec)))
      }] };
    };
    const fetchInstanceNotesChart = async (total) => {
      const host = props.args?.host;
      if (host == null) return { series: [] };
      const raw = await misskeyApiGet("charts/instance", {
        host,
      ...
      return { series: [{
        ...
        type: "area",
        color: "#008FFB",
        data: format(total ? raw.notes.total : sum(raw.notes.inc, negate(raw.notes.dec)))
      }] };
    };
    const fetchInstanceFfChart = async (total) => {
      const host = props.args?.host;
      if (host == null) return { series: [] };
      const raw = await misskeyApiGet("charts/instance", {
        host,
      ...
      return { series: [{
        ...
        type: "area",
        color: "#008FFB",
        data: format(total ? raw.following.total : sum(raw.following.inc, negate(raw.following.dec)))
      }, {
        name: "Followers",
        type: "area",
        color: "#00E396",
        data: format(total ? raw.followers.total : sum(raw.followers.inc, negate(raw.followers.dec)))
      }] };
    };
    const fetchInstanceDriveUsageChart = async (total) => {
      const host = props.args?.host;
      if (host == null) return { series: [] };
      const raw = await misskeyApiGet("charts/instance", {
        host,
      ...
      return {
        ...
        series: [{
          ...
          type: "area",
          color: "#008FFB",
          data: format(total ? sum(raw.drive.incUsage) : sum(raw.drive.incUsage, negate(raw.drive.decUsage)))
        }]
      };
    };
    const fetchInstanceDriveFilesChart = async (total) => {
      const host = props.args?.host;
      if (host == null) return { series: [] };
      const raw = await misskeyApiGet("charts/instance", {
        host,
      ...
      return { series: [{
        ...
        type: "area",
        color: "#008FFB",
        data: format(total ? raw.drive.totalFiles : sum(raw.drive.incFiles, negate(raw.drive.decFiles)))
      }] };
    };
    const fetchPerUserNotesChart = async () => {
    ...
    const fetchPerUserPvChart = async () => {
      const userId = props.args?.user?.id;
      if (userId == null) return { series: [] };
    ...
    const fetchPerUserFollowingChart = async () => {
      const userId = props.args?.user?.id;
      if (userId == null) return { series: [] };
    ...
    const fetchPerUserFollowersChart = async () => {
      const userId = props.args?.user?.id;
      if (userId == null) return { series: [] };
      const raw = await misskeyApiGet("charts/user/following", {
        userId,
    ...
    const fetchPerUserDriveChart = async () => {
      const userId = props.args?.user?.id;
      if (userId == null) return { series: [] };
