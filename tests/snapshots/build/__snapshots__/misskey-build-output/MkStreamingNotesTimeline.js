  ...
  setup(__props, { expose: __expose }) {
    ...
    if (props.src === "antenna") {
      paginator = markRaw(new Paginator("antennas/notes", {
        computedParams: computed(() => ({ antennaId: props.antenna })),
        useShallowRef: true
      }));
      ...
      paginator = markRaw(new Paginator("notes/mentions", { useShallowRef: true }));
      ...
      paginator = markRaw(new Paginator("channels/timeline", {
        computedParams: computed(() => ({ channelId: props.channel })),
        useShallowRef: true
      }));
    } else if (props.src === "role") {
      paginator = markRaw(new Paginator("roles/notes", {
        computedParams: computed(() => ({ roleId: props.role })),
        useShallowRef: true
      }));
    } else {
    ...
    const MIN_POLLING_INTERVAL = 1e3 * 10;
    const POLLING_INTERVAL = prefer.s.pollingInterval === 1 ? MIN_POLLING_INTERVAL * 1.5 * 1.5 : prefer.s.pollingInterval === 2 ? MIN_POLLING_INTERVAL * 1.5 : prefer.s.pollingInterval === 3 ? MIN_POLLING_INTERVAL : MIN_POLLING_INTERVAL;
    if (!store.s.realtimeMode) {
      // TODO: 先頭のノートの作成日時が1日以上前であれば流速が遅いTLと見做してインターバルを通常より延ばす
      useInterval(async () => {
        paginator.fetchNewer({ toQueue: !isTop() || isPausingUpdate });
      }, POLLING_INTERVAL, {
        immediate: false,
        afterMounted: true
      });
    ...
    function connectChannel() {
      ...
      if (props.src === "antenna") {
        ...
        connections.antenna = stream.useChannel("antenna", { antennaId: props.antenna });
        ...
        connections.channel = stream.useChannel("channel", { channelId: props.channel });
        connections.channel.on("note", prepend);
      } else if (props.src === "role") {
        if (props.role == null) return;
        connections.roleTimeline = stream.useChannel("roleTimeline", { roleId: props.role });
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(_resolveDynamicComponent(_unref(prefer).s.enablePullToRefresh ? MkPullToRefresh : "div"), { refresher: () => reloadTimeline() }, {
        default: _withCtx(() => [_unref(paginator).fetching.value ? (_openBlock(), _createBlock(_component_MkLoading, { key: 0 })) : _unref(paginator).error.value ? (_openBlock(), _createBlock(_component_MkError, {
          key: 1,
          onRetry: _cache[0] || (_cache[0] = ($event) => _unref(paginator).init())
        })) : _unref(paginator).items.value.length === 0 ? (_openBlock(), _createElementBlock("div", { key: "_empty_" }, [_renderSlot(_ctx.$slots, "empty", {}, () => [_createVNode(_component_MkResult, {
          type: "empty",
          text: _unref(i18n).ts.noNotes
        }, null, 8, ["text"])])])) : (_openBlock(), _createElementBlock(
          "div",
          {
            key: 3,
            ref_key: "rootEl",
          ...
          [
            _unref(paginator).queuedAheadItemsCount.value > 0 ? (_openBlock(), _createElementBlock(
              "div",
              {
                key: 0,
              ...
              [
                ...
                _createElementVNode(
                  "button",
                  {
                    ...
                    onClick: _cache[1] || (_cache[1] = ($event) => releaseQueue())
                  },
                  [_hoisted_1, _createTextVNode(
                    " " + _toDisplayString(_unref(i18n).ts.newNote),
                    1
                    /* TEXT */
                  )],
                  2
            ...
            _createVNode(_resolveDynamicComponent(_unref(prefer).s.animation ? _unref(TransitionGroup) : "div"), {
              ...
              default: _withCtx(() => [(_openBlock(true), _createElementBlock(
                _Fragment,
                null,
                _renderList(_unref(paginator).items.value, (note, i) => {
                  return _openBlock(), _createElementBlock(
                    _Fragment,
                    { key: note.id },
                    [i > 0 && _unref(isSeparatorNeeded)(_unref(paginator).items.value[i - 1].createdAt, note.createdAt) ? (_openBlock(), _createElementBlock("div", {
                      key: 0,
                      ...
                      [
                        _createElementVNode("span", null, [_hoisted_2, _createTextVNode(
                          " " + _toDisplayString(_unref(getSeparatorInfo)(_unref(paginator).items.value[i - 1].createdAt, note.createdAt)?.prevText),
                          1
                          /* TEXT */
                        )]),
                        _hoisted_3,
                        _createElementVNode("span", null, [_createTextVNode(
                          _toDisplayString(_unref(getSeparatorInfo)(_unref(paginator).items.value[i - 1].createdAt, note.createdAt)?.nextText) + " ",
                          1
                          /* TEXT */
                        ), _hoisted_4])
                      ],
                      ...
                      note,
                      withHardMute: true
                    }, null, 10, ["note", "withHardMute"])], 8, ["data-scroll-anchor"])) : note._shouldInsertAd_ ? (_openBlock(), _createElementBlock("div", {
                      key: 1,
                      "data-scroll-anchor": note.id
                    }, [_createVNode(MkNote, {
                      class: _normalizeClass(_ctx.$style.note),
            ...
            _withDirectives(_createElementVNode("button", {
              ...
              onClick: _cache[2] || (_cache[2] = (...args) => _unref(paginator).fetchOlder && _unref(paginator).fetchOlder(...args))
            }, [!_unref(paginator).fetchingOlder.value ? (_openBlock(), _createElementBlock(
              "div",
              { key: 0 },
