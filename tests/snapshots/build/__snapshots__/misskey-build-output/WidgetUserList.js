...
export default {
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const fetch = () => {
      ...
      misskeyApi("users/lists/show", { listId: widgetProps.listId }).then((_list) => {
        list.value = _list;
        misskeyApi("users/show", { userIds: list.value.userIds ?? [] }).then((_users) => {
          users.value = _users;
          fetching.value = false;
        });
      });
    ...
    return (_ctx, _cache) => {
      ...
      return _openBlock(), _createBlock(MkContainer, {
        ...
        header: _withCtx(() => [_createTextVNode(
          _toDisplayString(list.value ? list.value.name : _unref(i18n).ts._widgets.userList),
          1
          /* TEXT */
        )]),
        ...
        default: _withCtx(() => [_createElementVNode(
          ...
          [_unref(widgetProps).listId == null ? (_openBlock(), _createElementBlock("div", {
            key: 0,
            class: "init"
            ...
            default: _withCtx(() => [_createTextVNode(
              _toDisplayString(_unref(i18n).ts._widgets._userList.chooseList),
              1
              /* TEXT */
            )]),
            _: 1
          })])) : fetching.value ? (_openBlock(), _createBlock(_component_MkLoading, { key: 1 })) : (_openBlock(), _createElementBlock("div", {
            key: 2,
            class: "users"
