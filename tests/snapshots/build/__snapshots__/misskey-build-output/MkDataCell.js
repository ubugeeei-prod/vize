...
export default {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const editingValue = ref(undefined);
    ...
    function onCellDoubleClick(ev) {
      switch (ev.type) {
        case "dblclick": {
          beginEditing(ev.target);
          break;
        }
      }
    }
    function onOutsideMouseDown(ev) {
      ...
      if (isOutside || !equalCellAddress(cell.value.address, getCellAddress(ev.target))) {
    ...
    function onInputText(ev) {
      editingValue.value = ev.target.value;
    }
    ...
    async function beginEditing(target) {
      ...
      if (cell.value.column.setting.customValueEditor) {
        ...
        const newValue = await cell.value.column.setting.customValueEditor(cell.value.row, cell.value.column, cell.value.value, target);
          ...
          case "text": {
            ...
            await nextTick(() => {
              ...
              if (inputAreaEl.value) {
                inputAreaEl.value.querySelector("*").focus();
              }
            });
            break;
          }
    ...
    function endEditing(applyValue, requireFocus) {
      if (!editing.value) {
        return;
      }
    ...
    function emitValueChange(newValue) {
    ...
    return (_ctx, _cache) => {
      return _unref(cell).row.using ? (_openBlock(), _createElementBlock("div", {
        key: 0,
        ref_key: "rootEl",
        ref: rootEl,
        class: _normalizeClass(["mk_grid_td", _ctx.$style.cell]),
        style: _normalizeStyle({
          maxWidth: cellWidth.value,
          minWidth: cellWidth.value
        }),
        tabindex: -1,
        "data-grid-cell": "",
        ...
        [!editing.value ? (_openBlock(), _createElementBlock(
          "div",
          {
            key: 0,
          ...
          [_createElementVNode(
            "div",
            {
              ref_key: "contentAreaEl",
              ref: contentAreaEl,
              class: _normalizeClass(_ctx.$style.content)
            },
            [
              cellType.value === "text" ? (_openBlock(), _createElementBlock(
                "div",
                { key: 0 },
              ...
              cellType.value === "number" ? (_openBlock(), _createElementBlock(
                "div",
                { key: 0 },
                _toDisplayString(_unref(cell).value),
                1
              ...
              cellType.value === "date" ? (_openBlock(), _createElementBlock(
                "div",
                { key: 0 },
                _toDisplayString(_unref(cell).value),
                1
                /* TEXT */
              )) : cellType.value === "boolean" ? (_openBlock(), _createElementBlock("div", { key: 1 }, [_createElementVNode(
                "div",
                ...
                2
                /* CLASS */
              )])) : cellType.value === "image" ? (_openBlock(), _createElementBlock("div", { key: 2 }, [_unref(cell).value && typeof _unref(cell).value === "string" ? (_openBlock(), _createElementBlock("img", {
                key: 0,
                src: _unref(cell).value,
                alt: _unref(cell).value,
                class: _normalizeClass(_ctx.$style.viewImage),
                onLoad: emitContentSizeChanged
              }, null, 42, ["src", "alt"])) : _createCommentVNode("v-if", true)])) : _createCommentVNode("v-if", true)
            ],
            2
            /* CLASS */
          )],
          ...
          [
            ...
            cellType.value === "number" ? (_openBlock(), _createElementBlock("input", {
              key: 0,
              type: "number",
              class: _normalizeClass(_ctx.$style.editingInput),
              value: editingValue.value,
            ...
            cellType.value === "date" ? (_openBlock(), _createElementBlock("input", {
              key: 0,
              type: "date",
              class: _normalizeClass(_ctx.$style.editingInput),
              value: editingValue.value,
