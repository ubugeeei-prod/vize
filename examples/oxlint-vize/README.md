...
This example uses the locally built Vize Oxlint plugin bundle and `@vizejs/native`. It supports Node 22 and Node 24+, while the repository's `package.json#devEngines.runtime` remains the default local runtime. The example scripts rebuild dependencies automatically. If you want to build them up front from the repository root:
...
```bash
vp exec node ../../npm/oxlint/dist/cli.mjs -c .oxlintrc.json -f stylish src
```
...
- The Vize Oxlint plugin is loaded from `../../npm/oxlint/dist/index.mjs`.
