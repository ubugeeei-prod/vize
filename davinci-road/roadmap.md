...
S2 DOM emit walks, but not the fused build-path switch. P2-11 has 120 landed
installments through [#5829](https://github.com/ubugeeei-prod/vize/pull/5829). Installments
84-120 open the production option surface the switch needs: module output,
...
and disabled static-hoist routing, HTML re-entry close casing and the explicit
legacy selection guard. The
...
predicates, unsupported option shapes and explicit legacy flag deletion.
