...
publishes the materialized owner-keyed row count. This is still not the P3-7
finish line: the hydrated Real Project Matrix run must complete green on the
branch and record non-zero `patch_fact_entries` before the DOM corpus gate is
closed.
...
P3-7 still needs terminal Real Project Matrix TS-11 evidence over the
