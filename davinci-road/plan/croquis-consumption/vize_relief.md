<!-- GENERATED FILE — do not edit by hand.
     Regenerate: rust-script tools/commands/davinci/croquis-consumers.rs --write
     Verify:     rust-script tools/commands/davinci/croquis-consumers.rs --check -->

# Croquis consumption: `vize_relief`

One shard of the [Croquis consumption matrix](../croquis-consumption.md): every fact the matrix records about `crates/vize_relief/src`. The method and the product set live on that page.

## Resolved product sites

_None._

## Non-product `vize_croquis` imports

_None._

## Naive grep disagreements (resolved/grep)

| product            | resolved | grep |
| ------------------ | -------: | ---: |
| `BindingMetadata`  |        0 |    6 |
| `Scope`            |        0 |    3 |
| `Span`             |        0 |    8 |
| `Symbol`           |        0 |    5 |
| `Croquis.bindings` |        0 |    1 |
| `Croquis.hoists`   |        0 |    1 |
