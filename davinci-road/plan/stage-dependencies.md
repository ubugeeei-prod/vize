...
remain Cargo package ids and release identities only until a dedicated
compatibility change can rename them safely.
...
the preferred crate name, so source imports stay on `vize_s0`, `vize_s1`,
`vize_s2`, and `vize_s1_to_s2` while package publication remains compatible.
