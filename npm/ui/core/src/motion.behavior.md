...
| M4  | packaged stylesheet | `data-vize-motion~="enter"` on first render     | `@starting-style` entry ships verbatim for engines newer than the floor                                                     | `ships the starting-style entry recipe verbatim`                    |
| M5  | packaged stylesheet | `prefers-reduced-motion: reduce`                | every duration and delay token zeroes and recipes hard-stop                                                                 | `zeroes packaged motion under reduced motion`                       |
