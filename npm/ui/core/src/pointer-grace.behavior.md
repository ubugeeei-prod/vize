...
row is exercised by `src/pointer-grace*.test.ts`; compile-only assertions live
in `src/pointer-grace.types.test-d.ts`.
...
| G8  | public types   | invalid delay or mutating readonly refs | compilation rejects misuse                        | `src/pointer-grace.types.test-d.ts` |
