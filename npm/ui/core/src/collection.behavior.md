...
| Connected item roots           | Relevant child, text, or accessible-label DOM mutation                                     | A scoped observer invalidates ordering and extracted text; `refresh()` provides the same deterministic hook when observers are unavailable      |
...
| Missing explicit `textValue`   | A rendered element is available                                                            | Text is extracted from `aria-labelledby`, `aria-label`, content, image alt text, or input value; hidden decorative descendants are excluded     |
