...
is exercised by `src/virtualizer*.test.ts`; compile-only assertions live in
`src/virtualizer.types.test-d.ts`.
...
| V21 | public types   | invalid alignment or mutating readonly refs | compilation rejects misuse                                   | `src/virtualizer.types.test-d.ts`                            |
