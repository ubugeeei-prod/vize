...
proven by `src/context.test.ts`; compile-only assertions live in
`src/context.types.test-d.ts`.
...
| CT7 | public type API    | incorrect key/value use   | compile-only assertions reject generic drift and key mutation                            | `src/context.types.test-d.ts`                          |
