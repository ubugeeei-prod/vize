...
Every row is proven by `src/controllable-state.test.ts`; compile-only assertions
live in `src/controllable-state.types.test-d.ts`.
...
| CS7 | public type API  | invalid updater/defaults | compile-only assertions preserve the value type through reads, sets, and reset | `src/controllable-state.types.test-d.ts`          |
