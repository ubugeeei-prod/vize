...
Normative state/input -> outcome table for `progress-bar.vue`
(`@vizejs/ui/progress`). Every row is proven by the named mounted-DOM test in
`src/progress.test.ts`; compile-only assertions live in
`src/progress.types.test-d.ts`.
...
| P8  | SSR/hydration | runtime fixture     | server markup is stable and hydrates without warnings or node replacement                                      | `runtime-conformance.test.ts`                                     |
| P9  | public types  | invalid state       | TypeScript rejects closed-contract misuse                                                                      | `src/progress.types.test-d.ts`                                    |
