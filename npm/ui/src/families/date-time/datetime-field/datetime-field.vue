...
import { useDateTimeField } from "./datetime-field-runtime.ts";
import {
  fieldValidationInputStyle,
  useFieldValidationInput,
} from "../date-field/field-segment-runtime.ts";
import type { DateTimeFieldSlotState } from "./datetime-field-types.ts";
...
const props = defineProps<{
  ...
  /** Hidden input name that submits `YYYY-MM-DDTHH:MM[:SS]` with forms. @default undefined */
  readonly name?: string | undefined;
  /** Controlled date-time; `undefined` selects uncontrolled mode and `null` clears. @default undefined */
  readonly modelValue?: PlainDateTime | null | undefined;
  /** Initial uncontrolled date-time and native form-reset target. @default null */
  readonly defaultValue?: PlainDateTime | null | undefined;
  /** Earliest valid date-time; earlier values mark the field invalid. @default undefined */
  readonly min?: PlainDateTime | null | undefined;
  /** Latest valid date-time; later values mark the field invalid. @default undefined */
  ...
  /** IANA time zone for the key-time host clock fallback. @default undefined */
  ...
  /** Date-time whose segments seed stepping from empty; defaults to the host clock at key time. @default undefined */
...
const { field, id, direction, invalid, isoValue, slotState, segmentId, exposed } = useDateTimeField(
  props,
  emit,
  root,
);
  ...
  >
    <template v-for="segment in slotState.segments" :key="segment.index">
      <span
        ...
        data-vize-ui="datetime-field-segment"
        part="segment"
      ...
      >
      <span v-else aria-hidden="true" data-vize-ui="datetime-field-literal" part="literal">{{
        segment.text
      }}</span>
    </template>
    <input
      ...
      :aria-labelledby="id"
      autocomplete="off"
      data-vize-ui="datetime-field-input"
