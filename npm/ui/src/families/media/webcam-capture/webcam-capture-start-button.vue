...
const disabled = computed(
  () =>
    context.slotState.value.external || status.value === "active" || status.value === "requesting",
);
const text = computed(() => context.messages.value.start);

function onClick(event: MouseEvent): void {
  emit("click", event);
  if (!event.defaultPrevented) void context.start();
}
