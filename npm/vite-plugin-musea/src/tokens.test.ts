...
void test("parseTokens merges token directories into canonical reference paths", async () => {
  ...
  await fs.promises.writeFile(
    path.join(tempDir, "components.tokens.json"),
    JSON.stringify({
      button: {
        primary: {
          background: {
  ...
  assert.equal(resolvedTokenMap["button.primary.background"]?.$tier, "component");
  assert.equal(resolvedTokenMap["button.primary.background"]?.$reference, "color.semantic.surface");
  assert.equal(resolvedTokenMap["button.primary.background"]?.$resolvedValue, "#f7f7f7");
