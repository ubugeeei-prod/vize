...
function applyReferenceToken(
  token: Omit<DesignToken, "$resolvedValue">,
  tokenMap: Record<string, DesignToken>,
  dotPath: string,
): { valid: true } | { valid: false; error: string } {
  if (token.$tier === "component" && !token.$reference) {
    return { valid: false, error: "Component tokens require a reference" };
  }
  ...
  const validation = validateSemanticReference(tokenMap, token.$reference, dotPath);
  if (!validation.valid) {
...
export async function handleTokensCreate(
  ...
  try {
    ...
    const referenceResult = applyReferenceToken(token, currentMap, dotPath);
    if (!referenceResult.valid) {
      sendError(referenceResult.error, 400);
      return;
    }
