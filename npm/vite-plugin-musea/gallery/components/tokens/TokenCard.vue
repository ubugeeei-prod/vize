...
const displayValue = computed(() => {
  if (
    (props.token.$tier === "semantic" || props.token.$tier === "component") &&
    props.token.$resolvedValue !== undefined
  ) {
...
<template>
  <div
    class="token-card"
    :class="{
      'token-card--semantic': token.$tier === 'semantic',
      'token-card--component': token.$tier === 'component',
    }"
  >
    <!-- Preview -->
