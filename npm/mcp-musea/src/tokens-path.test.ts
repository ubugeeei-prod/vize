...
test("resolveConfiguredTokensPath accepts an in-project tokens directory", async () => {
  ...
  try {
    ...
    fs.mkdirSync(tokens);
    fs.writeFileSync(path.join(tokens, "color.json"), '{"color":{"brand":{"value":"#111"}}}');

    assert.equal(await resolveConfiguredTokensPath(root), tokens);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test("resolveConfiguredTokensPath rejects a tokens symlink that leaves the project", async () => {
