/** Differential runner for @nuxt/eslint-plugin's no-nuxt-config-test-key rule. */
...
function recordConfigTestKeyMessage(source, message) {
  return {
...
export async function recordNoNuxtConfigTestKeyCases(moduleEntry, corpus, packageVersionFrom) {
  ...
  for (const entry of corpus.noNuxtConfigTestKeyCases) {
    const messages = linter
      ...
      .map((message) => recordConfigTestKeyMessage(entry.source, message));
    ...
    const secondPassMessages = secondPass.messages.map((message) =>
      recordConfigTestKeyMessage(secondPass.output, message),
    );
    recorded[entry.id] = {
      messages,
