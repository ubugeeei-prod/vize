 ...
 *   See `bench/scale/configs.mjs` — the two configs differ only in the plugin.
 ...
 *   nix devshell, so `bench/scale/measure.mjs` implements warmup + N runs +
 ...
 *   generated local `node_modules/@bench/ui` package. Reasoning in
 *   `bench/scale/corpus.mjs`.
 ...
 *
 * Usage:
 *   node bench/scale.mjs                       # 1000,3000,5000,10000, 3 runs
 *   node bench/scale.mjs --scales 1000 --runs 5
 *   node bench/scale.mjs --scales 10000 --cold
 *   node bench/scale.mjs --generate-only --scales 10000
 *   node bench/scale.mjs --json bench/results/scale.json
 */
...
const VIZE_PLUGIN_ENTRY = join(__dirname, "..", "npm", "builder", "vite", "dist", "index.mjs");
