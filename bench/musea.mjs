 ...
 *
 *   node bench/musea.mjs
 *   node bench/musea.mjs --files 480 --runs 7 --warmups 2
 *
 ...
 *
 * Reproducibility: the corpus is generated from `bench/musea-corpus.mjs` at a
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else throw new Error(`bench/musea.mjs: unknown argument ${argv[i]}`);
  }
  return options;
}
...
async function observableDigest(stage, output) {
  ...
  if (typeof digest !== "string" || digest.length === 0) {
    throw new Error(
      `bench/musea.mjs: ${stage.id} produced no deterministic digest; every measured stage must prove equivalent output`,
    );
  }
  return digest;
}
 ...
 * alternates, matching `measureVariants` in bench/compare-tools.mjs so a stage
...
export async function runMuseaStages(stages, { runs, warmups }) {
  ...
  const verifyDigest = async (stage, output) => {
    ...
    if (seen === undefined) {
      ...
      throw new Error(
        `bench/musea.mjs: ${stage.id} produced different output between passes (${seen} -> ${digest}); refusing to report a median over two workloads`,
      );
    }
  };
    ...
    subprocess.once("exit", (code, signal) => {
      if (code !== 0) {
        reject(
          new Error(
            stderr.trim() ||
              `bench/musea.mjs: isolated worker exited with ${signal ?? `status ${code}`}`,
          ),
        );
        return;
      }
      try {
        ...
        reject(
          new Error(
            `bench/musea.mjs: isolated worker returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`,
          ),
        );
      }
    });
...
export function renderMuseaReport(data, { runs, warmups }) {
  ...
  lines.push(`Work directory: \`${data.workDir}\` (fixed path, see bench/musea.mjs).`);
  lines.push("");
