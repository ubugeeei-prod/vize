...
></div>"#
  ...
  {{
    ...
    #[test]
    fn test_nested_multiline_template_literal_interpolation_is_idempotent() {
        // The shadcn-vue `ChartStyle` shape: an interpolation whose expression
        // builds CSS with an outer multiline template literal that itself
        // contains a multiline `${ … }` holding a nested template literal.
        // oxc canonicalizes the `${ … }` code each pass while the quasi text
        // stays verbatim, so re-indenting only non-quasi lines keeps the whole
        // thing a fixed point. (#3247)
        let source = concat!(
        ...
        let options = FormatOptions::default();
        let first = format_template_content(source, &options).unwrap();
        let second = format_template_content(&first, &options).unwrap();
        assert_eq!(
            first, second,
            "nested multiline template literal must be idempotent;\nfirst:\n{first}\nsecond:\n{second}"
        );
        // The outer quasi keeps its column-0 CSS text verbatim.
        assert!(
            first.contains("\n${prefix} [data-chart=${id}] {\n"),
    ...
    fn test_textarea_whitespace_only_content_preserved() {
        ...
        // and must survive formatting; the empty-element close collapse used
        ...
        let again = format_template_content(&result, &options).unwrap();
    ...
    #[test]
    fn test_multiline_template_literal_in_interpolation_is_idempotent() {
        // A multiline template literal inside an interpolation carries
        // semantically significant string content. Re-indenting its inner
        // lines corrupts the value and, because each pass prepends indentation
        // again, is not a fixed point. The quasi lines must be preserved
        // verbatim. (#3247)
        let source = concat!(
            "<div>\n",
            "  {{\n",
        ...
        // The quasi lines keep their original column-0 indentation instead of
        // being pushed right by the interpolation indent.
        assert!(
            first.contains("\n${p} [data-chart] {\n  color: red;\n}\n"),
