//! `_createSlots` construction and per-slot entry emission.
//!
//! Emits the dynamic-slots path (`_createSlots` with conditional/looped slot
//! branches) and the individual slot object entries. Split out of
//! `slots/generate` to keep that file focused on the top-level slots object.
...
use super::detect::{child_is_slot_template, slot_children_have_meaningful_content};
