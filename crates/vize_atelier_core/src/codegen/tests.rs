...
fn result_output(result: &super::CodegenResult) -> vize_l0::String {
    let mut output = vize_l0::String::with_capacity(result.preamble.len() + result.code.len() + 1);
...
fn test_codegen_inline_setup_ref_component_prop_uses_value() {
    ...
    let mut bindings = vize_l0::FxHashMap::default();
    ...
    let binding_metadata = crate::BindingMetadata {
        bindings,
        props_aliases: vize_l0::FxHashMap::default(),
...
fn compile_prefixed(source: &str) -> vize_l0::String {
    let allocator = vize_l0::Allocator::new();
