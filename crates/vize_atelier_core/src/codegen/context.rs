...
impl CodegenContext {
    ...
    /// originating AST node (its `loc.span.start`). Line/column are computed
    /// from both offsets at map-assembly time. No-op unless the `source_map`
    /// flag is on, so call sites can invoke it unconditionally right before
    /// pushing the mapped text without affecting the generated `code`.
    #[inline]
    pub(super) fn record_mapping(&mut self, source_offset: u32) {
        if let Some(builder) = self.map_builder.as_mut() {
            let generated_offset = self.code.len();
            builder.add_raw(generated_offset, source_offset);
        }
    }
    ...
    #[inline]
    pub(super) fn record_mapping_named(&mut self, source_offset: u32, name: &str) {
        if let Some(builder) = self.map_builder.as_mut() {
            let generated_offset = self.code.len();
            builder.add_named(generated_offset, source_offset, name);
        }
    }
