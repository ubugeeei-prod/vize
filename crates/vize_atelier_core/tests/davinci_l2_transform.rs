...
fn the_v_slot_wording_never_drifts_from_relief() {
    // The series-3 messages, same discipline: the L2 pass duplicates
...
fn both_lanes_flag_the_duplicate_slot_name() {
    ...
    // legacy transform reports `VSlotDuplicateSlotNames`, the L2 pass
    ...
    assert!(
        lowered
            ...
            .any(|diagnostic| diagnostic.message.as_str()
                == vize_l1_to_l2::pass::vslot::DUPLICATE_MESSAGE),
        "the L2 pass must flag the duplicate: {:?}",
        lowered.diagnostics
    );
}
...
fn the_v_model_wording_never_drifts_from_relief() {
    // The series-5 messages, same discipline: the L2 pass duplicates
    // relief's text rather than the dependency; these pins own the
    // relief edge.
    use vize_atelier_core::ErrorCode;
    use vize_l1_to_l2::pass::vmodel;
...
fn both_lanes_flag_the_scoped_model() {
    ...
    // the legacy transform reports `VModelOnScope`, the L2 pass appends
    ...
    assert!(
        lowered
            ...
            .iter()
            .any(|diagnostic| diagnostic.message.as_str()
                == vize_l1_to_l2::pass::vmodel::ON_SCOPE_MESSAGE),
        "the L2 pass must flag the scoped model: {:?}",
        lowered.diagnostics
    );
}
...
fn both_lanes_flag_the_duplicate_key() {
    ...
    // legacy transform reports `VIfSameKey`, the L2 lowering appends
    // the same text to the unified channel.
    let (_, source) = BATTERY
        .iter()
    ...
    assert!(
        lowered
            ...
            .any(|diagnostic| diagnostic.message.as_str()
                == vize_l1_to_l2::pass::vif::SAME_KEY_MESSAGE),
        "the L2 lowering must flag the collision: {:?}",
        lowered.diagnostics
    );
}
