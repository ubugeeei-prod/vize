...
pub enum OldKey {
    ...
    /// quirk L2 mirrors on ordinary carriers and still counts for
...
pub struct OldBranch {
    ...
    /// L2 attribute surface, so key comparison is counted instead).
...
/// nested, document order — the same order the L2 projection walks.
pub fn collect(
