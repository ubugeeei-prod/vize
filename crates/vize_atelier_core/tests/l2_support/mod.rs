//! The P2-9 differential comparator: legacy transform lane vs the L2
...
//! from L2 (P2-11); until then this projection is the strongest
...
//! one. Dev-dependencies are stripped on publish, so the L2 lane and
...
//! path has no migrated read yet: the L2 lane runs *beside* the legacy
//! lane, not inside it.
//!
//! # Skip classes are counted, never silent
//!
//! The two lanes parse with different L1 front ends, and the L1 v1
...
//! legacy hard parse errors, L2 structural error diagnostics (evaluated
...
pub struct Counters {
    ...
    /// The L2 lowering reported an `Error` diagnostic (pre-pass).
    pub skipped_l2_errors: u64,
    /// `ui.if` ops compared.
    pub if_ops: u64,
    /// Branches compared.
    pub branches: u64,
    ...
    /// legacy lane lifts as the branch key; L2 mirrors ordinary branch
