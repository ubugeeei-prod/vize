...
//! (`crates/vize_atelier_core/src/codegen/children.rs`); L2 does both
//! at lowering (`crates/vize_l1_to_l2/src/lower/text.rs`), so its
...
//! them ([`super::text_old`]), L2 ops one-to-one — and compare the
//! parts exactly: kind, then text (the legacy condensed content against
//! the L2 condensed content; the trimmed expression on both sides).
//!
//! # Counted classes, never silence
//!
//! - `vpre_templates` — **retired**, and kept at zero as the proof. It
//!   counted templates skipped because L2 lowered a `v-pre` subtree as
//!   ordinary content (`defer.v-pre`, the recorded P2-8 deferral) while
//!   the legacy parser froze it. The installment that owed `v-pre` an L2
...
//!   and interpolation content; L1 v1 deliberately does not (the
//!   recorded deviation). A template whose L2 parts carry
...
//!   projections exclude those subtrees, and the L2 side counts the
...
//!   authored pipe (`msg | cap`) and whose L2 text is the legalize wrap
//!   (`_filter_cap(msg)`). The comparator's legacy transform does not
//!   run `process_expression` (`prefix_identifiers` off), so it reports
//!   the pre-legalize spelling; L2 `legacy-sugar` always wraps. They
//!   agree when wrap(authored) equals the L2 text — one side was
...
use vize_l2::expr::VueFilterExpr;
...
pub struct TextCounters {
    ...
    /// L2 units backed by a merged `opaque(compound)` op.
    pub compound_units: u64,
    /// Templates skipped: the legacy parse handles `v-pre`, L2 defers it.
    pub vpre_templates: u64,
    /// Templates skipped: entity-shaped text under the L1 no-decoding
    /// scope.
    pub entity_templates: u64,
    /// L2 text-family ops inside rawtext content, excluded on both
    /// sides.
    ...
    /// legacy authored pipe wrap-equals the L2 `_filter_*` text.
...
pub struct TUnit {
    pub parts: Vec<TPart>,
    /// L2 side: the unit is a merged compound op.
    pub compound: bool,
}

/// Entity-shaped text under the L1 no-decoding scope: `&` followed by
...
pub fn check(name: &str, source: &str, old: &[TUnit], s2: &[TUnit], counters: &mut TextCounters) {
    // The entity class is a template-level predicate on the L2 parts
    ...
    for (index, (old_unit, l2_unit)) in old.iter().zip(s2).enumerate() {
        if old_unit.parts.len() != l2_unit.parts.len() {
            diverged!(
        ...
        for (old_part, l2_part) in old_unit.parts.iter().zip(&l2_unit.parts) {
            if old_part.dynamic != l2_part.dynamic {
                diverged!("unit {index} part kind {old_part:?} vs {l2_part:?}");
            }
            match (&old_part.text, &l2_part.text) {
                (None, Some(_)) => counters.parts_compound += 1,
                (Some(old_text), Some(l2_text)) if old_text == l2_text => {
                    if old_part.dynamic {
                ...
                (Some(old_text), Some(l2_text))
                    if old_part.dynamic
                        && legalized_filter(old_text).as_deref() == Some(l2_text.as_str()) =>
                {
                    counters.parts_dynamic += 1;
                    counters.parts_filter += 1;
                }
                _ => diverged!("unit {index} part text {old_part:?} vs {l2_part:?}"),
            }
        }
        if l2_unit.compound {
...
/// Replay of `vize_l1_to_l2::pass::legacy::filter` wrap: `a | f` →
/// `_filter_f(a)`, `a | f(b)` → `_filter_f(a,b)`. Admission is
/// [`VueFilterExpr::parse_in`] — the same splitter L2 lowering uses.
