//! The L2 half of the surface projection ([`super::surface`]): one
//! owner's attribute and binding lists into the shared owner shape,
//! split from [`super::l2_lane`] under the source budget. Binding ids
...
/// as a model (span-shared bind/on). After legalize, L2 has the same
...
fn fold_sync_products(surface: &mut PSurface) {
    ...
    while bind_index < surface.binds.len() {
        ...
        surface.models.push(PModel {
            ...
            // bind the L2 legalize does not emit). Empty matches that.
            mods: Vec::new(),
            component: true,
        });
    }
