...
//! `TransformOptions.dialect`, and L2 via [`LegacyCaps`] at lower
...
pub fn compare_with(name: &str, source: &str, counters: &mut Counters, dialect: VueVersion) {
    ...
    // rule and the run grouping — and the L2 side lowers with
    // `preserve_comments = false`, the shipped `DomCompilerOptions`
    // default. Comparing a comment-bearing legacy tree against a
    // comment-free L2 one made the lanes disagree by construction on
    ...
    // `site/src/layouts/header/Navigation.vue` carries L2 facts
    ...
    // L2 lane: L1 parse -> L1-to-L2 lower -> the L2 passes through
    ...
    let mut lowered = vize_l1_to_l2::lower_with_caps(
        &l2_allocator,
        &tree,
        &surface_errors,
        LegacyCaps::for_version(dialect),
    );
    if lowered.diagnostics.iter().any(blocks_l2_comparison) {
        counters.skipped_l2_errors += 1;
        return;
    }
    ...
    // its tree, so the deterministic detector is the L2 lowering's own
    // deferral record.
    ...
    // actual mutations against the L2 facts' predictions. Template-
    ...
    if has_vpre {
        ...
        let mut old_shape = vize_l0::String::default();
        hoist_old::shape_of(&root.children, &mut old_shape);
        let mut l2_shape = vize_l0::String::default();
        hoist::shape_of_l2(&folio.ops, &mut l2_shape);
        if scan.classifier {
            counters.hoist.classifier_templates += 1;
        } else if scan.consts {
            counters.hoist.consts_templates += 1;
        } else if old_shape != l2_shape {
