...
/// The names the L1→L2 classifier treats as built-in directives — the
/// exclusion set of the custom-directive kind (mirrors
/// `vize_l1_to_l2::lower::directive::classify`).
...
/// The still-deferred set (plus structural residue the L2 lowering drops
...
pub fn classifier_ambiguous(el: &ElementNode<'_>) -> bool {
    ...
    let native = vize_l0::is_native_tag(el.tag);
    let legacy_component = el.tag == "component"
        || el.tag.chars().next().is_some_and(char::is_uppercase)
        || el.tag.contains('-');
    // The L2 classifier is `!is_native_tag`; disagreement is exactly a
