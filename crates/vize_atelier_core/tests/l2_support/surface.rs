...
//! wrapper elements the L2 lowering unwraps (`<template v-if>` branch
...
//! position is excluded by the L2 lowering's own selection rule,
//! mirrored on the legacy side; an empty `:name` is the recorded
//! `drop.slot-name-hole` agreement.
//!
//! # Counted classes, never silence
//!
//! - `builtins_excluded` — legacy props whose directive L2 still defers
//!   (`v-pre`) or drops as a structural duplicate, plus the
//!   codegen-only dialect flags (`v-once`, `v-memo`, `v-show`,
//!   `v-html`, `v-text`, `v-cloak`) that lower as Vue dialect bindings
...
//!   attribute and directive values; L1 v1 deliberately does not (the
...
//!   (foster parenting, implicit sections) against L1's authored
...
//!   names, L2 deliberately does not until the #4365 seam — the
...
pub struct SurfaceCounters {
    ...
    pub models: u64,
    /// Both lanes dropped the model: the L2 fault fact met the legacy
    /// removal.
    ...
    /// Legacy props L2 defers or drops (the remaining `defer.*` set).
    ...
    /// that L1 v1 deliberately does not, so the two trees can genuinely
    /// disagree on owner order and count inside table subtrees; counted
    /// per template, never averaged, owned by the future L1 tree-
