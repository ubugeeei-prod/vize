...
use vize_l0::{Allocator, Span, camelize, is_native_tag};
use vize_l1_to_l2::pass::hoist::constant_for_hoist;
...
/// A directive the L2 lowering still defers (`defer.*`), so the L2
/// facts cannot see it.
...
/// the nested-child class) across it, and L2 carries no comment ops.
...
pub struct TemplateScan {
    ...
    /// L2 component: a non-native tag the legacy default options leave
    ...
    /// constant and the L2 rule (deliberately weaker) does not — the
...
/// classifier and the L2 rule disagree. The shape gates mirror
/// `hoistable_static_bind_parts`; the two classifiers then run on the
/// same authored text — the shipped one directly (the exported
/// oracle), the L2 one through the same admission rule the lowering
...
fn const_rule_diverges(prop: &PropNode<'_>) -> bool {
    ...
    let key = if has_camel {
        ...
        vize_l0::String::from(arg.content)
    };
    ...
    legacy_constant(exp) != l2_constant(exp.content)
}

fn prefixed(prefix: char, name: &str) -> vize_l0::String {
    let mut out = vize_l0::String::with_capacity(1 + name.len());
    out.push(prefix);
    out.push_str(name);
    out
}
...
/// The L2 verdict, through the one shared admission rule.
fn l2_constant(content: &str) -> bool {
...
/// the L2 lowering unwraps them. Compared byte-for-byte against
/// [`super::hoist::shape_of_l2`]; a mismatch is the L1-scope
/// tree-construction class (`tree_templates`), counted, never walked.
pub fn shape_of(children: &[TemplateChildNode<'_>], out: &mut vize_l0::String) {
    for child in children {
        match child {
            TemplateChildNode::Element(el) => {
