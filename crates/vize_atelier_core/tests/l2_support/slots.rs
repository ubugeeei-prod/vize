...
//! (`vize_l0::is_native_tag`, computed from the authored tag on
...
//! child, realized by legacy dynamic-slot codegen and dropped at L2
...
pub struct SlotCounters {
    ...
    /// path; L2 carries it as `vue.directive`) — counted, unit still
    /// compared.
...
pub struct PUnit {
    ...
    /// Legacy-side counted classes (always `false` on the L2 side).
...
/// Project one L2 unit from the pass's published facts.
pub fn l2_unit(id: Option<NodeId>, facts: &SideTable<SlotFacts>) -> PUnit {
    let entry = id
    ...
    let groups = entry
        ...
        .map(|group| PGroup {
            ...
            invented: matches!(
                &group.name,
                SlotName::Static {
                    origin: vize_l2::scope::ScopeOrigin::Synthesized { .. },
                    ..
                }
            ),
...
pub fn check(
    ...
    if old_units.len() != l2_units.len() {
        diverged(
            name,
            source,
            &old_units,
            &l2_units,
            format_args!("unit count {} vs {}", old_units.len(), l2_units.len()),
        );
    }
    for (old, s2) in old_units.iter().zip(l2_units) {
        counters.units += 1;
        ...
        for (old_group, l2_group) in old.groups.iter().zip(&s2.groups) {
            if old_group != l2_group {
                diverged(
                    name,
                    source,
                    old,
                    s2,
                    format_args!("group {old_group:?} vs {l2_group:?}"),
                );
            }
            counters.groups += 1;
    ...
    if old_outlets.len() != l2_outlets.len() {
        diverged(
            name,
            source,
            &old_outlets,
            &l2_outlets,
            format_args!("outlet count {} vs {}", old_outlets.len(), l2_outlets.len()),
        );
    }
    for (old, s2) in old_outlets.iter().zip(l2_outlets) {
        if old != s2 {
            diverged(
                name,
                source,
