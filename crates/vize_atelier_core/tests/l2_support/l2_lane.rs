//! The L2-lane projection: the DOM-output-determining facts of every
...
use super::surface_l2::{opens_pattern_scope, surface_of};
...
/// Everything the L2 lane projects out of one artifact.
pub struct L2Projection {
    pub chains: Vec<L2Chain>,
    pub fors: Vec<L2For>,
    pub units: Vec<PUnit>,
        ...
        match op {
            ...
            FolioOp::Component(component) => {
                ...
                *next += 1 + u32::try_from(component.bindings.len()).expect("binding count fits");
                if l2_slot_active(&component.bindings, &component.children) {
                    out.units.push(l2_unit(id, tables.slot_facts));
                }
                let surface = surface_of(
            ...
            FolioOp::If(if_op) => {
                ...
                *next += 1;
                ...
                out.chains.push(L2Chain {
                    branches: if_op
                        .branches
                        .iter()
                        .enumerate()
                        .map(|(index, branch)| L2Branch {
                            ...
                            key: fact
                                ...
                                .map(|key| match &key.kind {
                                    BranchKeyKind::Static(value) => L2Key::Static(
                                        value.as_ref().map(|value| String::from(value.as_str())),
                                    ),
                                    BranchKeyKind::Dynamic { source, .. } => {
                                        L2Key::Dynamic(String::from(source.as_str()))
                                    }
                                }),
                        })
                        .collect(),
