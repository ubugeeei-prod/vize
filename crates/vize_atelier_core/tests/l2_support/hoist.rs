...
//! predictions of the L2 analysis facts (`StaticFacts`) driven through
...
//! as a verdict mismatch at one aligned position. The L2 facts are
...
//! `table_templates`, `models_templates` (legacy removes a model L2
...
pub struct HoistCounters {
    ...
    /// props-hoisted (L2 keeps no wrapper position; counted, never
    /// compared).
    ...
    /// The shipped const classifier admitted a value the L2 rule
    ...
    /// The two L1 trees nest differently (module docs).
