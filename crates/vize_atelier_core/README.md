...
It is not the owner of the whole compiler foundation. `vize_atlas` owns the
neutral request ledger, `vize_relief` owns source syntax, `vize_croquis` owns
derived semantics, and `vize_rendu` owns the borrowed render projection.
...
- `vize_atlas` records requested compiler products and fallback routes
- `vize_relief` owns syntax; `vize_croquis` derives semantic relationships
- `vize_rendu` exposes output-facing render operations
