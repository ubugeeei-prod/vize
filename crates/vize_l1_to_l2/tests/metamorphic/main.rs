...
//! whitespace-insignificant edits) must leave the L2 folio identical
...
fn the_matrix_plane_is_metamorphically_stable() {
    ...
    assert_eq!(report.failures, Vec::<vize_l0::String>::new());
    report
...
fn the_normalization_does_not_collapse_real_differences() {
    ...
    let folio = |source: &str| {
        ...
        vize_l2::folio::DisegnoFolio::of(&lowered.root.ops)
    };
...
fn the_merge_mutator_holds_through_its_own_site_path() {
    ...
    let lowered = vize_l1_to_l2::lower(&allocator, &tree, &errors);
    let merged = vize_l2::folio::DisegnoFolio::of(&lowered.root.ops);
