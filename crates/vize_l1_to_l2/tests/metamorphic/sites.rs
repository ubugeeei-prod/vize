...
//!
//! One immutable walk over a parsed L1 tree yields every candidate
...
use vize_l1::{Element, SurfaceChild, SurfaceTree};
...
pub struct Site {
    pub kind: Kind,
    /// Child-index path from the L1 root to the carrying node.
    pub path: Vec<usize>,
    pub detail: Detail,
        ...
        match child {
            SurfaceChild::Element(element) => {
                ...
                let child_parent = Parent {
                    component: !vize_l0::is_native_tag(tag),
