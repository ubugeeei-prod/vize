...
//! locals it binds (`(v) => v.id`). The L2 hoist pass now admits that same
...
fn l2(source: &str, inline: bool) -> Result<String, EmitError> {
    l2_with_ts(source, inline, false)
}

fn l2_with_ts(source: &str, inline: bool, is_ts: bool) -> Result<String, EmitError> {
...
fn typed_component_callback_props_match_shipped_hoists() {
    ...
    for source in cases {
        ...
        let emitted = l2_with_ts(source, true, true)
...
fn typed_parameter_and_local_callback_props_match_shipped_hoists() {
    ...
    let legacy = shipped_with_ts(source, true, true);
    let emitted = l2_with_ts(source, true, true).expect("L2 emit");
    assert_eq!(emitted, legacy, "{source:?}");
}
...
fn conditional_named_slot_outlet_props_match_shipped_hoists() {
    ...
    let emitted = l2(source, true).unwrap_or_else(|error| panic!("{source:?}: {error:?}"));
    assert_eq!(emitted, legacy, "{source:?}");
}

fn refused(source: &str) -> Reason {
    match l2(source, true) {
...
fn inline_root_surfaces_the_shipped_lane_hoists_match() {
    ...
    for (source, expected) in cases {
        ...
        let emitted =
            l2(source, true).unwrap_or_else(|error| panic!("{source:?} must emit, got {error:?}"));
        assert_eq!(emitted, legacy, "{source:?}");
    }
