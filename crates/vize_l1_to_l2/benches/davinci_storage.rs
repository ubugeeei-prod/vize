//! Allocation regression probes for L1-to-L2 lowering and L2 DOM emit.
//!
...
//! cases account only L2 DOM emission, including modifier classification and
...
fn davinci_storage(criterion: &mut Criterion) {
    ...
    bench_stage_with_metrics(
        ...
        |window| {
            ...
            let facts = vize_l1_to_l2::pass::run_transform(&mut lowered, &mut NoObserver);
            window.measure(|| {
...
fn davinci_complexity(criterion: &mut Criterion) {
    ...
    bench_stage_with_metrics(
        ...
        |window| {
            ...
            let lowered = lower(&allocator, &tree, &errors);
            window.measure(|| {
                let facts = vize_l1_to_l2::pass::cfg::run(&lowered);
