//! The lowering entry: L1 surface tree in, L2 artifact out — **total,
//! no rollback** (the MLIR import).
//!
//! [`lower`] never panics and never abandons partial work: every L1
//! construct either becomes an op of the existing L2 family or leaves a
...
pub struct Lowered<'a> {
    ...
    /// content included (`vize_l2::provenance`).
    ...
    /// Lowering-observed feature bits used by the L2 pass planner.
...
/// Lower a parsed L1 tree (and the tokenizer errors its parse reported)
/// into L2.
///
/// Total over arbitrary input: malformed source arrives as typed L1
...
/// Lower a parsed L1 source block while preserving file-absolute L0 spans.
#[must_use]
...
fn lower_source_block_with_caps_and_comment_policy<'a>(
    ...
    debug_assert!(
        ...
        "the source block must be the exact string parsed into the L1 tree"
    );
...
///
/// [`vize_l1::parse`] refuses such sources, so a tree reaching here was
