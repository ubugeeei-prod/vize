...
//! transform produced any root hoist. L2 realizes hoists directly during emit,
...
use crate::pass::{L2Facts, StaticFacts, StaticLevel};
...
fn branch_roots_have_legacy_hoist(
    ...
    for op in ops {
        ...
        match op {
            Op::Element(element) => {
                walk.skip(element.bindings.len());
                // A conditional named-slot template keeps its carrier in L2.
