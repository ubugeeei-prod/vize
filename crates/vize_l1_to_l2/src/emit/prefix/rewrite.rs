...
use vize_l0::expression_guard::{expression_exceeds_max_depth, expression_has_balanced_delimiters};
...
/// codegen compatibility while the L2 selector validates the whole pattern.
...
pub(super) fn with_prefix_parse<T>(
    ...
    if !vize_l0::expression_guard::expression_is_safe_to_parse(content) {
        return None;
    }
    let allocator = Allocator::new();
...
pub(super) fn with_whole_expression<T>(
    ...
    let allocator = Allocator::new();
    let wrapped = vize_l0::cstr!("({content}\n)");
