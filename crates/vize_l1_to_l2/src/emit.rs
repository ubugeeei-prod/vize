//! L2 → DOM render-function emission (P2-11).
//!
//! The published home for the DOM backend. `vize_atelier_dom` calls this
//! module for normal DOM emission; it writes JS directly from L2 ops, without
...
/// same arithmetic the L2 passes use so compound text facts resolve.
struct EmitCx<'facts> {
    buf: Buf,
    source: &'facts str,
    facts: &'facts L2Facts,
