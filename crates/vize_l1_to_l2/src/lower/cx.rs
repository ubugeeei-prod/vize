...
//! # Ids are page order
//!
//! L2 node ids are dense and page-ordered — every op line top to bottom,
//! attached bindings between their owner's line and its children
//! (`folio-format.md`, "Node numbering"; `vize_l2::verify` checks
...
//! law is `Cx::op_count == L2Folio::of(root).op_count()`, tested per
//! fixture.
//!
...
impl<'a> Cx<'a> {
    ...
    pub(crate) fn attach_texts(
        ...
        if let Some(id) = node {
            ...
            let before = vize_l0::cstr!("parts={}", parts.parts.len());
    ...
    /// Byte offset of `slice` inside the authored source. Every L1 string
    /// is a slice of the one source (`vize_l1`'s P1-10 contract), so
    ...
    pub(crate) fn offset(&self, slice: &str) -> u32 {
        match self.block.offset_of(slice) {
            ...
            None => {
                debug_assert!(
                    false,
                    "L1 handed the lowering a string that is not a block slice"
                );
                self.block.start()
            }
        }
    ...
    /// Render an L1 typed hole into the unified channel. The tokenizer
