//! Span recovery over the L1 surface — the extent questions the lowering
//! asks about an authored node, kept beside [`Cx`](super::Cx) rather than
//! inside it.
//!
//! L1 is a contiguous partition of the source, so every extent here is
...
use vize_l0::Span;
use vize_l1::{CloseTag, Element, ElementClose, SurfaceChild};

use super::Cx;

/// End offset of an element's extent: the last byte its subtree rendered.
/// L1 is a contiguous partition of the source, so this is the end of the
...
pub(crate) fn attr_span(cx: &Cx<'_>, attr: &vize_l1::Attribute<'_>) -> Span {
...
pub(crate) fn attr_slice<'a>(cx: &Cx<'a>, attr: &vize_l1::Attribute<'a>) -> &'a str {
