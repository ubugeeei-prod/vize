//! pug L1 → L2 (Davinci P4-12c, charter #12).
//!
//! # The shape: desugar at L1, one Vue lowering
//!
...
//! that template into the Vue L1 surface and runs the one Vue lowering
//! ([`crate::lower`]), so pug and HTML templates share every L2 lane —
//! DOM, Vapor and SSR compile, Patina, the L2 passes — with no second
...
//!
//! L2 spans stay measured against the derived template; consumers map a
...
pub fn derive_template_with(
    ...
    // A source L1 could not address was not parsed: refuse it before the
...
/// A pug template lowered to L2 through its derived Vue template.
...
/// Lower a pug tree to L2. Total: any input yields ops and/or diagnostics.
#[must_use]
pub fn lower_pug<'a>(
    ...
    let (surface, surface_errors) = vize_l1::parse(allocator, html);
