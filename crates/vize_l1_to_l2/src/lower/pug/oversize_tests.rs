...
use vize_l1::pug::{PugError, PugErrorCode, parse_pug};

use super::{PugRendering, derive_template_with};

/// An L1 tree reporting `SourceTooLarge` derives no template: one error
/// diagnostic, empty HTML, no emitted nodes. (A 4 GiB input is impractical,
/// so the L1 error is supplied beside a small tree.)
#[test]
