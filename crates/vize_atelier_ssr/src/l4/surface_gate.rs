...
pub(super) fn slot_v_pre_interpolates(source: &str, ops: &[vize_l2::op::Op<'_>]) -> bool {
    ops.iter().any(|op| match op {
        ...
        vize_l2::op::Op::Element(element) => slot_v_pre_interpolates(source, &element.children.ops),
...
fn has_interpolation(ops: &[vize_l2::op::Op<'_>]) -> bool {
    ops.iter().any(|op| match op {
        vize_l2::op::Op::Interpolation(_) => true,
        vize_l2::op::Op::Element(element) => has_interpolation(&element.children.ops),
        vize_l2::op::Op::If(if_op) => if_op
            .branches
            .iter()
