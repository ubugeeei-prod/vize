...
pub(super) fn needs_merged(attached: &Attached<'_, '_>) -> bool {
    attached.iter().any(|segment| match segment.source {
        ...
        Source::Binding(l2::BindingOp::VueDirective(_)) => true,
        _ => false,
    })
}

fn has_binding(attached: &Attached<'_, '_>, test: impl Fn(&l2::BindingOp<'_>) -> bool) -> bool {
    attached.iter().any(|segment| match segment.source {
...
impl Emitter<'_, '_, '_, '_, '_, '_> {
    ...
    pub(super) fn merged_attrs(
        ...
        let content_override = has_binding(attached, |binding| {
            matches!(
                binding,
                l2::BindingOp::VueHtml(_) | l2::BindingOp::VueText(_)
            )
        });
    ...
    fn merged_props_args(
        ...
        let runtime_type = tag == "input"
            && has_binding(
                attached,
                |binding| matches!(binding, l2::BindingOp::Bind(bind) if !matches!(bind.name, Some(DynamicName::Static(_)))),
            )
        ...
        for segment in attached {
            match segment.source {
                ...
                Source::Binding(binding) => match binding {
                    ...
                    l2::BindingOp::Model(model) if runtime_type && !model::dropped(self, model) => {
                        dynamic_model =
                            Some(self.expr(&model.contract.read, TransformContent::Decoded)?);
                    }
                    l2::BindingOp::Model(model) => model::collect_root(
                        self,
                        attached,
                        model,
                        tag,
