        ...
        match (segment.kind, segment.source) {
            ...
            (_, Source::Binding(binding)) => {
                match binding {
                    ...
                    l2::BindingOp::On(_) => {}
                    l2::BindingOp::Model(model) => model::admit(model, tag)?,
                    l2::BindingOp::VueShow(show) => admit_value(Some(&show.value))?,
                    l2::BindingOp::VueHtml(html) => admit_value(html.value.as_ref())?,
                    l2::BindingOp::VueText(text) => admit_value(text.value.as_ref())?,
                    // Rendered through `_ssrGetDirectiveProps`; a dynamic
                    // argument must be a bare identifier.
                    l2::BindingOp::VueDirective(directive) => {
                    ...
                    l2::BindingOp::VueOnce(_) | l2::BindingOp::VueCloak(_) => continue,
...
/// A directive value the transform rewrite can own. An opaque value (the L2
...
pub(super) fn bind<'r, 'a>(binding: &'r l2::BindingOp<'a>) -> Result<Option<Bind<'r, 'a>>> {
    let l2::BindingOp::Bind(bind) = binding else {
        return Ok(None);
    };
    if bind
        .modifiers
...
pub(super) fn bound_value<'r, 'a>(
    ...
    attached.iter().find_map(|segment| match segment.source {
        Source::Binding(l2::BindingOp::Bind(bind)) => match bind.name {
...
fn show_value<'r, 'a>(attached: &Attached<'r, 'a>) -> Option<&'r ExprRef<'a>> {
    attached.iter().find_map(|segment| match segment.source {
        Source::Binding(l2::BindingOp::VueShow(show)) => Some(&show.value),
        _ => None,
    })
}
        ...
        match segment.source {
            ...
            Source::Binding(binding) => match binding {
                l2::BindingOp::Bind(_) => {
                    ...
                    let name = match bind.name {
                        BindName::Static(name) if bind.camel => Some(vize_l0::camelize(name)),
                ...
                l2::BindingOp::Model(model) => model::emit_inline(em, attached, model, tag)?,
                l2::BindingOp::VueShow(show) if !explicit_style => {
