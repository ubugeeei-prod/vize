...
impl<'r, 'a> Emitter<'_, 'r, 'a, '_, '_, '_> {
    ...
    pub(super) fn vnode_if(&mut self, if_op: &'r l2::IfOp<'a>) -> Result<String> {
    ...
    /// 128 | 256))`.
    pub(super) fn vnode_for(&mut self, for_op: &'r l2::ForOp<'a>) -> Result<String> {
        let fact = self.segment(self.pos)?.fact;
        self.pos += 1;
        for helper in [
    ...
    fn vnode_for_body(&mut self, fact: u32, ops: &'r [l2::Op<'a>]) -> Result<(String, bool)> {
        ...
        if let [l2::Op::Element(element)] = ops {
    ...
    fn bound_key_value(&self, bind: &l2::BindOp<'_>) -> Result<String> {
