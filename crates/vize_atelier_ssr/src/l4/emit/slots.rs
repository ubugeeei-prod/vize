...
fn slot_content<'r, 'a>(bindings: &'r [l2::BindingOp<'a>]) -> Option<&'r l2::SlotContentOp<'a>> {
    bindings.iter().find_map(|binding| match binding {
        l2::BindingOp::SlotContent(content) => Some(&**content),
        _ => None,
    })
}
...
fn slot_head(file: &str, content: &l2::SlotContentOp<'_>) -> Result<(String, Option<String>)> {
...
pub(super) fn slot_pattern(file: &str, content: &l2::SlotContentOp<'_>) -> Result<Option<String>> {
...
impl<'r, 'a> Emitter<'_, 'r, 'a, '_, '_, '_> {
    ...
    pub(super) fn template_slot(
        &self,
        start: usize,
    ) -> Option<(&'r l2::ElementOp<'a>, &'r l2::SlotContentOp<'a>)> {
