...
impl<'r, 'a> Emitter<'_, 'r, 'a, '_, '_, '_> {
    ...
    /// `vnode_child_expression` for the child at the cursor; a merged L2 text
    ...
    fn vnode_child(&mut self, out: &mut std::vec::Vec<String>) -> Result<()> {
        ...
        match (segment.kind, segment.source) {
            ...
            (Kind::OpenElement, Source::Element(element))
                if element.tag == "template"
                    && element
                        .bindings
                        .iter()
                        .any(|binding| matches!(binding, l2::BindingOp::SlotContent(_))) =>
            {
            ...
            (Kind::OpenElement, Source::Element(element)) => {
                let expression =
                    vize_l0::ensure_sufficient_stack(|| self.vnode_element(element, None))?;
                out.push(expression);
            }
            (Kind::Component, Source::Component(component)) => {
                let expression =
                    vize_l0::ensure_sufficient_stack(|| self.vnode_component(component))?;
                out.push(expression);
            }
            (Kind::SlotOutlet, Source::Slot(slot)) => {
                let expression = vize_l0::ensure_sufficient_stack(|| self.vnode_outlet(slot))?;
                out.push(expression);
            }
            (Kind::If, Source::If(if_op)) => {
                let expression = vize_l0::ensure_sufficient_stack(|| self.vnode_if(if_op))?;
                out.push(expression);
            }
            (Kind::For, Source::For(for_op)) => {
                let expression = vize_l0::ensure_sufficient_stack(|| self.vnode_for(for_op))?;
                out.push(expression);
            }
    ...
    fn vnode_component(&mut self, component: &'r l2::ComponentOp<'a>) -> Result<String> {
        let open = self.segment(self.pos)?;
    ...
    pub(super) fn vnode_slots(&mut self, component: &'r l2::ComponentOp<'a>) -> Result<String> {
        let start = self.pos;
    ...
    pub(super) fn vnode_slot_fn(&mut self, spec: &super::slots::SlotSpec) -> Result<String> {
        let mut params = vize_l0::FxHashSet::default();
