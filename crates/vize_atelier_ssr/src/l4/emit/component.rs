        ...
        match (segment.kind, segment.source) {
            ...
            (_, Source::Binding(binding)) => {
                ...
                match binding {
                    ...
                    l2::BindingOp::Model(model) => admit_value(Some(&model.contract.read))?,
                    l2::BindingOp::VueShow(show) => admit_value(Some(&show.value))?,
                    l2::BindingOp::VueHtml(_)
                    | l2::BindingOp::VueText(_)
                    | l2::BindingOp::VueDirective(_)
                    | l2::BindingOp::VueOnce(_)
                    | l2::BindingOp::VueMemo(_)
                    | l2::BindingOp::VueCloak(_) => {}
                    l2::BindingOp::SlotContent(_) => {}
                    l2::BindingOp::VueSlotScope(_) => {
                        return Err(LegacyReason::Operation.into());
                    }
                    l2::BindingOp::VueSync(_) | l2::BindingOp::VueCssBind(_) => {
    ...
    fn static_or_bound(&self, attached: &Attached<'_, '_>, name: &str) -> Result<Option<String>> {
        for segment in attached {
            match segment.source {
                ...
                Source::Binding(l2::BindingOp::Bind(bind)) if matches!(bind.name, Some(DynamicName::Static(bound)) if bound == name) =>
                {
