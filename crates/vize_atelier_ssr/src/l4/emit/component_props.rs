...
use vize_l0::{String, ToCompactString, camelize, cstr};
use vize_l1_to_l2::{TransformContent, TransformExpressions, decode_template_entities};
...
use crate::l4::LegacyReason;
use crate::l4::string_plan::{SsrSegmentSource as Source, SsrStringPayloadKind};
...
impl Emitter<'_, '_, '_, '_, '_, '_> {
    ...
    fn component_binding(
        &mut self,
        binding: &l2::BindingOp<'_>,
        entries: &mut std::vec::Vec<VNodePropEntry>,
    ) -> Result<Option<String>> {
        match binding {
            l2::BindingOp::Bind(bind) => {
            ...
            l2::BindingOp::Model(model) => self.model_props(model, entries)?,
            l2::BindingOp::VueShow(show) => {
    ...
    fn callable_handler(&self, processed: vize_l1_to_l2::TransformedExpr) -> Result<String> {
...
fn is_boundary(binding: &l2::BindingOp<'_>) -> bool {
    match binding {
        l2::BindingOp::Bind(bind) => !matches!(bind.name, Some(DynamicName::Static(_))),
        l2::BindingOp::On(on) => !matches!(on.name, Some(DynamicName::Static(_))),
        l2::BindingOp::Model(model) => matches!(model.argument, Some(DynamicName::Dynamic(_))),
        _ => false,
    }
}
