//! The L2 walk that consumes partition facts in page order.
...
impl<'facts, 'r, 'a> Cx<'facts, 'r, 'a> {
    ...
    fn lower_region(&mut self, region: &'r l2::Region<'a>) {
        for op in &region.ops {
            self.lower_op(op);
        }
    }

    fn lower_op(&mut self, op: &'r l2::Op<'a>) {
        match op {
            l2::Op::Element(element) => {
                let element: &'r l2::ElementOp<'a> = element;
            ...
            l2::Op::Component(component) => {
                let component: &'r l2::ComponentOp<'a> = component;
            ...
            l2::Op::Interpolation(interpolation) => {
            ...
            l2::Op::For(for_op) => {
                let for_op: &'r l2::ForOp<'a> = for_op;
    ...
    fn lower_attached(
        &mut self,
        attributes: &'r [l2::Attribute<'a>],
        bindings: &'r [l2::BindingOp<'a>],
