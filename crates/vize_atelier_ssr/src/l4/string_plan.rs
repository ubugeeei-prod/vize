//! L2->L4 string-plan lowering for SSR.
...
use vize_l2_to_l3::{PartitionFacts, PartitionKind};

pub use payload::{SsrStringPayload, SsrStringPayloadKind};

/// SSR string plan emitted from L2 before JavaScript text generation.
#[derive(Debug)]
...
///
/// Segments are in L2 page order, except that an element's static
...
pub struct SsrStringSegment<'r, 'a> {
    ...
    pub span: Span,
    /// Index of the partition fact this segment read: the owning op's L2
    ...
    /// The L2 construct this segment plans, for emitters that need the whole
    /// typed operand (attribute values, binding names, retained expressions).
    pub source: SsrSegmentSource<'r, 'a>,
}

/// The L2 construct behind one plan segment.
///
...
pub enum SsrSegmentSource<'r, 'a> {
    ...
    Attribute(&'r l2::Attribute<'a>),
    Binding(&'r l2::BindingOp<'a>),
    Text(&'r l2::TextOp<'a>),
    Interpolation(&'r l2::InterpolationOp<'a>),
    Comment(&'r l2::CommentOp<'a>),
    If(&'r l2::IfOp<'a>),
    Branch(&'r l2::IfBranch<'a>),
    For(&'r l2::ForOp<'a>),
    Slot(&'r l2::SlotOp<'a>),
}

/// The stable SSR L4 vocabulary for the first string-plan slice.
...
/// Result of lowering L2 into a string plan.
#[derive(Debug)]
...
/// partition facts. The fact stream is consumed in the L2 page-order that the
/// L3 lowering uses, so stale or mismatched facts become explicit errors.
#[must_use]
pub fn lower_l2_to_string_plan<'r, 'a>(
