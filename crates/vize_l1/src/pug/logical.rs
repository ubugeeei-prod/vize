...
//! into the next token's `leading` slice — the L1 byte-fidelity law holds
...
impl<'a> Logical<'a> {
    pub(crate) fn new(allocator: &'a Allocator, source: &'a str) -> Self {
        ...
        let mut text = vize_l0::StringBuilder::with_capacity_in(source.len(), allocator);
...
fn push<'a>(
    text: &mut vize_l0::StringBuilder<'a>,
