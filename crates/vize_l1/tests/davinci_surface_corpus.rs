...
//! L1 parser — SFC block markup as markup, `<script>`/`<style>` content
...
use vize_l1::{HoleCounts, hole_counts, parse, render};
...
fn assert_round_trip(source: &str, context: &str) {
    ...
    assert_eq!(out.as_str(), source, "L1 fidelity diverged: {context}");
}
