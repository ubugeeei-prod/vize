...
const anyInstanceInferenceUsesPublicInstance: true = null as unknown as __VizeUsePublicInstance<any>
...
const listeners = <Public modelValue={1} someValue="ok" onSelect={(value) => { const notAny: false = null as unknown as IsAny<typeof value>; return value.toUpperCase() }} />
const modelListener = <Public modelValue={1} someValue="ok" {...{ 'onUpdate:modelValue': (value: number) => value.toFixed() }} />
const fallthrough = <Public modelValue={1} someValue="ok" class="public" style="color:red" data-testid="public" aria-label="Public" onClick={(event) => { const mouse: MouseEvent = event; const notAny: false = null as unknown as IsAny<typeof event>; return mouse.preventDefault() }} onInput={(event) => { const input: InputEvent = event; return input.preventDefault() }} />
const callableListeners = <Callable onCommit={(value) => value.toUpperCase()} onCancel={() => undefined} />
const runtimeObjectListeners = <RuntimeObject onSave={(value) => value.toUpperCase()} onReset={() => undefined} />
const runtimeArrayListeners = <RuntimeArray onOpen={() => undefined} onClose={() => undefined} />
const genericListener = <Generic item="value" onPick={(value) => { const exact: 'value' = value; return exact.toUpperCase() }} />
...
const wrongModelListener = <Public modelValue={1} someValue="ok" {...{ 'onUpdate:modelValue': (value: string) => value }} />
...
const wrongNativeListenerProp = <NativeListenerProp onClick={(value: MouseEvent) => value.preventDefault()} />
