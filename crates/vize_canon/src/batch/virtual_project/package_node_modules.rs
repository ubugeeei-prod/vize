...
//! project-keyed namespace outside `node_modules`, so the project-root and
//! ancestor installs are merged at the mirror root. Editor roots are also
//! session-private.
...
use crate::batch::runtime_deps::protected_vue_namespace_packages;
use crate::batch::runtime_deps::symlink_package_dir;

use merged::{MirroredEntries, merged_ancestor_links, merged_links};
...
/// virtual root, that have a real `node_modules` sibling. When the virtual root
/// is outside `<project_root>/node_modules` (the session-private editor shape),
/// the project-root and ancestor installs are merged entry-by-entry at
/// `<canon>/node_modules`, preserving the nearest package for duplicate names.
/// Canon's `vue`, `@vue`, and `vite` runtime entries and any package shadows
/// remain owned by the mirror.
///
...
pub(super) fn package_node_modules_links<'a>(
    ...
    let ancestor_installs = ancestor_node_modules(project_root);
    if !virtual_root.starts_with(&project_node_modules) {
        let root = PathBuf::new();
        candidates.push(root.clone());
        seen.insert(root.clone());
        let root_entries = mirrored.entry(root).or_default();
        for runtime_entry in ["vue", "vite"] {
            root_entries.record(std::iter::once(runtime_entry));
        }
        for runtime_package in protected_vue_namespace_packages(project_root) {
            root_entries.record(["@vue", runtime_package.as_str()].into_iter());
        }
    }
    ...
    candidates
        ...
        .flat_map(|relative| {
            ...
            if relative.as_os_str().is_empty() {
                return mirrored
                    .get(&relative)
                    .map_or_else(Vec::new, |root_entries| {
                        merged_ancestor_links(&ancestor_installs, &virtual_dir, root_entries)
                    });
            }
...
fn ancestor_node_modules(project_root: &Path) -> Vec<PathBuf> {
    // TypeScript walks *every* ancestor of an importer, including ancestors
    // without package.json. A single nearest install can be partial (pnpm
    // workspace packages commonly have different linked dependencies), so the
    // mirror must preserve the complete nearest-first lookup chain.
    project_root
        .ancestors()
...
/// shadow files that Canon owns below the same `node_modules` path.
pub(super) fn merge_aware_node_modules_links<'a>(
    real_dir: &Path,
    virtual_dir: &Path,
    materialized_files: impl Iterator<Item = &'a Path>,
) -> Vec<PackageNodeModulesLink> {
    let mut mirrored = MirroredEntries::default();
    let mut owns_entries = false;
    for file in materialized_files {
        let Some(relative_parent) = file.strip_prefix(virtual_dir).ok().and_then(Path::parent)
        else {
            continue;
        };
        owns_entries = true;
    ...
    if !owns_entries {
        return vec![PackageNodeModulesLink {
            virtual_dir: virtual_dir.to_path_buf(),
            real_dir: real_dir.to_path_buf(),
        }];
    }
    merged_links(real_dir, virtual_dir, &mirrored)
}
