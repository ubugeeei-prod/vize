...
/// Flatten the real ancestor lookup chain into one mirror `node_modules`.
/// The first package found wins, just as it does when TypeScript walks from the
/// source file toward the filesystem root. Split scopes by package so a nearer
/// partial `@scope` does not hide packages installed in a farther ancestor.
pub(super) fn merged_ancestor_links(
    real_dirs: &[std::path::PathBuf],
    virtual_dir: &Path,
    mirrored: &MirroredEntries,
) -> Vec<PackageNodeModulesLink> {
    ...
    for real_dir in real_dirs {
        for candidate in merged_links(real_dir, virtual_dir, mirrored) {
            let is_whole_scope = candidate
                .virtual_dir
                .parent()
                .is_some_and(|parent| parent == virtual_dir)
                && candidate
                    .virtual_dir
                    .file_name()
                    .is_some_and(|name| name.to_string_lossy().starts_with('@'));
            if is_whole_scope && real_dirs.len() > 1 {
                for name in sorted_entry_names(&candidate.real_dir) {
                    let scoped = link(&candidate.real_dir, &candidate.virtual_dir, &name);
                    if seen.insert(scoped.virtual_dir.clone()) {
                        links.push(scoped);
                    }
                }
            } else if seen.insert(candidate.virtual_dir.clone()) {
