...
use crate::virtual_ts::{
    ...
    scope::{FORWARDED_RETURN_KEY, FORWARDED_ROOT_HELPERS},
};
...
impl ForwardedRoots {
    ...
    /// Emit the helpers the probes call, and return whether there are any.
    pub(in crate::virtual_ts::generator) fn emit_helpers(&self, ts: &mut String) -> bool {
        if self.any() {
...
mod tests {
    ...
    /// `(root starts, setup return fields, props tail)`.
    fn plan(
        template: &str,
        instantiated: bool,
        check_required: bool,
        ...
        (roots.starts().to_vec(), fields, tail)
    }
    ...
    fn required_forwarded_props_exclude_what_the_root_binds_itself() {
        assert_eq!(
            plan("<Basic :foo=\"bar\" @close=\"bar\" />", true, true).2,
            " & Omit<Setup<T>[\"__vize_forwarded\"], \"foo\" | \"onClose\">"
        );
        assert_eq!(
            plan("<Basic />", true, true).2,
    ...
    fn every_other_root_keeps_the_declared_surface() {
        let declared = (Vec::new(), Vec::new(), String::default());
