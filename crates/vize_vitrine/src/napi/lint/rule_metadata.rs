...
pub(super) struct PatinaRuleMetaNapi<'a> {
    ...
    pub(super) default_severity: &'a str,
...
pub(super) fn collect_patina_rule_metadata() -> Vec<PatinaRuleMetaNapi<'static>> {
...
fn rule_name_set(registry: RuleRegistry) -> FxHashSet<&'static str> {
    registry
        .rules()
        .iter()
