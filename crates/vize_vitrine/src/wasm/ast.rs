        ...
        match child {
                    ...
                    .map(|prop| match prop {
                        ...
                        PropNode::Directive(directive) => serde_json::json!({
                            ...
                            "arg": directive.arg.as_ref().map(|argument| match argument {
                                vize_relief::ExpressionNode::Simple(expression) => expression.content.as_str().to_string(),
            ...
            TemplateChildNode::Interpolation(interpolation) => serde_json::json!({
                ...
                "content": match &interpolation.content {
                    vize_relief::ExpressionNode::Simple(expression) => expression.content.as_str(),
                    _ => "<compound>",
                }
            }),
            _ => serde_json::json!({
