//! The L2 verifier: local invariant checks between passes, debug/CI only.
//!
...
//!
//! Documented in `docs/davinci/plan/folio-format.md` ("L2 verifier
//! invariants") — the format doc is where "canonical" is written down.
//!
//! | code   | rigor      | invariant                                        |
//! | ------ | ---------- | ------------------------------------------------ |
//! | L2V001 | structural | a span never runs backwards                      |
//! | L2V002 | structural | a nested span stays inside its owner's           |
//! | L2V003 | structural | every side-table `NodeId` resolves               |
//! | L2V004 | canonical  | `ui.if` owns at least one branch                 |
//! | L2V005 | canonical  | the leading branch carries a condition           |
...
pub enum ViolationCode {
    /// L2V001 — a span runs backwards (`start > end`).
    SpanOrder,
    /// L2V002 — a nested line's span escapes its owner's.
    RegionNesting,
    /// L2V003 — a side table references a node the artifact does not
    /// number.
    IdResolution,
    /// L2V004 — `ui.if` owns no branch.
    EmptyIf,
    /// L2V005 — the leading branch of `ui.if` is unconditional.
    LeadingElse,
    /// L2V006 — an unconditional branch is not the trailing branch.
    BranchOrder,
}
...
impl ViolationCode {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ViolationCode::SpanOrder => "L2V001",
            ViolationCode::RegionNesting => "L2V002",
            ViolationCode::IdResolution => "L2V003",
            ViolationCode::EmptyIf => "L2V004",
            ViolationCode::LeadingElse => "L2V005",
            ViolationCode::BranchOrder => "L2V006",
        }
    }
}
...
mod tests {
    ...
    fn codes_render_as_their_stable_strings() {
        assert_eq!(ViolationCode::SpanOrder.as_str(), "L2V001");
        assert_eq!(ViolationCode::RegionNesting.as_str(), "L2V002");
        assert_eq!(ViolationCode::IdResolution.as_str(), "L2V003");
        assert_eq!(ViolationCode::EmptyIf.as_str(), "L2V004");
        assert_eq!(ViolationCode::LeadingElse.as_str(), "L2V005");
        assert_eq!(ViolationCode::BranchOrder.as_str(), "L2V006");
    }
    ...
    fn a_violation_renders_as_one_line() {
        ...
        assert_eq!(
            ...
            "L2V004 @3:9 `ui.if` owns no branch"
        );
    }
