//! The L2 op family: one concrete typed enum per position, never a uniform
...
//!
//! Every type here is arena-resident through `vize_l0::{Box, Vec}`,
...
/// One L2 op standing in a region (a child position).
///
...
/// One L2 op attached to an element or component (a binding position).
///
...
/// The op family is `Drop`-free by construction: `vize_l0::{Box, Vec}`
