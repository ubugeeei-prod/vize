//! The between-pass wiring of the L2 verifier (P2-6): rigor per
...
fn check_panics_with_the_exact_report_naming_the_pass() {
    ...
    assert_eq!(
        *message,
        "L2 verifier: 1 violation(s) after `s2.lower`\n\
         L2V004 @0:10 `ui.if` owns no branch"
    );
}
...
fn check_table_panics_with_the_exact_report_on_a_dangling_id() {
    ...
    assert_eq!(
        *message,
        "L2 verifier: 1 violation(s) after `s2.diagnose`\n\
         L2V003 @0:0 side table references %99, but the artifact numbers 2 ops"
    );
}
...
/// asserted here the way `vize_l0`'s own stamp tests assert it.
#[cfg(debug_assertions)]
#[test]
