...
impl<'a> TransformContext<'a> {
    ...
    pub(crate) fn add_template(
        ...
        if let Some(spans) = self.template_spans.as_mut()
            && !template.links().is_empty()
        {
            spans.insert(template_index, template.links().to_vec());
        }
    ...
    pub(crate) fn text_template(&self, text: &TextNode<'_>) -> EmitDocument {
        match self.template_spans {
            Some(_) => EmitDocument::mapped(text.content, text.loc.span.start),
            None => EmitDocument::plain(text.content),
        }
    }
