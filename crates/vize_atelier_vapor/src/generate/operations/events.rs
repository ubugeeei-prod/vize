...
pub(super) fn generate_set_event(ctx: &mut GenerateContext, set_event: &SetEventIRNode<'_>) {
    ...
    let invoker_body = if let Some(value) = set_event.value.as_deref() {
        ...
        {
            let mut body = SpannedText::plain("e => ");
            ...
            SpannedText::from(resolve_inline_handler(ctx, value))
        }
    } else {
        SpannedText::plain("() => {}")
    };
    ...
    if set_event.delegate {
        ...
        let mut line =
            SpannedText::plain(&cstr!("{}.$evt{} = _createInvoker(", element, event_name));
...
fn wrap_handler(callee: &str, handler: &SpannedText, list: &str) -> SpannedText {
