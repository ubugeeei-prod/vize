...
use vize_atelier_core::{
    ...
    codegen::spanned::{SpanAnchor, SpannedText},
};
...
pub(crate) struct VaporSourceSpans {
    ...
    pub(crate) templates: FxHashMap<usize, std::vec::Vec<SpanAnchor>>,
...
pub(crate) type TemplateSpans = FxHashMap<usize, std::vec::Vec<SpanAnchor>>;
...
impl VaporSourceSpans {
    ...
    pub(crate) fn collect(
        root: &RootNode<'_>,
        templates: FxHashMap<usize, std::vec::Vec<SpanAnchor>>,
    ) -> Self {
        let mut spans = Self {
            templates,
...
/// Rebase template-string anchors onto the `escape_template` output of
/// `template` written at the generated offset `base` (a backslash, `"`, a
/// newline or a carriage return each grow by one byte).
pub(crate) fn escaped_template_anchors(
    template: &str,
    anchors: &[SpanAnchor],
    base: usize,
) -> std::vec::Vec<SpanAnchor> {
    let bytes = template.as_bytes();
    let (mut cursor, mut escapes) = (0usize, 0usize);
    anchors
        .iter()
        .map(|anchor| {
            while cursor < anchor.offset.min(bytes.len()) {
                escapes += usize::from(matches!(bytes[cursor], b'\\' | b'"' | b'\n' | b'\r'));
                cursor += 1;
            }
            SpanAnchor {
                offset: base + anchor.offset + escapes,
                ..anchor.clone()
            }
        })
        .collect()
...
impl GenerateContext<'_> {
    ...
    pub(crate) fn if_head(&self, if_node: &crate::ir::IfIRNode<'_>) -> SpannedText {
    ...
    /// element.
    pub(crate) fn else_head(&self, if_node: &crate::ir::IfIRNode<'_>) -> SpannedText {
        let mut head = SpannedText::plain("}, ");
    ...
    pub(crate) fn spanned_at(&self, text: &str, source: Option<u32>) -> SpannedText {
        match source.filter(|_| self.spans.is_some()) {
            Some(source) => SpannedText::mapped(text, source),
            None => SpannedText::plain(text),
        }
    }

    /// A resolved expression node, anchored when maps are on.
    pub(crate) fn spanned_expression_node(&self, node: &SimpleExpressionNode<'_>) -> SpannedText {
    ...
    pub(crate) fn push_spanned(&mut self, piece: &SpannedText) {
        if let Some(anchors) = self.anchors.as_mut() {
            ...
            anchors.extend(piece.anchors().iter().map(|anchor| SpanAnchor {
    ...
    pub(crate) fn push_line_spanned(&mut self, piece: &SpannedText) {
        self.push_indent();
