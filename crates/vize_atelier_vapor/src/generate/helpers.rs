...
use super::{
    ...
    operations::{generate_operation, set_prop_call, set_text_call},
...
pub(crate) fn generate_effect(
    ...
    if effect.operations.len() == 1 {
        ...
        if let Some(op_code) = generate_operation_inline(ctx, op) {
            let mut line = SpannedText::plain("_renderEffect(() => ");
...
pub(crate) fn generate_operation_inline(
    ...
    match op {
        ...
        OperationNode::SetDynamicProps(set_props) => Some(SpannedText::from(
