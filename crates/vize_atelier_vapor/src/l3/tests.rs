use super::{
    LegacyReason, VaporL3BridgeOptions, VaporL3BridgeStatus, admit, lower_source_for_vapor,
    retained::Retained,
};
...
pub(super) fn options() -> VaporL3BridgeOptions {
...
fn lowered<'a>(allocator: &'a Allocator) -> vize_l2_to_l3::Lowered<'a> {
...
pub(super) fn lowered_source<'a>(
    ...
    source: &str,
) -> vize_l2_to_l3::Lowered<'a> {
    let scratch = Allocator::new();
    let (tree, errors) = vize_l1::parse(&scratch, source);
    let s2 = vize_l1_to_l2::lower(&scratch, &tree, &errors);
...
fn corrupt_graph_and_partition_are_rejected_instead_of_falling_back() {
    ...
    for mutation in 0..4 {
        ...
        match mutation {
            ...
            _ => s3.partition.ops[0].kind = vize_l2_to_l3::PartitionKind::Dynamic,
        }
        assert!(
            matches!(
                admit(s3, &Retained::new(&allocator)),
                VaporL3BridgeStatus::Rejected(_)
            ),
            "mutation {mutation}"
        );
    }
...
fn generic_graph_verification_does_not_imply_backend_admission() {
    ...
    for mutation in 0..6 {
        ...
        assert!(vize_l3::verify::verify(&s3.program).is_empty());
        let status = admit(s3, &Retained::new(&allocator));
        if matches!(mutation, 0 | 1 | 3 | 5) {
            assert!(
                matches!(status, VaporL3BridgeStatus::Rejected(_)),
                "mutation {mutation}: {status:?}"
            );
        } else {
            assert!(
                matches!(status, VaporL3BridgeStatus::Legacy(_)),
                "mutation {mutation}: {status:?}"
            );
        }
    }
...
fn unsupported_source_semantics_have_explicit_legacy_routes() {
    ...
    assert!(matches!(
        ...
        VaporL3BridgeStatus::Legacy(LegacyReason::Options)
    ));
    ...
    assert!(matches!(
        ...
        VaporL3BridgeStatus::Legacy(LegacyReason::Selected)
    ));
}
...
fn empty_static_text_cannot_shift_materialized_child_addresses() {
    ...
    assert!(matches!(
        admit(s3, &Retained::new(&allocator)),
        VaporL3BridgeStatus::Legacy(LegacyReason::ExpressionOrEncoding)
    ));
}
