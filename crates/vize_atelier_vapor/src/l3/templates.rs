//! `<template v-if>` and `<template v-for>` carriers unwrap in L2, so L3 sees
//! only their content; L2 keeps what the wrapper carried as side facts. A
...
use vize_l3::op::{OpId, OpKind, Program};

use super::{LegacyReason, native::validate::reference, retained::Retained};

/// A template-carried loop: its L3 op and its wrapper key text and span.
...
pub(super) fn collect<'a>(
    ...
    for (node, wrapper) in s2.for_wrappers.iter() {
        ...
        // The loop's directive value locates its L3 op: the innermost loop
        // spanning it.
        let value = s2
            .provenance
            .iter()
...
/// compound text parts do; L2 kept only the text.
fn key<'a>(
    allocator: &'a Allocator,
    source: &str,
    text: &str,
