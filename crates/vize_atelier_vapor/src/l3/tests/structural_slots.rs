...
use crate::l3::{VaporL3BridgeStatus, lower_source_for_vapor, retained::Retained};
...
fn conditional_slot_condition_is_owned_by_the_checked_payload() {
    ...
    let condition = lowered
        ...
        .operands
        .iter_mut()
        .find(|operand| operand.role == vize_l3::operand::OperandRole::Condition)
        .unwrap();
    condition.value.text = "replacement";
    let code = generated(
        crate::l3::admit(lowered, &Retained::new(&allocator)),
        &allocator,
    );
...
fn loop_slot_source_is_owned_by_the_checked_payload() {
    ...
    let source = lowered
        ...
        .operands
        .iter_mut()
        .find(|operand| operand.role == vize_l3::operand::OperandRole::ForSource)
        .unwrap();
