...
use crate::l3::{VaporL3BridgeStatus, admit, lower_source_for_vapor, retained::Retained};
...
fn checked_select_payload_owns_the_model_assignment() {
    ...
    for operand in &mut s3.program.operands {
        if matches!(
            operand.role,
            vize_l3::operand::OperandRole::ModelRead | vize_l3::operand::OperandRole::ModelWrite
        ) {
