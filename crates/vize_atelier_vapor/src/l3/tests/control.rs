...
use crate::l3::{
    LegacyReason, VaporL3BridgeStatus, admit, lower_source_for_vapor, retained::Retained,
};
...
fn unsupported_control_flow_selects_exact_legacy_reasons() {
    ...
    for (source, reason) in [
        // L2 keeps carrier branch keys as side facts (template wrappers:
        // `tests/templates.rs`).
        (
...
fn corrupt_control_flow_graphs_are_rejected_instead_of_falling_back() {
    ...
    for mutation in 0..5 {
        ...
        let find = |role: OperandRole, text: &str, operands: &[vize_l3::operand::Operand<'_>]| {
            operands
                .iter()
        ...
        match mutation {
            ...
            _ => {
                let body = s3
                    ...
                    .ops
                    .iter()
                    .position(|op| op.kind == vize_l3::op::OpKind::InsertNode && op.span.start == 4)
                    .unwrap();
                s3.program.ops[body].effect = None;
                s3.partition.ops[body].kind = vize_l2_to_l3::PartitionKind::Static;
            }
        }
        assert!(
            vize_l3::verify::verify(&s3.program).is_empty(),
...
fn branch_gaps_match_the_retained_lane_and_comment_gaps_stay_legacy() {
    ...
    for source in [
        ...
        assert!(
            matches!(
                status,
                VaporL3BridgeStatus::Legacy(LegacyReason::SurfaceSemantics)
            ),
            "{source}: {status:?}"
        );
    }
