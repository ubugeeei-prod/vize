//! Backend admission is a projection, not a boolean promise about a generic L3
...
/// other expression carries L2's retained parse for the shared generator.
#[derive(Debug, Clone, Copy)]
struct Expr<'a> {
    text: &'a str,
...
struct Binding<'a> {
    ...
    /// L3's element-kind operand for a model; checked against the target tag.
