...
use vize_l0::{FxHashMap, String, ToCompactString, append, cstr};
...
mod tests {
    ...
    fn make_entry(title: &str, category: Option<&str>, tags: &[&str]) -> CatalogEntry {
        CatalogEntry {
            title: title.into(),
            description: Some(vize_l0::cstr!("{} description", title)),
            category: category.map(|s| s.into()),
            tags: tags
                .iter()
                .map(|s| vize_l0::CompactString::from(*s))
                .collect(),
            status: ArtStatus::Ready,
            variant_count: 2,
            doc_path: vize_l0::cstr!("{}.md", slugify(title)),
            source_path: vize_l0::cstr!("{}.art.vue", slugify(title)),
            order: None,
        }
    }
