...
fn template_child_text_loc() {
    ...
    let child = TemplateChildNode::Text(vize_l0::Box::new_in(text, &&allocator));
...
fn template_child_hoisted_loc() {
    ...
    assert_eq!(child.loc().span, vize_l0::Span::new(0, 0));
...
fn source_location_stub() {
    ...
    assert_eq!(stub.span, vize_l0::Span::new(0, 0));
}
...
fn source_location_new() {
    ...
    assert_eq!(loc.span, vize_l0::Span::new(0, 5));
}
...
fn source_location_set_end() {
    ...
    loc.set_end(9);
    assert_eq!(loc.span, vize_l0::Span::new(3, 9));
}
