...
#[test]
fn sfc_sections_entry_keeps_custom_foreign_descendants_on_l2() {
    ...
    for (label, source) in [
        ...
        assert_eq!(
            ...
            Some(1),
            "{label} must keep the L2 SFC sections fast path"
        );
    }
}
...
fn sfc_sections_entry_pops_html_reentry_closes_case_insensitively() {
    ...
    assert_eq!(
        counter_total(&counters, "davinci.s2_dom.files"),
        Some(1),
        "case-insensitive HTML closes inside foreign re-entry must not poison the L2 selector"
    );
}

#[test]
fn sfc_sections_entry_uses_parser_backed_l2_for_root_html_title() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.clear();
    profiler.enable();
    ...
    assert_eq!(
        counter_total(&counters, "davinci.s2_dom.files"),
        Some(1),
        "root HTML title must use L2 after parser recovery, not the direct foreign fast path"
    );
}
