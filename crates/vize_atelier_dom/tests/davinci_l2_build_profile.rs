...
/// fixture -> current L2 observer walks and profiled build walks.
///
/// Source-map-free DOM compiles now enter L2 directly and report only the
/// single code-producing walk. Preserving fact products are folded before
/// emission; the legacy pre-L2 transform remains absent after L2 emission
/// succeeds.
///
/// | fixture       | L2 | build |
/// | ------------- | -- | ----- |
...
fn profile_build_walks_report_the_current_p2_12b_gap() {
    ...
    for fixture in &LADDER {
        ...
        let (current_l2_walks, current_build_walks) = current_walks(fixture.name);
        ...
        let l2_walks = counter(&counters, "davinci.s2_dom.total.walks");
        ...
        assert_eq!(
            pre_s2_walks, 0,
            "{} no longer pays a pre-L2 template walk",
            fixture.name
        );
        assert!(
            pre_s2_walks < baseline.walks && pre_s2_visits <= baseline.visits,
            "{} pre-L2 walk stayed under the P2-12a DOM ceiling",
            fixture.name
        );
        assert_eq!(
        ...
        assert_eq!(
            l2_walks, current_l2_walks,
            "{} current L2 observer walk count",
            fixture.name
        );
        assert_eq!(
            build_walks,
            pre_s2_walks + l2_walks,
