...
#[test]
fn l2_slot_outlets_match_the_shipped_dom_lane_byte_for_byte() {
    support::assert_l2_matches_shipped(support::battery::outlets::OUTLET_BATTERY);
}
...
#[test]
fn l2_slot_outlet_patch_sites_match_the_shipped_dom_lane_per_node() {
    ...
    for case in PATCH_SITE_CASES {
        ...
        let new = emit_dom_source(&allocator, case.src)
            .unwrap_or_else(|error| panic!("{}: L2 emit refused: {error:?}", case.name))
            .assembled();
...
#[test]
fn l2_vue2_slot_outlet_v_on_matches_the_shipped_dom_lane_byte_for_byte() {
    support::assert_l2_matches_shipped_with_dialect(VUE2_BATTERY, VueVersion::V2);
}
