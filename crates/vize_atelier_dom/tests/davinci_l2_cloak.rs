//! P2-11 `v-cloak` witness: `vue.cloak` lowers through L2 and emits the
...
#[test]
fn l2_v_cloak_matches_the_shipped_dom_lane_byte_for_byte() {
    let battery: Vec<_> = CASES.iter().map(|case| (case.name, case.src)).collect();
    support::assert_l2_matches_shipped(&battery);
}

#[test]
fn l2_v_cloak_patch_flags_match_the_shipped_dom_lane_per_node() {
