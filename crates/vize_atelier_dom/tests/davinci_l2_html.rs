//! P2-11 `v-html` witness: `vue.html` lowers through L2 and emits the
...
#[test]
fn l2_v_html_matches_the_shipped_dom_lane_byte_for_byte() {
    let battery: Vec<_> = CASES.iter().map(|case| (case.name, case.src)).collect();
    support::assert_l2_matches_shipped(&battery);
}

#[test]
fn l2_v_html_patch_flags_match_the_shipped_dom_lane_per_node() {
...
#[test]
fn l2_pug_example_text_decodes_double_escaped_parentheses_like_shipped_lane() {
