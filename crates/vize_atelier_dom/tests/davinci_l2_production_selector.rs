//! P2-11 witness: supported source-map-free DOM compiles use the L2 production
...
#[test]
fn source_map_free_dom_compile_uses_l2_without_profiler() {
...
#[test]
fn source_map_compiles_use_l2_with_the_compatibility_map_when_profiled() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    ...
    assert_eq!(
        counter_total(&counters, "davinci.s2_dom.files"),
        Some(1),
        "source-map compiles must use L2 once the map is verified against compatibility codegen"
    );
}
...
fn unsupported_options_stay_on_compatibility_without_profiler() {
    ...
    for (label, options) in cases {
        ...
        assert!(
            ...
            "{label} must stay on the compatibility path until L2 supports it"
        );
        assert_eq!(
            counter_total(&counters, "davinci.s2_dom.files"),
            None,
...
#[test]
fn in_tag_comment_compiles_use_l2_codegen() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
...
#[test]
fn sfc_sections_entry_uses_l2_once_sections_land() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
...
#[test]
fn sfc_sections_entry_uses_l2_for_foreign_namespace_templates() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    ...
    for (label, source) in [
        ...
        assert_eq!(
            counter_total(&counters, "davinci.s2_dom.files"),
            Some(1),
            "{label} must use the L2 SFC sections fast path"
        );
    }
}
