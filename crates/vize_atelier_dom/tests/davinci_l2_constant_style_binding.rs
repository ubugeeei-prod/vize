...
use vize_l1_to_l2::{BindingKind, BindingTable, DomEmitMode, DomEmitOptions};
...
const BATTERY: &[(&str, &str)] = &[
    ...
    // hoists the whole props object, which the L2 pass's own staticness
...
fn constant_style_bindings_match_the_shipped_dom_lane() {
    ...
    support::assert_l2_matches_shipped_with_options(
        BATTERY,
...
fn only_a_constant_binding_skips_the_style_helper() {
    ...
    let style_prop = |src: &str, inline: bool| {
        vize_l1_to_l2::emit_dom_source_with_options(
            &allocator,
            src,
