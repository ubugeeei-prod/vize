//! P2-12b witness: profile counters describe the L2 emitter used by the
...
use vize_l0::String;
use vize_l0::profiler::{CounterSummary, global_profiler};

static PROFILER_TEST_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

#[test]
fn profile_reports_real_l2_dom_walks() {
...
#[test]
fn profile_reports_ladder_l2_dom_walk_budget() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    support::profile::assert_l2_dom_emit_counts_cover_ladder();
    ...
    for fixture in &LADDER {
        ...
        assert_eq!(
            ...
            "{} profiled L2 emit must keep the compatibility preamble",
            fixture.name
        );
        assert_eq!(
            result.code, compat.code,
            "{} profiled L2 emit must keep the compatibility code",
            fixture.name
        );
        assert_eq!(counter(&counters, "davinci.s2_dom.files"), 1);
        assert_eq!(
...
#[test]
fn source_map_disabled_scope_id_stays_on_l2_codegen() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    ...
    assert_eq!(
        ...
        1,
        "direct scope_id compiles are covered by the L2 production option surface"
    );
}

#[test]
fn source_map_disabled_hoisted_scope_id_stays_on_l2_codegen() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
...
#[test]
fn source_map_disabled_runtime_global_name_stays_on_l2_codegen() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    ...
    assert_eq!(
        counter(&counters, "davinci.s2_dom.files"),
        1,
        "custom runtime-global compiles are covered by the L2 production option surface"
    );
}

#[test]
fn source_map_disabled_comments_use_l2_codegen() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    ...
    assert_eq!(
        counter(&counters, "davinci.s2_dom.files"),
        1,
        "comment-preserving source-map-free compiles are covered by the L2 production option surface"
    );
}
