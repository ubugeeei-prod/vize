...
//! predicate matchers are covered by the L2 DOM production selector.
...
#[test]
fn declarative_custom_element_matcher_uses_l2_for_template_sections() {
    ...
    assert_eq!(
        ...
        Some(1),
        "declarative custom-element matchers should enter the L2 production selector"
    );
}

#[test]
fn declarative_custom_element_matcher_uses_l2_for_sfc_sections() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    ...
    assert_eq!(
        counter_total(&counters, "davinci.s2_dom.files"),
        Some(1),
        "declarative custom-element matchers should enter the L2 SFC sections fast path"
    );
}

#[test]
fn static_predicate_custom_element_matcher_uses_l2_for_template_sections() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    ...
    assert_eq!(
        counter_total(&counters, "davinci.s2_dom.files"),
        Some(1),
        "opaque custom-element predicates should enter the L2 production selector"
    );
}

#[test]
fn static_predicate_custom_element_matcher_uses_l2_for_sfc_sections() {
    let _guard = lock_profiler();
    let profiler = global_profiler();
    profiler.disable();
    profiler.clear();
    ...
    assert_eq!(
        counter_total(&counters, "davinci.s2_dom.files"),
        Some(1),
        "opaque custom-element predicates should enter the L2 SFC sections selector"
    );
}
