...
) -> Result<RenduRoot, SfcGraphAdapterError> {
    RenduLowerer::new(snapshot, None).lower()
}

pub(crate) fn lower_relief_snapshot_to_rendu_with_anchor(
    snapshot: &ReliefSnapshot,
    anchor: SourceAnchor,
) -> Result<RenduRoot, SfcGraphAdapterError> {
...
impl<'a> RenduLowerer<'a> {
    ...
    fn lower(mut self) -> Result<RenduRoot, SfcGraphAdapterError> {
    ...
    ) -> Result<Vec<RenduNodeId>, SfcGraphAdapterError> {
    ...
    ) -> Result<RenduNodeId, SfcGraphAdapterError> {
        ...
        match node {
            ...
            ReliefSnapshotNode::Hoisted(hoist) => {
                let index = u32::try_from(hoist.index)
                    .map_err(|_| SfcGraphAdapterError::HoistIndexOverflow(hoist.index))?;
    ...
    fn lower_if(&mut self, node: &SnapshotIf) -> Result<RenduNodeId, SfcGraphAdapterError> {
    ...
    fn lower_branch(
        &mut self,
        branch: &SnapshotIfBranch,
    ) -> Result<RenduIfBranch, SfcGraphAdapterError> {
    ...
    fn lower_for(&mut self, node: &SnapshotFor) -> Result<RenduNodeId, SfcGraphAdapterError> {
