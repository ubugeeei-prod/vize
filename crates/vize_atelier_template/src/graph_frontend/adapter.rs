...
use super::{
    TemplateGraphAdapterError, lower_relief_snapshot_to_rendu, project_relief_snapshot_to_flow,
};

/// Zero-allocation view exposing independent products for one cached snapshot.
#[derive(Debug, Clone, Copy)]
pub struct TemplateGraphAdapter<'a> {
    snapshot: &'a ReliefSnapshot,
}

impl<'a> TemplateGraphAdapter<'a> {
    ...
    pub fn lower_rendu(self) -> Result<RenduRoot, TemplateGraphAdapterError> {
        lower_relief_snapshot_to_rendu(self.snapshot)
    }

    /// Explicitly project this syntax snapshot into control/data/effect flow.
    pub fn project_flow(self) -> Result<FlowGraph, TemplateGraphAdapterError> {
