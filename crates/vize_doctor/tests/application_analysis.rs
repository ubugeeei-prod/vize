...
fn accepts_relative_sources_without_a_workspace_boundary() {
    // With no project root and a relative registered path, the source is already
    // workspace-relative and must be preserved rather than rejected.
    let mut analyzer = CrossFileAnalyzer::new(CrossFileOptions::minimal());
...
fn accepts_case_mismatched_workspace_prefix() {
    ...
    // containment check must still relativize the path instead of rejecting it
    // as outside the workspace.
    let mut analyzer =
        CrossFileAnalyzer::with_project_root(CrossFileOptions::minimal(), "/Workspace");
    let app = analyzer.add_file("/workspace/src/App.ts", "export const app = 1");
    let result = CrossFileResult {
        diagnostics: vec![CrossFileDiagnostic::new(
            CrossFileDiagnosticKind::UnresolvedImport {
                specifier: "./missing".into(),
