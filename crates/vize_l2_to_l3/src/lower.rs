use vize_l0::{Allocator, Span, ensure_sufficient_stack};
...
/// Result of one L2→L3 lowering.
#[derive(Debug)]
pub struct Lowered<'a> {
    /// Flat L3 program in the `built` phase.
    pub program: Program<'a>,
    /// Static/dynamic partition facts exported beside the program.
    pub partition: PartitionFacts<'a>,
}

/// Lower an L2 root region to an L3 program plus shared partition facts.
#[must_use]
pub fn lower<'a>(allocator: &'a Allocator, root: &l2::Region<'_>) -> Lowered<'a> {
...
impl<'a> Cx<'a> {
    ...
    fn finish(mut self, root: &l2::Region<'_>) -> Lowered<'a> {
