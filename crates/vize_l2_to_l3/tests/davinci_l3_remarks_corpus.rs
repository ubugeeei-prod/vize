//! TS-32 for P3-10: the corpus remarks-diff over L3 extraction.
//!
//! The same sweep as the L2 remarks corpus (every `.vue` file under
//! `tests/_fixtures`, `_git` and `node_modules` excluded): each inline HTML
//! template is lowered L1 -> L2 -> L3 and run through the `s3` `OPTIMIZE`
...
fn file_remarks(path: &str, source: &str, out: &mut Vec<CorpusRemark>) {
    ...
    let mut lowered = vize_l2_to_l3::lower(&allocator, &s2.root);
...
#[test]
fn l3_extraction_remarks_match_the_committed_baseline() {
    let root = repo_root();
