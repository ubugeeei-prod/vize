...
use vize_l2_to_l3::{FolioPartitionFact, L3PartitionFolio, PartitionKind, lower};
...
fn full_print_is_identity_on_canonical_text() {
    let folio = L3PartitionFolio::parse(CANONICAL).expect("canonical text parses");
...
fn the_live_mirror_prints_the_lowered_facts_exactly() {
    ...
    let folio = L3PartitionFolio::of(&lowered.partition);
    assert_eq!(folio.print_to_string(FolioMode::Full).as_str(), CANONICAL);
    assert_eq!(
        L3PartitionFolio::parse(CANONICAL).expect("canonical text parses"),
        folio
    );
}
...
fn structural_round_trip_holds_for_both_kinds() {
    ...
    assert_eq!(L3PartitionFolio::parse(printed.as_str()), Ok(folio));
}
