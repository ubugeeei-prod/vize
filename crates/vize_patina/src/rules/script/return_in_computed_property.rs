...
mod tests {
    ...
    fn test_writable_getter() {
        ...
        let valid =
            "const v = computed({ get() { return count.value }, set(x) { count.value = x } })";
    ...
    fn test_invalid_return_only_in_nested_function() {
        ...
        assert_eq!(
            lint("const d = computed(() => { const h = () => { return count.value } })")
                .error_count,
            1
        );
    }
    ...
    fn test_valid_return_in_branch() {
        assert_eq!(
            lint("const s = computed(() => { if (c.value) { return 'p' } return 'n' })")
                .error_count,
            0
        );
    }
