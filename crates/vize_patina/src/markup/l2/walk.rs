//! Element-tree walks over an L2 artifact.
//!
//! Scopes are regions in L2; the element tree the facade presents keeps the
//! authored carriers visible: a `ui.if` branch or `ui.for` region unwrapped
//! from a `<template>` surfaces that template (read off L1) around its region.
...
pub(in crate::markup) enum L2Step<'a> {
...
/// The step a scope region contributes: its unwrapped carrier when L1
...
pub(in crate::markup) fn scope_region<'a>(
    ...
    match doc.unwrapped_carrier(span, region) {
        Some(carrier) => L2Step::Element(
            MarkupElement::from_l2_carrier(carrier, doc, region, span),
            region,
        ),
        None => L2Step::Region(region),
    }
}
...
fn walk_step<'a>(
    ...
    match step {
        L2Step::Element(element, region) => {
            enter(element);
            walk_region(doc, region, enter, exit);
            exit(element);
        }
        L2Step::Region(region) => walk_region(doc, region, enter, exit),
    }
}
    ...
    for op in ops {
        match op {
            Op::Element(_) | Op::Component(_) | Op::Slot(_) => {
                ...
                let step =
                    L2Step::Element(MarkupElement::from_l2(element, doc), element.children());
