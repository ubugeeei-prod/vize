//! [`L2Bound`]: one attached L2 binding op, with the authored L1 attribute it
...
//! (`v-cloak.foo`) survives only on L1, where the facade reads it back.

use super::binding::op_span;
use super::surface::SurfaceDirective;
use vize_l0::Span;
use vize_l2::op::{BindingOp, DynamicName};

/// One attached binding op and its authored spelling.
#[derive(Clone, Copy)]
pub(in crate::markup) struct L2Bound<'a> {
    pub(in crate::markup) op: &'a BindingOp<'a>,
    pub(in crate::markup) surface: Option<&'a vize_l1::Attribute<'a>>,
}
