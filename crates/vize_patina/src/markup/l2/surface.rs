//! L1 surface reads for the L2 facade: authored attributes the L2 op tree
//! consumed or dropped, located on the lossless L1 tree instead of a private
//! re-scan of the source (architecture: surface-shaped consumers read L1).
//!
//! L1 is a contiguous partition of the source, so an element is found by its
//! start offset (the L2 op span starts at the L1 element's `<tag` token) with a
...
use vize_l0::Span;
use vize_l1::{Attribute, Element, SurfaceChild, SurfaceTree, Token};
...
pub(in crate::markup) fn offset_in(source: &str, slice: &str) -> u32 {
    ...
    debug_assert!(
        ...
        "L1 handed the facade a slice outside the lowered source"
    );
...
/// The L1 element whose `<tag` token starts at `start`.
...
/// The L1 sibling list holding the element whose `<tag` token starts at
/// `start`.
...
/// allocating — the shapes `vize_l1_to_l2`'s classifier splits (`v-name`,
