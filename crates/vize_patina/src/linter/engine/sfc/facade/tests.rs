...
fn disabled_rules_and_severity_overrides_apply_to_the_batch() {
    ...
    let source = "<template><textarea style=\"color:red\">{{ value }}</textarea></template>";
...
fn sfc_batch_does_not_inspect_script_jsx() {
    let result = linter().lint_sfc(
        r#"<script setup lang="tsx">const view = <button style="color:red">go</button></script>"#,
        "test.vue",
    );
