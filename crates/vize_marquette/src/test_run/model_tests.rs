...
pub(crate) fn filled(fill: char, length: usize) -> vize_l0::String {
    let mut output = vize_l0::String::with_capacity(length);
    for _ in 0..length {
        output.push(fill);
    }
    output
...
fn digest(fill: char) -> vize_l0::String {
    filled(fill, 64)
}

fn retained(fill: char) -> TestRunRetainedEvidence {
    let fingerprint = digest(fill);
    let mut reference = vize_l0::String::from("sha256:");
