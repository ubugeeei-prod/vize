...
pub(super) fn acquire(
    ...
    let lease = tempfile::Builder::new()
        .prefix(&vize_l0::cstr!(".lease-{}-", std::process::id()))
        .tempfile_in(&entry)?;
    lease.as_file().lock()?;
...
fn record_project_access(project: &Path) -> Result<(), std::io::Error> {
    ...
    fs::write(path, vize_l0::cstr!("{:039}\n", elapsed.as_nanos()))
}
...
impl CacheLock {
    fn acquire(path: &Path, scope: &str) -> Result<Self, std::io::Error> {
            ...
            && (metadata.file_type().is_symlink() || !metadata.is_file())
        {
            return Err(std::io::Error::new(
                ErrorKind::InvalidData,
                vize_l0::cstr!("Nuxt config {scope} lock is not a regular file"),
            ));
        }
        ...
        loop {
            ...
            if started.elapsed() >= LOCK_TIMEOUT {
                return Err(std::io::Error::new(
                    ErrorKind::TimedOut,
                    vize_l0::cstr!("timed out waiting for the Nuxt config {scope} lock"),
                ));
            }
...
fn project_lock_path(project_cache: &Path) -> Result<PathBuf, std::io::Error> {
    ...
    Ok(bucket.join(vize_l0::cstr!(".project-{}.lock", digest.to_string_lossy())))
}
