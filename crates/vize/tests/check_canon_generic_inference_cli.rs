...
fn unique_case_dir(name: &str) -> PathBuf {
    workspace_root()
        ...
        .join("vize-tests")
        .join("tests")
        .join(cstr!("check-canon-generic-{name}-{}", std::process::id()).as_str())
}
...
defineProps<{ modelValue?: T | null }>()
defineEmits<{ "update:modelValue": [value: T | null] }>()
</script>
...
const modelValue = shallowRef<T | null>(null)
</script>
...
fn check_generic_model_type_infers_from_value_key_prop() {
    ...
    let project_root = create_case(
        ...
        &[
            (
                "src/Child.vue",
                r#"<script setup lang="ts" generic="T, K extends keyof T | undefined = undefined">
type ModelValue<T, K> = K extends keyof T ? T[K] : T

defineProps<{
  items: T[]
  valueKey?: K
  modelValue?: ModelValue<T, K> | null
}>()

defineEmits<{
  "update:modelValue": [value: ModelValue<T, K> | null]
}>()
</script>
...
<template>
  <Child v-model="value" :items value-key="id" />
</template>
"#,
            ),
        ],
