export type Slot = (...args: any[]) => unknown[];
export type Slots = Readonly<Record<string, Slot | undefined>>;
export interface VNode {
  readonly __v_isVNode: true;
}
type DistributeRef<T> = T extends Ref<infer V> ? V : T;
export type ShallowUnwrapRef<T> = { [K in keyof T]: DistributeRef<T[K]> };

export interface ComponentPublicInstance<Props = {}> {
  $props: Props;
