...
#[cfg(feature = "native")]
#[test]
fn did_open_defers_but_preserves_initial_type_diagnostics() {
    ...
    open_vue(
        server,
        uri,
        "<script setup lang=\"ts\">const answer: number = 42</script>\n",
        1,
    );

    // The foreground didOpen path returned before attempting native type
    // diagnostics. Previously the missing runtime failure was recorded before
    // didOpen completed, which is the same blocking sequence the cold
    // completion benchmark observes.
