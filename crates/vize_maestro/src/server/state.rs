...
impl ServerState {
    ...
    fn update_art_virtual_docs(&self, uri: &Url, content: &str) {
        use crate::virtual_code::{
            ScriptCodeGenerator, TemplateCodeGenerator, VirtualDocuments, find_variant_state_blocks,
        };
        ...
        for (index, variant) in art_desc.variants.iter().enumerate() {
            if let Some(ref loc) = variant.loc {
                state_blocks.extend(find_variant_state_blocks(
                    content,
                    loc.start as usize,
                    loc.end as usize,
                    index,
        ...
        if let Ok(descriptor) = vize_atelier_sfc::parse_sfc(content, sfc_options) {
            if let Some(ref script_setup) = descriptor.script_setup {
                let mut script_doc = generate_art_script_setup_virtual_doc(
                    ...
                    Some((
                        ...
                        script_setup.loc.start as usize,
                    )),
                    &state_blocks,
                    content,
                );
            ...
            } else if !state_blocks.is_empty() {
                let mut script_doc =
                    generate_art_script_setup_virtual_doc(base_uri, None, &state_blocks, content);
                script_doc.uri = vize_carton::cstr!("{base_uri}.__script_setup.ts").to_string();
                docs.script_setup = Some(script_doc);
            }
...
fn generate_art_script_setup_virtual_doc(
    base_uri: &str,
    script_setup: Option<(&str, usize)>,
    state_blocks: &[crate::virtual_code::ArtStateBlockRange],
    source: &str,
    ...
    content.push_str("// Virtual TypeScript for <script setup> and <variant><state>\n");
    content.push_str("// Generated by vize_maestro\n\n");

    if let Some((script, source_start)) = script_setup {
        content.push_str("// Root <script setup>\n");
        ...
        if !script.is_empty() {
            mappings.push(SourceMapping::with_features(
                SourceRange::new(source_start as u32, (source_start + script.len()) as u32),
    ...
    for state in state_blocks {
        let state_content = state.content(source);
        content.push_str(&format!(
            "// Variant {} <{}> state\n",
            state.variant_index, state.tag_name
        ));
        content.push_str(&format!(
            "function __VIZE_art_variant_{}_state() {{\n",
            state.variant_index
        ));

        let generated_start = content.len() as u32;
        content.push_str(state_content);
        let generated_end = content.len() as u32;
        if !state_content.is_empty() {
            mappings.push(SourceMapping::with_features(
                SourceRange::new(state.content_start as u32, state.content_end as u32),
                SourceRange::new(generated_start, generated_end),
                MappingFeatures::all(),
            ));
        }
...
"#;
    ...
    #[test]
    fn update_art_virtual_docs_isolates_variant_state_in_script_setup_doc() {
        let state = ServerState::new();
...
"#;
        ...
        assert!(script_setup.content.contains("Root <script setup>"));
        assert!(
            script_setup
                .content
                .contains("function __VIZE_art_variant_0_state()")
        );
        assert!(
            script_setup
                .content
                .contains("function __VIZE_art_variant_1_state()")
        );
