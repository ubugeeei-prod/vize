...
//! coordinates.
//!
//! Import rewrites compose with the JSX mapping: native positions first map
//! back through the importer rewrite, then through the original JSX lowering.
...
pub(super) fn source_offset_to_virtual_position(
    ...
    let mapping = mapping_for_source_offset(&projection.mappings, source_offset)?;
    let gen_offset = source_offset_to_generated(mapping, source_offset);
    Some(byte_offset_to_position(
        &projection.code,
        projection
            .import_source_map
            .get_virtual_offset(gen_offset as u32) as usize,
    ))
}
...
pub(super) fn virtual_range_to_source(
    ...
    let virtual_ts = &projection.code;
    let mappings = &projection.mappings;
    let import_map = &projection.import_source_map;
    ...
    let start_gen = import_map.get_original_offset(start_gen as u32) as usize;
    let end_gen = import_map.get_original_offset(end_gen as u32) as usize;
...
mod tests {
    ...
    fn round_trip(source: &str, marker: &str) {
        ...
        let (line, ch) =
            source_offset_to_virtual_position(&generated, source_offset).expect("forward map");
        // The generated byte at (line, ch) should correspond back to the same
        // source offset region.
        let (src_start, _, _, _) =
            virtual_range_to_source(&generated, source, line, ch, line, ch + 1).expect("back map");
    ...
    fn maps_component_prop_diagnostic_to_authored_name_subspan() {
        ...
        assert_eq!(
            virtual_range_to_source(
                &JsxVirtualTs {
                    code: virtual_ts.into(),
    ...
    fn maps_member_access_trigger_in_setup_body() {
        ...
        let mapped = source_offset_to_virtual_position(&generated, dot);
        assert!(
            mapped.is_some(),
