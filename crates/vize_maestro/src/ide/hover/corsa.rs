...
impl HoverService {
    ...
    pub(super) async fn hover_template_with_corsa(
        ...
        if !crate::ide::is_in_vue_template_expression(&ctx.content, ctx.offset)
            && let Some(hover) =
                Self::hover_template_ref_with_corsa(ctx, corsa_bridge.as_ref()).await
        {
            return Some(hover);
        }
    ...
    async fn hover_template_ref_with_corsa(
        ctx: &IdeContext<'_>,
        corsa_bridge: Option<&Arc<CorsaBridge>>,
    ) -> Option<Hover> {
        ...
        let doc = crate::ide::corsa_support::open_canonical_virtual_document(ctx, bridge).await?;
        let (line, character) = crate::ide::corsa_support::canonical_source_offset_to_position(
            &doc,
            target.binding_start,
        )?;
        let hover = bridge
            .hover(&doc.request_uri, line, character)
            .await
            .ok()??;
        let mut converted = Self::convert_lsp_hover(hover);
        converted.range = Some(target.value_range(&ctx.content));
        Some(converted)
    }
