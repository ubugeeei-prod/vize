...
pub(crate) fn native_dom_attribute_info(
    ...
    let is_boolean = vize_l0::is_boolean_attr(&normalized_attr);
    let category = if vize_l0::is_html_tag(tag_name) {
        "HTML attribute"
    } else if vize_l0::is_svg_tag(tag_name) {
        "SVG attribute"
    } else {
        "MathML attribute"
    };
...
fn normalize_attribute_name(tag_name: &str, attr_name: &str) -> Option<String> {
    ...
    if vize_l0::is_svg_tag(tag_name)
...
fn is_known_native_attribute_for_tag(tag_name: &str, attr_name: &str) -> bool {
    if vize_l0::is_html_tag(tag_name) {
    ...
    if vize_l0::is_svg_tag(tag_name) || vize_l0::is_math_ml_tag(tag_name) {
