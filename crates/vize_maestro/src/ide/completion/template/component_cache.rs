...
pub(super) fn cached_component_metadata(
    ...
    let uri = tower_lsp::lsp_types::Url::from_file_path(resolved).ok()?;
    ctx.state.upsert_artifact_source(&uri, &content)?;
    let artifact = ctx.state.sfc_descriptor(&uri)?;
    let relief = ctx.state.sfc_relief(&uri)?;
    let croquis = ctx.state.sfc_croquis(&uri)?;
    let metadata = Arc::new(extract_component_metadata(
        artifact.descriptor()?,
        Some(croquis.analysis()),
        relief.as_ref().as_ref().map(|syntax| syntax.snapshot()),
