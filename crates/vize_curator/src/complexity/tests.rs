...
fn complexity_markdown_explains_dimensions_and_hotspots() {
    ...
    assert!(markdown.contains(
        "reactive-graph (10) via template cyclomatic=3, prop edges=3, provide fanout=3, reactive cycles=1"
    ));
}
...
fn complexity_markdown_handles_empty_hotspots() {
    ...
    assert!(markdown.contains("No component templates were analyzed."));
