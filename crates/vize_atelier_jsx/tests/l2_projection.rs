//! P2-16 JSX-to-L2 projection witnesses.

use vize_atelier_jsx::{JsxLang, lower_source};
use vize_l0::Allocator;
use vize_l2::op::{BindingOp, DynamicName, Namespace, Op};

#[test]
fn dynamic_bind_props_project_to_l2_bindings() {
    ...
    let l2 = root.l2.as_ref().expect("dynamic bindings are admitted");
    assert_eq!(l2.op_count, 3);
...
#[test]
fn v_on_directives_project_to_l2_on_bindings() {
    let allocator = Allocator::new();
    ...
    let l2 = root.l2.as_ref().expect("event binding is admitted");
...
#[test]
fn event_handler_directives_project_to_l2_on_bindings() {
    let allocator = Allocator::new();
    ...
    let l2 = root.l2.as_ref().expect("event bindings are admitted");
    assert_eq!(l2.op_count, 3);
    let Op::Element(element) = &l2.root.ops[0] else {
        panic!("root is an element");
    };
...
#[test]
fn component_default_children_project_to_l2_regions() {
    let allocator = Allocator::new();
    ...
    let l2 = root.l2.as_ref().expect("component children are admitted");
    assert_eq!(l2.op_count, 3);
...
#[test]
fn forwarded_v_slots_projects_to_l2_slots_spread() {
    let allocator = Allocator::new();
    ...
    let l2 = root.l2.as_ref().expect("forwarded slots project to L2");
    assert_eq!(l2.op_count, 2);
    assert!(l2.features.has_slot_carriers());
    let Op::Component(component) = &l2.root.ops[0] else {
        panic!("root is a component");
...
#[test]
fn fragment_children_project_to_one_l2_region() {
    let allocator = Allocator::new();
    ...
    let l2 = root.l2.as_ref().expect("fragment children are admitted");
    assert_eq!(l2.op_count, 2);
    assert_eq!(l2.root.ops.len(), 2);
    let Op::Element(span) = &l2.root.ops[0] else {
        panic!("first fragment child is an element");
    };
    assert_eq!(span.tag, "span");
    assert!(span.children.ops.is_empty());
    let Op::Interpolation(count) = &l2.root.ops[1] else {
