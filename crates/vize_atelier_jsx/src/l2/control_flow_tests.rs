...
fn logical_and_child_projects_to_l2_if() {
    ...
    let l2 = root.l2.as_ref().expect("logical child projects to L2");
    assert_eq!(l2.op_count, 4);
    assert!(l2.features.has_if_ops());
    assert!(!l2.features.has_for_ops());
    let Op::Element(root_element) = &l2.root.ops[0] else {
...
#[test]
fn map_child_projects_to_l2_for_scope() {
    let allocator = Allocator::new();
    let source = "const App = () => <ul>{items.map((item, index) => <li>{item}</li>)}</ul>;";
    let lowered = lower_source(&allocator, allocator.as_oxc(), source, JsxLang::Jsx);
    let root = lowered.roots.first().expect("one JSX root");

    let l2 = root.l2.as_ref().expect("map child projects to L2");
    assert!(l2.features.has_for_ops());
    assert!(!l2.features.has_if_ops());
    let Op::Element(root_element) = &l2.root.ops[0] else {
        panic!("root is an element");
    ...
    let facts = l2.scopes.get(for_id).expect("ui.for records scope facts");
