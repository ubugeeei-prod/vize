//! P2-16 differential witnesses for JSX VDOM L2 migration.
...
#[test]
fn l2_vdom_admitted_cases_match_relief_codegen() {
    let cases = [
        (
            "element",
    ...
    for (name, source, lang) in cases {
        let l2 = compile_case(source, lang, EmitRoute::ForceL2);
        let relief = compile_case(source, lang, EmitRoute::ForceRelief);

        assert_eq!(
            l2.preamble, relief.preamble,
            "{name}: L2 VDOM preamble diverged from Relief"
        );
        assert_eq!(
            l2.code, relief.code,
            "{name}: L2 VDOM code diverged from Relief"
        );
    }
}
...
fn compile_case(source: &str, lang: JsxLang, route: EmitRoute) -> Output {
    ...
    match route {
        EmitRoute::ForceL2 => root.root.children.clear(),
    ...
    assert!(component.map.is_none(), "L2 parity lane is source-map-free");
