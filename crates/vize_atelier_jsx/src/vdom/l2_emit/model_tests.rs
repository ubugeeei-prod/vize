...
#[test]
fn component_v_model_emits_from_l2() {
    let source = "const A = () => <Input v-model={value} />;";
    let l2 = compile_case(
        source,
        EmitRoute::ForceL2,
        "component v-model projects to L2",
    );
...
#[test]
fn component_v_model_static_arg_modifiers_emit_from_l2() {
    let source = "const A = () => <Input v-model={[value, \"foo\", [\"trim\"]]} />;";
    let l2 = compile_case(
        source,
        EmitRoute::ForceL2,
        "component v-model argument with modifiers projects to L2",
    );
    let relief = compile_case(
        source,
        EmitRoute::ForceRelief,
