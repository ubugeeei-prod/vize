...
fn source_map_option_stays_on_relief_vdom_fallback() {
    ...
    assert_l2_emit_declines(SOURCE_MAP_SOURCE, &options, "source map");
...
fn hoist_static_option_stays_on_relief_vdom_fallback() {
    ...
    assert_l2_emit_declines(HOIST_STATIC_SOURCE, &options, "static hoisting");
...
fn cache_handlers_option_stays_on_relief_vdom_fallback() {
    ...
    assert_l2_emit_declines(CACHE_HANDLERS_SOURCE, &options, "handler caching");
...
fn assert_l2_emit_declines(source: &str, options: &VdomCompileOptions, projection: &str) {
    ...
    let l2 = root.l2;
    assert!(
        super::root_is_supported(l2.as_ref().expect(projection)),
        "{projection} should be L2-supported before option fallback"
    );

    let emit = super::try_emit_l2_vdom(
        &allocator,
        l2,
        false,
        None,
