...
const L2_SUPPORTED_SOURCE: &str = "const A = () => <div>{count}</div>;";

#[test]
fn babel_compat_surface_options_decline_l2_emit() {
    assert_l2_emit_declines_with_compat(
        L2_SUPPORTED_SOURCE,
        VdomCompatOptions {
...
fn assert_l2_emit_declines_with_compat(
    source: &str,
    ...
    assert!(
        ...
        "{projection} should be L2-supported before compat fallback"
    );
