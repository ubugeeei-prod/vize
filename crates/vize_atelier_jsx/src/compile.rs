...
impl JsxCompileOutput {
    pub(crate) fn from_owned_components(
        components: Vec<JsxComponent>,
        source: &str,
    ...
    /// A v3 map relocated from the component fragment into the assembled
    /// module and normalized to the host's real source filename.
    pub fn module_source_map(&self, module_code: &str, filename: &str) -> Option<String> {
        if self
            .components
            .iter()
        ...
        let fragment = component.map()?;
        let generated_start = module_code.find(component.code())?;
        source_map::relocate_fragment_map(
            fragment,
            module_code,
            generated_start,
            filename,
